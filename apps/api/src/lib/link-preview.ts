// 利用者が書いた URL のページを Worker が fetch して、題名と画像を取る（040）。
// 「利用者の URL をサーバが fetch する」口はここ 1 本だけ（security-requirements.md 6節）。
// 6節の条件はこのファイルの中で全部満たし、呼ぶ側（want.ts）は条件を知らない。
//
// 数値の根拠（artifacts/040/spike.md の実測）:
...
export const LINK_PREVIEW_USER_AGENT = "nisoine-link-preview/1 (+https://nisoine.com)";
...
export interface LinkPreview {
  ...
  // 実際に読んだページの URL（リダイレクト後）。読めなければ null。
  // 呼ぶ側は Amazon の短縮 URL（amzn.asia）をこれで正規化する
...
// IPv4 リテラル（"127.1" のような短縮形も URL パーサが数字とドットだけにする）と
// IPv6 リテラル（URL.hostname は "[::1]" の形）
function isIpLiteral(hostname: string): boolean {
  if (hostname.startsWith("[")) return true;
  return /^\d+(\.\d+){0,3}$/.test(hostname);
}

// IP リテラル・localhost・.local・.internal は取りに行かない（Cloudflare 側でも塞がれているが二重に）。
// リダイレクトの行き先も同じ検査を通す
...
// http / https 以外は取りに行かない（入力スキーマでも弾いている。二重）
...
// `<meta property="og:title" content="…">` と content が先の書き方の両方を拾う。
// 拾うのは決まった属性値だけで、DOM は組み立てない（6節）。
// 属性値に `<` `>` を許さない（閉じ引用符の無い壊れた属性が次のタグまで値になるのを防ぐ）。
// 引用符は開いた種類と同じもので閉じる（`Levi's`・`Kids'` は商品名に普通にある）
const QUOTED_VALUE = `(?:"([^"<>]*)"|'([^'<>]*)')`;
...
// 店ごとの規則は Amazon のホストに限る（効く範囲を狭くすれば、壊れたときの範囲も狭い。6節）
...
// 全店共通で maxLength で切る（入力欄の初期値なので利用者が直せる）。コードポイントで数える（サロゲートペアを割らない）
...
// 文字コードは Content-Type の charset で選ぶ。無い・未知なら UTF-8
// （楽天は EUC-JP。Workers の TextDecoder("EUC-JP") は動く）
...
// 画像は Content-Type と先頭バイトの両方を見る（6節）
...
// 外部の応答ヘッダをそのままログに流さない（本文を 200 文字で切るのと同じ理由）
...
// リダイレクトは 3 回まで。redirect: "manual" で 1 段ずつ辿り、行き先も同じ検査を通す
// （fetch に任せると行き先を検査できない）。
// Cookie・認証ヘッダは送らない（Workers の fetch は Cookie を持たず、ヘッダはここに並べたものだけ）
...
async function fetchImage(
  ...
  // 閉じているのは下の先頭バイトの検査で、ここは失敗理由を分けるため（型が違うのか、先頭バイトが合わないのか）。
  // `in` だと `constructor` 等のプロトタイプ名が通るので hasOwn
...
export async function fetchLinkPreview(url: string, options: FetchLinkPreviewOptions): Promise<LinkPreview> {
  ...
  try {
    ...
    // 店ごとの規則は、実際に読んだページ（リダイレクト後）のホストで判定する
    // （Amazon アプリの共有は amzn.asia の短縮 URL）
