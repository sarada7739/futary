...
**最終更新**: 2026-09-14 / セッションB。**044（AIまとめに名前を出す）: 実装完了。PR #321（`task/044-ai-names-impl`。A の起票ブランチ `task/044-ai-names` が残っていたので別名）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の実機で「1 回作り直すと名前で呼ばれる」
