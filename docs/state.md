...
**最終更新**: 2026-08-27 / セッションB（004実装完了、Rレビュー待ち）
...
（003のL14と同じ制約）。ブランチ `task/004-couple-and-invite` にコミット済みで、
Rレビュー・PR作成はこれから（下記「次の一手」参照）。
...
| M | タスク | 内容 | 状態 |
|---|---|---|---|
| M1 | 001〜005 | 足回り・デザイン基盤・認証・ペア成立・認可 | 進行中（001・002 完了、003 マージ済み・実ログイン確認待ち、004 実装完了・PR #9 作成済み・Rレビュー待ち） |
...
- 004-couple-and-invite（ブランチ `task/004-couple-and-invite`、PR #9）: 実装・
  テスト（52件緑）・security-auditor（2回、High/Medium指摘は全て修正済み）・
  PR作成まで完了。Rレビュー待ち。オンボーディング画面の実機確認
  （実際のGoogleログイン）は003と同じ理由で未実施
...
1. 004（PR #9、ブランチ `task/004-couple-and-invite`）を R がレビューし、
   受け入れ判定を行う
2. 人間が Google Cloud Console で OAuth クライアントを作成し、`.dev.vars` の
   ...
   まとめて実機確認する
3. 上記が済んだら 003・004 を「完了タスク」に移動する
4. `docs/tasks/005-authorization-middleware.md` に着手（新しいセッションで、
   `/clear` してから）
...
| L24 | `docs/security-requirements.md` 4節は「invite.acceptの失敗をIP単位で制限（例: 10回/時間）」とだけ書いているが、実装は `user_id` 10回/時間 + `ip_address` 50回/時間の二本立て（`apps/api/src/procedures/couple.ts`）。004監査Highの指摘（IPv6の/64ローテーションでIP単位のみだと回避できる）を受けた強化で、CGNATを考慮してIP側を緩めた判断も理由が明記されているが、**要件ドキュメント側が更新されていない**。003で指摘された「ドキュメントが実装していない統制を主張する」の逆パターン（今回は実装の方が要件より手厚いが、読んだ人には伝わらない）（Rレビュー004 必須対応②で指摘） | 要件だけ読んだ人は「IPのみ・10回/時間」と誤解する | A に要件4節の更新を依頼。B は設計ドキュメントを書き換えられないため、ここに起票のうえ次のA呼び出し（エスカレーション）で連絡する |
