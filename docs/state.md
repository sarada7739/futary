...
**最終更新**: 2026-08-31 / セッションB（**021（予定の持ち主とふたりの予定）
がRの受け入れを得てmainへマージ済み（PR #140。ブランチも削除済み）。**
スキーマ（`is_shared`列・CHECK）・契約（`isShared`/`canEdit`）・
`event.update`/`event.delete`の認可WHERE句・`event.list`の`canEdit`計算・
UI（「ふたりの予定にする」・編集不可の表示・元がplan以外のときは選択肢
からplanを外す）を実装した。security-auditorでHigh 1件（マイグレーションの
SELECT不具合）・Medium 3件・Low 3件を検出、すべて対応済み。実装中に
「kindの変更が権限を奪う」問題（相手を締め出せる経路）をめぐりA・Rの
往復が3回発生した——**Aの設計が3回続けて範囲を狭く取りすぎていたことを
Rが3回とも発見**（更新前しか見ていない→実行者しか見ていない→1回の操作
しか見ていない）。最終的に権限の条件を「操作が安全か」ではなく「状態遷移
が許されるか」で書き直し、`kind<>'plan'`への変換自体を区分をまたぐ限り
拒む形で決着した。security-report.mdの記録も監査原文の範囲
（「片方の判断だけで」）に書き戻した。apps/api 277件・apps/app 107件
すべて緑。**カレンダー画面は認証必須のためB側では実機確認ができない
（`artifacts/021/manual-check.md`）。次は021の人間の実機確認を依頼する**）。
...
**次は021の人間の実機確認、その後は014（ゲストデモ）に進む見込み**
（シード仕様はPR #119で固め済みとRから聞いている）。
...
23. **021の人間の実機確認を依頼する。**確認項目は`artifacts/021/
    manual-check.md`参照
...
| L85 | 021（予定の持ち主とふたりの予定）はRの受け入れを得てmainへマージ済み（PR #140）だが、カレンダー画面が認証必須のためB（自動化）は実機確認ができない | 自分の予定が相手から編集できない見た目になっているか、「ふたりの予定」チェックが実際に相手の編集権限を変えるか、種別の選択肢からplanが適切に出し分けられているかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/021/manual-check.md`参照。**次はこの受け入れ依頼を人間へ行う** |
