...
**最終更新**: 2026-09-14 / セッションB。**043（リリース履歴）: 実装完了。PR #315（`task/043-release-notes-impl`。A の起票ブランチ `task/043-release-notes` が残っていたので別名）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の実機で「シート → 閉じる → 出ない」
