...
**最終更新**: 2026-08-30 / セッションB（**010（カレンダーAPI）を実装し、
PR `task/010-calendar-api` を作成した。Rレビュー待ち。** `events`テーブル・
`lib/date.ts`（JST集約）・`event.list/create/update/delete`を実装し、
繰り返し記念日の射影（年またぎ・400日上限・3暦年に触れる窓・うるう年02-29の
両方向）をテストで証明した。005の認可テスト（`security-requirements.md` 3節）も
`event`系4手続きを追加した上で緑。**010単体はsecurity-requirements.md 10節1の
必須監査対象に該当しない**（認証・招待・画像アップロード・認可ミドルウェアを
触っていない。006・008と同じ扱い）ため、単体監査はせずM3の他タスクと
まとめる方針とした（ユーザー確認済み）。`artifacts/010/test-results.md`参照。
...
- 010-calendar-api（ブランチ `task/010-calendar-api`。実装・テスト・型チェック・lint
  すべて完了し、Rレビュー待ち。`artifacts/010/test-results.md`参照。単体でのsecurity-auditor
  監査は必須対象外〈006・008と同じ扱い〉のため未実施。M3の他タスクとまとめて監査する）
