...
**最終更新**: 2026-08-30 / セッションB（**017（画像の全画面表示）、Rの受け入れを得て
mainへマージ済み（PR #80）。** Rレビューで「画像の外側のみ閉じる」実装の当たり判定
バグ（レターボックス部分をPressableが覆い閉じない）を指摘され、Aの判断で「どこを
タップしても閉じる」仕様に変更して解決（当たり判定という概念自体を無くす方針。
PR #81で仕様先行更新、PR #80で実装修正）。**L59・017ともコード側は完了。
残るは人間の実機確認のみ**（Rが列挙した4項目。`artifacts/017/manual-check.md`
参照）。次はM3本体、010（カレンダーAPI）に着手する）
...
7. **次はM3本体、010（カレンダーAPI）に着手する。** `events`テーブルに記念日・
   予定・会った日を統合する設計。`couple_id`スコープと日付・タイムゾーン
   （JST固定）の扱いが要点（`conventions.md`6節: 日付・タイムゾーンが絡む
   計算は必ずテストを書く対象）
