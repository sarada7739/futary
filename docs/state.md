...
送った直後に出た論点（L81〜L84）を順次解決中。**L83（FABの影が正方形に
見える不具合）はmainへマージ済み（PR #130）。L81・L82（パネルの見た目・
新規素材の扱い）はAが判断し（PR #131・#132）、B側でパネルアイコン6個の
描き起こしとグリッド実装まで完了、Rレビュー依頼中（PR #133）。**その後
人間から新規に、マイページ「ホーム上部の表示」ボタンがiPhone幅で単語の
途中で折り返されるという報告（L84）があり、`fix/profile-primary-date-wrap`
で修正しRレビュー依頼中。**次はPR #133・このPRのRレビュー結果を待つ**）。
