...
（B側の実装判断だったが伝え損ねていたと回答済み。L81としてAへ判断を依頼中）
(2) FABボタンの周囲に薄い四角い影が見える不具合の報告、および新規デザイン
素材（`docs/sample/透過素材/UlQMVnAB.png`）の配置。**FABの不具合は
`fix/fab-shadow-square`で修正し、Rの受け入れ・提案2件（tokens.tsへの注記、
FAB寸法の一本化）も反映のうえmainへマージ済み（PR #130。L83解決）。**
新規素材はJPEGを.pngに誤って拡張子を付けたもの（透過なし）と判明し、
Aへ報告済み（L82。回答待ち）。**次はAからのL81・L82の回答待ち。021には
着手継続する**）。ホームを投稿一覧から「状態を見て入口を選ぶ画面」に
