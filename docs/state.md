...
**最終更新**: 2026-08-30 / セッションB（**017（画像の全画面表示）の実装完了。**
`apps/app/components/image-viewer.tsx`（新規）で`Modal`ベースの全画面表示を実装。
`post.list`の署名付きURLをそのまま使い新しい手続きは作っていない（005の認可
テスト不変で確認済み）。開閉の3導線（×ボタン・バックドロップタップ・Esc/
Android戻る）を画面結合テスト10件で固定。型チェック・lint・テストすべて緑。
**残るは人間の実機確認のみ**（L59とまとめて依頼する。`artifacts/017/
manual-check.md`参照）。M3の実行順は L59（実装済み）→ 017（実装済み）→ 010〜013）
...
3. ~~017（画像の全画面表示）に着手する~~ → **実装完了。** `Modal`ベースの
   `image-viewer.tsx`を新設。閉じる導線3つを画面結合テストで固定。
   **人間の実機確認（L59とまとめて依頼）だけ残っている**
