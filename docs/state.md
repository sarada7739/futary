...
**最終更新**: 2026-09-26 / セッションB。**065（LP の AI まとめの帯、スマホで文字を上下の真ん中に）: PR（`task/065-ai-band-center`）。R の手番。**
- タスク定義の 2 行（`justify-content: center`・`text-wrap: balance`）だけでは 375 幅の文字が動かなかった（`.ai-band-copy` の `flex: 1 1 auto` が縦並びで帯の高さいっぱいに伸びる）→ 720px 以下で `.ai-band-copy { flex-grow: 0 }` を足した。A に知らせた
- 375 幅: 文字の上下 24・84 → 54・54。1280 幅は main と画素で同じ。`artifacts/065/`
- 人間の手番: 本番デプロイの後に iPhone で帯を見る。064 の `seed:remote`（wrangler の再ログインで D1・R2 の権限を取り直してから）。続き: 061 段階3・062・063 のデプロイの承認 → Safari のタブを開き直してピンクのタイムラインを撮る・ピンクのホーム。060・059・058・057・056 の確認・047 の観点・Stripe の本番の Price・Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-26 / セッションA。**人間が本番で 062・063 を OK。065（LP の AI まとめの帯、スマホで文字を上下の真ん中に）を起票。B の手番。**
- 人間の指示（iPhone の Safari）: 帯の文字が上に寄る → 真ん中に。原因は 720px 以下の縦並びに `justify-content: space-between` が残り、059 で子が 1 つになったこと。説明の 1 文字落ち（「…ま / す。」）も `text-wrap: balance` で
- 人間の手番: 061 段階3 のタブバー（iPhone の Safari。二重化・帯が消えたか）。（続き）060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
...
**最終更新（旧）**: 2026-09-26 / セッションA。**064 完了: 人間がデプロイし `seed:remote` を流して、本番のデモ（ゆいのほしいもの）にグンゼが出るのを確かめた。B は待機。**
- 同じデプロイで 061 段階3・062・063 も本番に入った（確認はまだ）
- 人間の手番: iPhone の Safari でタブバー（061 段階3。ぼかし 1 層で二重化・帯が消えたか）とピンクのホームの写真タイル（062）。（続き）060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: 061 の 0節 #3・#4 の書き換え（A）・iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50・2027 年 2 月ごろ: 2028 年の祝日の表。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションB。**064: R 受け入れ（必須修正なし）→ main（#444）を取り込み CI 緑 → #443 を squash merge。B は待機。**
- （A 追記）064 の 1 節の件数の食い違い（R の知らせ）を「ゆい 3・れん 2」に直した
- 判定 `artifacts/064/review.md`。グンゼはゆいのほしいもの（ゆい 3・れん 2）。seed:local / seed:remote が通るようになった
- **人間の手番: デプロイの後に `pnpm --filter @futary/db seed:remote`**（本番のデモにグンゼが出る）。続き: 061 段階3・062・063 のデプロイの承認 → Safari のタブを開き直してピンクのタイムラインを撮る・ピンクのホーム。060・059・058・057・056 の確認・047 の観点・Stripe の本番の Price・Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションA。**064 は PR #443（R の手番）。B の知らせ 2 つを定義に反映した。**
- グンゼは**ゆい**のほしいもの（人間の指示。デモはゆいの一覧が最初に出る）。064 の 0節 #1・T1、040 の 6 節、`docs/sample/README.md` を「ゆい 3・れん 2」に
- `seed:local`・`seed:remote` は 058 から落ちていた（`packages/date` の拡張子の無い `export *` を Node が読めない）。#443 で `.ts` を付けた。`conventions.md` 2節に「`packages/*/src` の相対 import には `.ts`」を足した
- 人間の手番: #443 のマージ・デプロイの後に `pnpm --filter @futary/db seed:remote`。（続き）061 段階3・062・063 のデプロイの承認 → iPhone でタブバー・ピンクのホーム。060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: 061 の 0節 #3・#4 の書き換え（A）・iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50・2027 年 2 月ごろ: 2028 年の祝日の表。開いている PR: Dependabot の fflate（#340・#382）
...
- 人間の手番（続き）: 061 段階3・062・063 のデプロイの承認（最新だけ）→ iPhone でタブバー・ピンクのホーム。060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: 061 の 0節 #3・#4 の書き換え（A）・iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50・2027 年 2 月ごろ: 2028 年の祝日の表。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションB。**063 追補: R 受け入れ（必須修正なし）→ CI 緑 → #440 を squash merge（main 678d8bb）。063 は追補まで完了。B は待機。**
- 判定は `artifacts/063/review-stage3.md` の末尾に追記
...
**最終更新（旧）**: 2026-09-25 / セッションB。**063 追補（テストの題名から経緯を消す。A の #439）: PR（`task/063-test-titles`）。R の手番。**
- 13 ファイル・25 行。差分は全部 `it`・`describe` の行。テスト 1,533 のまま全部緑。報告 `artifacts/063/titles.md`
- 人間の手番（続き）: 061 段階3・062 のデプロイ → Safari のタブを開き直してピンクのタイムラインを撮る・ピンクのホーム。060・059・058・057・056 の確認・047 の観点・Stripe の本番の Price・Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションA。**063 は 3 段階ともマージ済み（main 6d31f01）。B の問い 2 つに答えた。B の手番: テストの題名の経緯を消す小さな PR（063 の追補）。**
- 1. 祝日の同梱の表は延ばさない（内閣府の CSV は翌年分までしか無い）。内閣府が翌年分を公表したら（例年 2 月）作り直す。2028 年以降は holidays-jp 頼みで、取れなければ祝日が赤くならないだけ（058 の 0節 #9 に書いた）
- **audit の無視リストに image-size の 2 件を戻した。**#435 で外したのは npm レジストリの勧告の更新途中で一時的に消えていたため。今は修正版 2.0.3 付きで high として出る。Metro 0.87 が ^1.0.2 で `.default(content)` と呼ぶので 2.x には上げられない。9節の見出しを「修正版の無い・上げられない勧告」に
- 2. テストの題名の経緯（「R の必須修正1」等。約 10 件）は消す。063 の 0節 #4 に題名も入れた。題名は何を確かめるかだけに
- 人間の手番: 061 段階3・062・063 のデプロイの承認（最新だけ）→ iPhone でタブバー・ピンクのホーム。（続き）060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: 061 の 0節 #3・#4 の書き換え（A）・iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50・**2027 年 2 月ごろ: 2028 年の祝日の表を作り直す**。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションB。**063 段階3: R 受け入れ（必須修正なし）→ main を取り込み CI 緑 → #437 を squash merge（main 73244d6）。063 は 3 段階とも完了。B は待機。**
- 判定 `artifacts/063/review-stage3.md`（0節 #6 の削除 0 本・it.each の値を 1 つずつ突き合わせ）。CI の全件 1,533 緑
- 残る記録: 同梱の祝日表は 2026・2027 年の分だけ（本番は外部の JSON で上書きされる）。延ばすかは A
- 人間の手番（続き）: 061 段階3・062 のデプロイ → Safari のタブを開き直してピンクのタイムラインを撮る・ピンクのホーム。060・059・058・057・056 の確認・047 の観点・Stripe の本番の Price・Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-25 / セッションB。**063 段階3（テストとテストファイルのコメント）: PR（`task/063-stage3`）。R の手番。これで 063 の 3 段階が揃う。**
- テスト 1,539 → 1,533（消した 7 本・`it` 29 本 → `it.each` 7 つ）。消した 7 本は `artifacts/063/removed-tests.md`（0節 #6 は 0 本）。報告 `artifacts/063/stage3.md`
- 時限式を 1 つ直した: `weather.test.ts` の祝日が本物の今年で引いていて 2027-01-01 に赤くなる（同梱の表は 2026・2027 年）。写しの年で引く。`viewer-key-coverage.test.ts` の免除の固定を行番号 → 行の中身に（段階2 の記録 1）
- テストのコメント行 2,167 → 1,485。コメント以外が変わったのは上の 9 ファイルだけ（`compare-code.mjs`）
- 人間の手番（続き）: 061 段階3・062 のデプロイ → Safari のタブを開き直してピンクのタイムラインを撮る・ピンクのホーム。060・059・058・057・056 の確認・047 の観点・Stripe の本番の Price・Dependabot の fflate（#340・#382）
...
**最終更新（旧）**: 2026-09-25 / セッションB。**063 段階1（使われなくなったコード + apps/api/src・packages/db/src・scripts のコメント）: PR #432（`task/063-stage1`）。R の手番。その後 段階2（apps/app・packages/ui/src・apps/landing のコメント）・段階3（テスト）。**
...
**最終更新（旧）**: 2026-09-25 / セッションA。**063（不要・冗長なコメントと不要なテスト、使われなくなったコードを消す）を起票。B の手番。3 段階（1: デッドコード + api・db・scripts のコメント / 2: app・ui・landing のコメント / 3: テスト）。**
- 人間の指示。コメントは「理由・制約・セキュリティの意図」だけ残し、経緯・指摘者・言い換えを消す。テストは `couple_id`・認証・セキュリティ・課金・規則を機械で守らせるものは必ず残す。消したテストは 1 本ずつ `artifacts/063/removed-tests.md` に
- 061 の残骸（`+html.tsx` の SVG・`theme.ts` の `filterId`・`lens*`）は 063 の段階1 で消す（B の記録への回答）
- 人間の手番: 061 段階3・062 のデプロイの承認（最新だけ）→ iPhone でタブバー・ピンクのホームの写真タイル。（続き）060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: 061 の 0節 #3・#4 の書き換え（A）・iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-18 / セッションB。**061 段階3（板の `filter: url()` とピルの `backdrop-filter` を外す）: R 受け入れ → #427 を squash merge（main 158434a）。次は人間: デプロイの承認（最新だけ）→ **Safari のタブを閉じて開き直してから**ピンクのタイムライン（写真のある投稿）を撮る。FAB が全部見えるかも。B は待機。**
- 残る CSS の効果は `glass-blur` の `backdrop-filter: blur() saturate()` 1 層だけ。**これでも二重化・帯が残るなら Safari ではぼかし自体が成り立たない → A・人間の判断（Safari だけ不透明に戻すか、ガラスをやめるか）。**判定 `artifacts/061/review-stage1.md` の末尾、報告 `artifacts/061/stage3.md`
- 062 も人間の手番: 同じデプロイでピンクのホームの写真タイルを見る
- **A へ**: 061 の `+html.tsx` の SVG 定義と `theme.ts` の `filterId`・`lens*` は参照されなくなった（消すかは A。0節 #3・#4 の書き換えと一緒に）。（続き）5節の申し送り 4 件
- 人間の手番（続き）: 060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price。未着手: iOS 段階0（Opus で）・メール認証・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-18 / セッションB。**062: R 受け入れ → #426 を squash merge（main aa2fbfc）。061 段階3（ガラス板の `filter: url()` も外す）: PR #427（`task/061-stage3`）。R の手番。**
- 061: 段階2 のデプロイ（a4a942e）後も人間の iPhone Safari でアイコンと文字が二重にずれ・上端の帯が残った（「まだ変わらない」）。R が決めていた段階3 を #427 に。残る層はぼかし・板・色収差・フチ（`url(` 無し）。報告 `artifacts/061/stage3.md`。これでも残るなら段階4（ピルの `backdrop-filter`・色収差の影）
- 062: 判定 `artifacts/062/review-stage1.md`。人間の手番: デプロイの承認 → ピンクのホームの写真タイルを見る
- **A へ**: 061 の `+html.tsx` の SVG 定義と `theme.ts` の `filterId` は参照されなくなった（消すかは A。0節 #3・#4 の書き換えと一緒に）。（続き）5節の申し送り 4 件
- 人間の手番（続き）: 060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price。未着手: iOS 段階0（Opus で）・メール認証・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-18 / セッションB。**062（ピンクの機能パネルにも写真タイル。ホワイトと同じ形に）: 実装完了。PR #426（`task/062-pink-panel-photos`）。R の手番。**
- `feature-panel.tsx` を 1 つの形に（`appearance` の分岐・`PanelSurface`・「COMING SOON」・`opacity` を消した。`testID` は `feature-panel`）。T1〜T4 緑（app 630）。画面 `artifacts/062/`（両外観 375 幅）。報告 `artifacts/062/stage1.md`。`releases.ts` は触っていない（人間の指示）
- 気づき: カードの高さ 113 → 159（375 幅）。0節 #7 のとおり
- 人間の手番: 061 段階2 のデプロイ → iPhone で 2 画面の撮り直し（縞が残れば段階3）。（続き）060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）
- **A へ**（続き）: 061 の 0節 #3・#4 の書き換え（A が次にやる）・5節の申し送り 4 件

---

**最終更新（旧）**: 2026-09-18 / セッションA。**062（ピンクの機能パネルにも写真タイル。ホワイトと同じ形に）を起票。B の手番。**
- 人間の指示: ホワイトだけにある写真タイルをピンクにも。`releases.ts` には載せない（人間の指示）。部品を 1 つの形にして `appearance` の分岐を消す（違いは色と影だけ）。「COMING SOON」は消す（9 枚とも押せるので使えないパネルは無い）
- **A の次**: 061 の 0節 #3・#4（Safari は `url()` を捨てる／板の側を歪ませる）の前提が実機で崩れた（B の記録）→ 定義を現在の形に直す（#422 はマージ済み。次の手番）
- 人間の手番: 061 段階2 のデプロイの承認（最新だけ）→ iPhone でピンクのマイページ・ホワイトのタイムラインを撮り直す（縞が残るなら段階3: 板の `filter: url()` も外す）。（続き）060 の開発者ツールでコメントが無いこと・059 の 2 列・058 の天気の行・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）
---

**最終更新（旧）**: 2026-09-18 / セッションB。**061 段階2（Safari の壊れ 2 箇所）: R 受け入れ（必須修正なし）→ #422 を squash merge（main b713eaf）。次は人間: デプロイの承認（最新だけ）→ iPhone で同じ 2 画面（ピンクのマイページ・ホワイトの写真のあるタイムライン）を撮り直す。B は待機。**
- 直ったら 061 は閉じる。**縞が残るなら段階3**: ガラス板の `filter: url()` も外す（Safari はぼかしだけの曇りガラス）。判定は `artifacts/061/review-stage1.md` の末尾
- **A へ**: タスク定義 0節 #3・#4 の前提（Safari は url() を捨てる／板の側を歪ませる）が実機で崩れた。（続き）5節の申し送り 4 件。ピンクの彩度（`saturate: 1.8`）は人間の感想待ち
- 人間の手番（続き）: 060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price。未着手: iOS 段階0（Opus で）・メール認証・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-18 / セッションB。**061 段階2（iPhone Safari で壊れた 2 箇所: バーの上下の赤い帯・FAB の下半分が覆われる・ホワイトの上端の縞）: 修正 PR #422（`task/061-safari-fix`）。R の手番（差分と Chromium の撮り直し）。**
- 屈折の層 `glass-refraction`（`backdrop-filter: url()`）を消した（Safari が捨てずに処理して壊れた絵を出していた。人間の指示）。`tablist` に `zIndex: 1`（FAB がガラスの層の上に来る）。G5 を反転（`backdrop-filter` に `url(` を置かない）。app 626 緑。報告 `artifacts/061/stage2.md`・画面 `artifacts/061/stage2/`（Chromium。`fabOverhang` 12 のまま）
- マージ・デプロイ後: **人間が iPhone で同じ 2 画面（ピンクのマイページ・ホワイトのタイムライン）をもう一度撮る。**縞が残るなら板の `filter: url()` も外す（ぼかしだけの曇りガラス）
- **A へ**: タスク定義 0節 #3・#4「Safari は url() を捨てる／板の側を歪ませる」の前提が実機で崩れた。定義の書き換えは A。（続き）5節の申し送り 4 件。ピンクの彩度（`saturate: 1.8`）は人間の感想待ちで触っていない
- 人間の手番（続き）: 060・059・058・057・056 の確認・047 の確認観点・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-18 / セッションB。**061（下部タブバーを湾曲ガラスにする）: R の撮影済み → 証跡を `artifacts/061/stage1/` に置いた（#420 を R 受け入れ → squash merge。main 4321901）。次は人間: デプロイの承認（最新だけ）→ 実機で見る。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
...
- **リモートの `task/061-glass-tab-bar` が消せていない**（代行セッションの環境で git のブランチ削除が通らなかった）。人間か B が消す
- **A の手番（タスク定義5節の申し送り 4 件）**: (1) `architecture.md` 7節にガラスのトークン（`glass`）を書く (2) **タブがリンク（`<a href>`）でなくなった。直せるが直すかは A の判断**（`Link` は `useLinkToPathProps` → `appendBaseUrl` で `/app` を足すので `<Link href asChild>` で戻せる。代償は部品が実行時に expo-router を読むこと） (3) ホワイトもガラスにしたが色収差と影は 0 にした。039「装飾は無い」の解釈でよいか (4) Safari で本物の屈折が出ないことを仕様に書くか
...
**最終更新（旧）**: 2026-09-17 / **代行セッション（A・R・B のいずれでもない）**。**061（下部タブバーを湾曲ガラスにする）: 実装 → R レビュー（受け入れ・条件付き。必須修正 3）→ 必須 3 件に対応済み。PR #418（`task/061-glass-tab-bar`）。次は R の再確認（特にローカルでの撮影）。**
- **A・R・B が起動しなかったため、人間の直接の指示で独立したセッションが B の手番を代行した。**ハーネスの外にいるセッションで、コミットの `Session: B` は役の意味。識別は `Claude-Session` の行
- 判定は `artifacts/061/review-stage1.md`（R の全文＋代行者の対応）。報告は `artifacts/061/stage1.md`
- **必須 2（FAB の縦位置が 6px 下がる）は直した。**旧の `paddingTop: space.sm` と項目の stretch を `tablist` に戻した。項目のずれは計算すると 8px ではなく 4px（旧 y=36 → 32）で、原因と向きは R の指摘どおり
- **必須 1（証跡）: `artifacts/061/scripts/capture.mjs` を用意した。撮影は R**（クラウドの代行セッションからは実機もブラウザも開けない）。`capture.json` で見る数値は `stage1.md` の表
- **未確認: 見た目そのもの・Safari/Firefox の描画・iPhone の 60fps と発熱・LP の iframe（056）の中での重さ。**確認観点はタスク定義3節
- **A の手番（タスク定義5節の申し送り 4 件）**: (1) `architecture.md` 7節にガラスのトークン（`glass`）を書く (2) **タブがリンク（`<a href>`）でなくなった。直せるが直すかは A の判断**（`Link` は `useLinkToPathProps` → `appendBaseUrl` で `/app` を足すので `<Link href asChild>` で戻せる。代償は部品が実行時に expo-router を読むこと） (3) ホワイトもガラスにしたが色収差と影は 0 にした。039「装飾は無い」の解釈でよいか (4) Safari で本物の屈折が出ないことを仕様に書くか
- 人間の手番（続き）: 059 の 2 列・058 の予定の無い日・057・056 の確認・047 の確認観点・Stripe の本番 Price。未着手: iOS 段階0（Opus で）・メール認証・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**060（本番の LP からコメントを落とす）: R 受け入れ（必須修正なし）→ #416 を squash merge（main 423c530）。次は人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` の開発者ツールでコメントが無いのを見る。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
...
**最終更新（旧）**: 2026-09-17 / セッションA。**060（本番の LP からコメントを落とす。`build-public.mjs` で HTML・CSS のコメントを除く）を起票。B の手番。**
- 人間の指示: 開発者ツールで `nisoine.com` の HTML（21 個）・CSS（約 980 個）のコメントが読める状態 → ビルド時に落とす。ソースは今まで通り。秘密情報は含まれていない（A が確認）
- R の記録（059）: T4 の参照を「054 T3」から実体（`h1` 1 つ・`h3` は `h2` の後）に直した。800 幅でボタンが 2 行はそのままでよい（0節 #7 の通り。人間の指摘があれば直す）
- 人間の手番: 059 のデプロイの承認（最新だけ）→ `https://nisoine.com/` で 2 列・帯・カードの余白。（続き）058 の予定の無い日の天気の行・057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**059（LP の「さわってみる」を 2 列に）: R 受け入れ（必須修正なし）→ #413 を squash merge（main ed52ac6）。次は人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` で「さわってみる」の 2 列・帯・カードの余白を見る。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 判定 `artifacts/059/review-stage1.md`。R の記録: 800 幅でボタンが 2 行（判断は A・人間）・**A へ**: 059 の T4 の参照「054 T3」は 054 に無い（B が 059 T4 として書いた）
- 人間の手番（続き）: 058 の予定の無い日の天気の行・057・056 の確認・047 の確認観点・Stripe の本番 Price。未着手: iOS 段階0（Opus で）・メール認証・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**059（LP の「さわってみる」を 2 列に・帯の写真を外す・写真カードの余白・文言）: 実装完了。PR #413（`task/059-landing-demo`）。R の手番。**
- LP の HTML・CSS・`hands-cafe.jpg` の削除・`landing.test.ts`（054 T4 を 10 枚・056 T5 の固定値・059 T1〜T5）。api 773 緑。報告 `artifacts/059/stage1.md`・画面 `artifacts/059/`
- 気づき: 800 幅でボタンが 2 行に折れる（0節 #7 の範囲。B は変えていない）。リリース履歴には足さない（人間の指示）
- 人間の手番: 058 のデプロイの承認（最新だけ）→ 予定の無い日の天気の行。（続き）057・056 の確認・047 の確認観点・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションA。**059（LP の「さわってみる」を 2 列に・AI まとめの帯の写真を外す・写真カードの下に余白・文言「デモ画面です。」）を起票。B の手番。**
- 人間の指示（2026-09-17）4 つ: `hands-cafe.jpg` を消す / スマホを左・右に見出し + 触り方の案内 + ボタン（人間の選択。文言は 059 の 0節 #5）/ 「ゆいとれんのデモです。」→「デモ画面です。」/ 節 3 の写真の下に白の余白（節 6・7 も揃える）
- B の起票で 058 の 0節 #5 を「7 日以内の日なら（予定の有無によらず）」に直した（#409 の定義）。054 の表・056 の #4・#9・`docs/sample/README.md` も現在の定義に
- 人間の手番: 058 のデプロイの承認（最新だけ）→ マイページで地域を選んで月表示と予定の詳細を見る。（続き）057・056 の確認・047 の確認観点（猶予中のまま）・Stripe の本番 Price
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**058 の追補 2（予定の無い日にも天気の行）: R 受け入れ（必須修正 1 = テストの日付依存を直した）→ #409 を squash merge（main 4c47aeb）。次は人間: デプロイの承認（最新だけ）→ 7 日以内で予定の無い日を選んで「この日の予定はありません」の下の天気の行を見る。B は待機。**
- 判定は `artifacts/058/review-stage1.md` の末尾。**A へ**: 058 の 0節 #5「7 日以内の予定なら」→「7 日以内の日なら」（起票済み）
...
**最終更新（旧）**: 2026-09-17 / セッションB。**058 の追補 2（予定の無い日にも天気の行。人間の指示）: 実装完了。PR #409（`task/058-weather-without-events`）。R の手番。**
- `calendar.tsx` の条件から「予定あり」を外しただけ。T7 を反転 + 7 日の外は出ないテスト。app 595 緑。報告 `artifacts/058/stage3.md`・画面 `artifacts/058/stage3/`
- **A へ**: 058 の 0節 #5「7 日以内の予定なら」→「7 日以内の日なら」
- 人間の手番: デプロイの承認（最新だけ）→ 予定の無い日を選んで見る。（続き）057・056 の確認・047 の確認観点・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**058 の追補（絵の差し替え）: R 受け入れ → #408 を squash merge。次は人間: デプロイの承認（最新だけ）→ マイページで地域を選んで月表示と予定の詳細を見る。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 判定は `artifacts/058/review-stage1.md` の末尾。R の記録: 雲がホワイトの地で薄い（人間の判断）・濃紺の地で塗りつぶしの取り残しがわずか（アプリの地では目に付かない）
- 人間の手番（続き）: 057・056 の確認・047 の確認観点（猶予中のまま）・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**058 の追補: 天気の絵を人間の原本（#407）から切り出して差し替え。PR #408（`task/058-icons`）。R の手番。**
- `make-icons.py`（マスの地を縁からの塗りつぶしで透明に。絶対の色で判定）。画面 `artifacts/058/stage2/`。気づき: 雲がホワイトの地で薄い（人間の絵のまま）
- 人間の手番: 058 のデプロイの承認 → 地域を選んで見る。（続き）057・056 の確認・047 の確認観点・Stripe の本番 Price

---

**最終更新（旧）**: 2026-09-17 / セッションA。**058 完了（main a5afd2c）。人間が天気の絵 `docs/sample/weather/icons-source.png` を置いた（A のツリーで追跡に入れた）。B の手番: 仮の絵を本物に差し替える小さな PR（058 の 0節 #6・#7。上段真ん中は使わない）。**
- 人間の手番: 058 のデプロイの承認（最新だけ）→ 地域を選んで月表示。057・056 の確認・047 の確認観点・Stripe の本番 Price
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-17 / セッションB。**058（カレンダーに天気と祝日）: R 受け入れ（必須修正なし）→ #406 を squash merge。次は人間: デプロイの承認（最新だけ。0026 は deploy.yml が当てる）→ マイページで地域を選び、月表示と予定の詳細を見る。`docs/sample/weather/icons-source.png` を置く（来たら B が別の小さな PR で仮の絵を差し替え）。B は待機。**
...
**最終更新（旧）**: 2026-09-17 / セッションA。**058（カレンダーに天気と祝日）を起票。B の手番。ただし絵 `docs/sample/weather/icons-source.png` は人間がまだ置いていない（B は API と祝日から着手できる）。**
- 天気は気象庁の JSON（鍵なし）を Worker が 1 時間キャッシュ。地域は個人ごと（`couple_members.weather_area`。位置情報は取らない）。絵は 5 つ（上段真ん中は使わない）。同じ地域なら 1 行。祝日は同梱の表 + holidays-jp を 1 日 1 回
- 人間の手番: 絵を置く。057 のデプロイ後の確認（`/app/admin`）。047 の確認の残り。Stripe の本番 Price
- 人間の決定（2026-09-17）: 利用者のデータの分析（ポリシーの改訂）は**やらない**
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションA。**057 完了（main 819bf27・#402）。B の起票で `architecture.md` 4節（`admin_actions`）・5節（`admin.*`・`couple.get.isAdmin`）を直した。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 人間の手番: デプロイの承認（最新だけ）→ `/app/admin` で自分を探し free → paid → `admin_actions` に残るのを見る。056 の枠の中のデモ。047 の確認の残り。Stripe の本番 Price
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**057（運営の画面）: 人間が `ADMIN_EMAILS` を secret に入れた → #402 を squash merge。次は人間: デプロイの承認（最新だけ。マイグレーション 0025 は deploy.yml が当てる）→ マイページの「運営 ›」→ `/app/admin` で自分を探し free → paid（30 秒以内に戻す）→ 直近の操作に残るのを見る。B は待機。**
...
**最終更新（旧）**: 2026-09-16 / セッションB。**056（LP の中でデモ）: R 受け入れ（必須修正 1 は閉じた）→ #398 を squash merge。次は人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` の枠の中でデモを触る（完了条件）。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 判定 `artifacts/056/review-stage1.md`。報告 `artifacts/056/stage1.md`
- 人間の手番（続き）: 047 の確認観点の残り（今は猶予中のまま。鍵の後 → paid に戻すの 2 本は帰宅後に B が流す〈許可〉）・Stripe の本番 Price。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**056（PR #398）: A の決定（#400。0節 #3・#3b・#3c）で直した。親を飛ばすのは `exitGuestMode` だけ・框の中はサインイン画面を出さず「デモを読み込めませんでした」+「アプリを開く」・お知らせのシートは框の中で出さず `releaseSeen` も書かない。T4b・T7 緑。R の手番（2 回目）。**
- 報告 `artifacts/056/stage1.md`（更新）。`capture.json`: 框の中にシート無し・localStorage 無し・帯の「ログイン」で親が `/app/`・ログイン中でも框はデモ

---

**最終更新（旧）**: 2026-09-16 / セッションB。**056（PR #398）: R 差し戻し。必須修正 1 は 0節 #3 の形に由来するので A の判断待ち: 框の中でデモの読み込みが失敗（`demoFailed`）すると、利用者が何も押していないのに親ページ（LP）が `/app/` へ飛ぶ。R の案: 親を飛ばすのは `exitGuestMode`（利用者の操作）だけにし、`demoFailed` は框の中に「デモを読み込めませんでした」を出して止める。**
- 判定 `artifacts/056/review-stage1.md`。R の記録: 043 のお知らせのシートが框の中でも初回に出る（塞ぐかは A）
- A が決めたら B が直して R へ

---

**最終更新（旧）**: 2026-09-16 / セッションB。**056（LP の中でデモ）: 実装完了。PR #398（`task/056-demo-in-phone-frame`）。R の手番。人間の指示で「框の中に実ユーザーのデータが写る経路」を塞いだ（框の中は API・認証に Cookie を送らない = 常にデモ。二重に `isAuthenticated` も false）。**
...
**最終更新（旧）**: 2026-09-16 / セッションA。**056: R が #398 を差し戻し（必須修正 1）。A の判断: 親を飛ばすのは `exitGuestMode` の 1 箇所だけ（`showAuth` で飛ばさない）・框の中ではサインイン画面を出さず失敗は「デモを読み込めませんでした」+「アプリを開く」の 1 行・043 のシートは框の中で出さず `releaseSeen` も書かない。0節 #3・#3b・#3c・T4b・T7。B の手番。**
...
**最終更新（旧）**: 2026-09-16 / セッションA。**056（LP の中でデモを触れる。スマホの枠に `/app/?demo=1` を iframe で）を起票（#396）。B の手番。**
- 人間の了承: `frame-ancestors` を `'none'` → `'self'`。枠の絵は AI 生成（`docs/sample/landing/phone-frame.png`）
- 人間の手番: 047 の確認観点（自分のペアで運営の SQL で `expires_at` を過去にして猶予 → 鍵 → paid に戻す）・Stripe の本番 Price（¥420/¥4,200）
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**054・047（追補まで）を本番にデプロイ済み（人間）。B が本番を確認: LP・`/tech`・法務の文言・写真・CSP・`planState` すべて OK。本番の `demo-couple` に `couple_plans` が無かったので人間の許可で paid の行を入れた（045 の定義どおり）。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 人間の手番: 047 の確認観点（自分のペアで運営の SQL で `expires_at` を過去にして猶予 → 鍵 → paid に戻す）・Stripe の本番 Price（¥420/¥4,200）
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**047 の追補: R 受け入れ → #394 を squash merge。次は人間: デプロイの承認（最新だけ）→ 047 の確認観点（運営の SQL で `expires_at` を過去にして猶予 → 鍵 → paid に戻す）。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
- 判定は `artifacts/047/review-stage1.md` の末尾。R の記録 1・2 は閉じた
- 人間の手番（続き）: 054 のデプロイ確認・Stripe の本番 Price（¥420/¥4,200）。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**047 の追補（A の指示 #392）: 045 の部品の「無制限」→「写真 50 万枚まで」「プレミアムで 50 万枚まで」（定数から）・T12（削除済みアルバムの写真は 30 の枠を食わない）。PR #394（`task/047-followup`）。R の手番。**
- 受け入れ → squash merge → 人間: デプロイの承認（最新だけ）→ 047 の確認観点。そのあと B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションA。**047（鍵）完了（main 234557d・#391）。R の記録 3 つを処理（#392）: 047 の 0節 #14（045 の部品の「無制限」→「50 万枚まで」）・#15（`deleted_at` の条件は残しテスト T12）・`architecture.md` の `couple.get`/`photo.list`。B の手番: #14・#15 を 1 つの小さな PR（047 追補）。**
- 人間の手番: デプロイの承認（最新だけ）→ 047 の確認観点（運営の SQL で `expires_at` を過去にして猶予 → 鍵 → paid に戻す。人間のペアで 1 度）。Stripe の本番 Price（¥420/¥4,200）→ 鍵の差し替えは B の PR
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**047（やめたあとの鍵）: R 受け入れ（必須修正なし）→ #391 を squash merge。次は人間: デプロイの承認（最新だけ）→ 確認観点（運営の SQL で `expires_at` を過去にして猶予 → 鍵 → paid に戻す）。B は待機。**
...
**最終更新（旧）**: 2026-09-16 / セッションB。**054 の追補: R 受け入れ → #389 を squash merge。次は人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` と `/tech` を見る。A が 047 を起票。B は待機。**
- 判定は `artifacts/054/review-stage1.md` の末尾。048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**054 の追補（A の指示 #388）: LP「思い出」の文言を「過去の投稿がそっと届きます」に・`billing.test.ts` 425 行の時刻を 1 度取りに。PR #389（`task/054-followup`）。R の手番。**
- 受け入れ → squash merge → 人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` と `/tech` を見る → 047（A が起票。B は待機）
- 048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションA。**054 完了（main 0599f57）。B の申し送り 2 つを処理: (1) LP の「思い出」の文言を「過去の投稿がそっと届きます」に（`memory.*` は画像の無い投稿も出す）。(2) `billing.test.ts` の flake（`Date.now()` を 2 度呼ぶ）。どちらも B が 1 つの小さな PR で。そのあと A が 047 を起票。**
- 人間の手番: デプロイの承認（最新だけ）→ `https://nisoine.com/` と `/tech` を見る。Stripe の本番 Price（¥420/¥4,200）の準備
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**054（LP を一般向けに作り直す）: R 受け入れ（必須修正なし）→ #387 を squash merge。次は人間: デプロイの承認（最新だけ）→ `https://nisoine.com/` と `/tech` を見る。B は 047（A の起票待ち）まで待機。**
- 判定 `artifacts/054/review-stage1.md`。報告 `artifacts/054/stage1.md`。R の記録: 375px の見出し「置き / 場」の折れは任意（直していない）
- **A へ**: 3 節「思い出」の「過去の 1 枚が届きます」は `memory.*` が文だけの投稿も出すので「1 枚」と少し食い違う（直していない）
- 055 も main でマージ済み。048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま。開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**054（LP を一般向けに作り直す）: 実装完了。PR #387（`task/054-landing-for-users`）。R の手番。**
- `index.html` を 1 節の構成・3 節の文言で書き直し。技術構成は `/tech`（`tech.html`）へ。写真 11 枚（合計 522KB）。JS 無し。T1〜T6 緑。報告 `artifacts/054/stage1.md`・画面 `artifacts/054/*.png`
- **A へ**: 3 節「思い出」の「過去の 1 枚が届きます」は `memory.*` が文だけの投稿も出すので「1 枚」と少し食い違う（報告に書いた。直していない）
- 受け入れ → squash merge → 人間: デプロイの承認 → `https://nisoine.com/` と `/tech` を見る → 次は 047（A が起票）
- 055 は main 92910c7 でマージ済み（デプロイの承認は人間）。048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま。開いている PR: Dependabot の fflate（#340・#382）
---

**最終更新（旧）**: 2026-09-16 / セッションB。**055（プレミアムの写真 50 万枚）: R 受け入れ（必須修正なし）→ #386 を squash merge。次は人間: デプロイの承認（最新だけ）。B は 054（LP。`docs/tasks/054-landing-for-users.md`）に着手。**
- 判定 `artifacts/055/review-stage1.md`。報告 `artifacts/055/stage1.md`（T3 の実測 602KB）
- 054 のあと 047（A が起票）。048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま
- 開いている PR: Dependabot の fflate（#340・#382）

---

**最終更新（旧）**: 2026-09-16 / セッションB。**055（プレミアムの写真 50 万枚）: 実装完了。PR #386（`task/055-premium-photo-limit-500k`）。R の手番。**
- アルバム件数の上限 100 → 1,000 件・`PAID_ALBUM_PHOTO_LIMIT` 500_000・リリース履歴 3.2.0。報告 `artifacts/055/stage1.md`（T3 の実測 602KB）
- 規約 8 節に枚数は無かったので触っていない。`architecture.md` に直すべき「100 件」は無し
- 受け入れ → squash merge → 人間: デプロイの承認 → 次は 054（LP。`docs/tasks/054-landing-for-users.md`）→ 047（A が起票）
- 048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま。開いている PR: Dependabot の fflate（#340・#382）
---

**最終更新（旧）**: 2026-09-16 / セッションA。**人間の指示で LP を一般向けに作り直す。055（プレミアムの写真 50 万枚）→ 054（LP。技術構成は `/tech` へ）を起票。B の手番: 055 から。**
- 054: `docs/tasks/054-landing-for-users.md`。絵 `docs/sample/landing/mockup-full.jpg`・写真 10 枚（AI 生成。`docs/sample/README.md`）。**文言は 3 節に全部ある。絵にある「質問」「みんなの声」「30GB」「通報」は載せない**。価格は静的に ¥420/¥4,200（人間の決定）。Google Play は「準備中」の 1 行
- 055: `docs/tasks/055-premium-photo-limit-500k.md`。アルバム件数の物理上限 100 → 1,000 件（500 枚 × 1,000 = 50 万）。リリース履歴 3.2.0
- 048 段階2 の本番の鍵への差し替えは人間の Stripe 準備待ちのまま（本番 Price は ¥420/¥4,200 で作る）。047（鍵）は 054 の後に A が起票
- 未着手のもの: iOS 段階0（Opus で）・メール認証（マジックリンク）・042 の上限 50
- 開いている PR: Dependabot の fflate 0.8.3（#340・#382。B が通常の手順で）

---

**最終更新（旧）**: 2026-09-16 / セッションA。**048 段階2（決済）: 本番でサンドボックスの一周を人間が確認 OK（2026-09-16）。次は本番の鍵への差し替え（人間の Stripe 本番モードの準備が先）→ 047（鍵）。B は待機。**
...
**最終更新（旧）: 2026-09-15 / セッションB。**053（独自ドメイン nisoine.com）: R 受け入れ → 人間の許可で secret（`BETTER_AUTH_URL`・`TRUSTED_ORIGINS` = `https://nisoine.com`）と R2 CORS を本番に適用 → #373 を squash merge（main 0505471）。次は人間の手番: GitHub の Deploy（`production`）の承認。**

---

**最終更新（旧）: 2026-09-15 / セッションB。**人間の合図で 048 段階2（決済。3.1.0）を開始。A の手番: 定義の見直し・特商法の表記の雛形・規約 8 節。人間の手番（段階0）: Stripe アカウント・テストモードの商品と価格 2 つ（Price ID を B へ）・Billing Portal・Webhook 署名と API 鍵。B は揃ったら実装（テストモードで一周まで。特商法の文面が無い間は本番に出さない）。**
- 053: 本番は正常（301/200/HSTS/CSP/403 を B が確認）。人間の回線（ソフトバンク）の DNS が古く、1.1.1.1 の VPN で開けた（時間で解決）。人間の残り: OAuth の URI・同意画面の URL（052 分も）・OpenAI データ共有オフ・実機・Search Console・Always Use HTTPS・1 週間後に旧 URI 削除
- 047（鍵）は段階2の後。042 の上限 50 は後回し
...
**最終更新（旧）**: 2026-09-15 / セッションB。**052（プライバシーポリシーと利用規約のページ）: R 受け入れ（dd4db9c。`artifacts/052/review-stage1.md`）→ A の判断で節番号を詰め・表の CSS を足した 2 コミット目を積んだ。R に追加分だけ見てもらう手番。**受け入れ → B が squash merge → デプロイ（人間の承認）→ 人間: Google の OAuth 同意画面に `https://…/privacy` を登録・OpenAI のデータ共有をオフ
- PR #367（`task/052-legal-pages`）。報告は `artifacts/052/stage1.md`（「A の判断で直したこと」を追記。T1〜T3 取り直し済み）
- **A へ**: 表の最小幅 560px と `.decision` の `min-width: 0` は「包み 1 つだけ」より多い（包みだけでは列が狭いまま・カードが画面の外へ出るのを実測）。減らすなら A の判断
- 048 段階2（決済。3.1.0）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**052（プライバシーポリシーと利用規約のページ）: 実装完了。PR #367（`task/052-legal-pages`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間: Google の OAuth 同意画面に `https://…/privacy` を登録・OpenAI のデータ共有をオフ
- 報告は `artifacts/052/stage1.md`（T1〜T3・スクリーンショット・B が決めたこと）。lint・型・全テスト緑
- **A へ**: 規約の節番号は 8 を省いたので 7 → 9 に飛ぶ（文面を変えない約束。048 段階2 で埋まる）。節ごとに `.decision` のカードで区切った。表はスマホ幅で列が狭いが横スクロールは無い
- 048 段階2（決済。3.1.0）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションA。**052（プライバシーポリシーと利用規約のページ）を起票した。先に人間が草案の空欄を埋める。**
- 草案: `docs/legal/privacy-policy-draft.md`・`docs/legal/terms-draft.md`。人間が埋めるもの: 運営者名・連絡先・年齢・ログ保持日数・制定日・裁判所の地・AI 事業者名と学習の一文
- B は埋まってから `apps/landing/privacy.html`・`terms.html` を作り、フッター・マイページ・サインインにリンク。デプロイ後、人間が Google の OAuth 同意画面に URL を登録
- 051 完了（改名。人間の実機・OAuth の名前・OGP まで OK）。048 段階2（決済）・iOS・ドメインは合図待ち

---

**最終更新（旧）: 2026-09-15 / セッションB。**051（Nisoine への改名）: 完了。人間が OAuth 同意画面の名前を Nisoine に・X で OGP のカードが出るのを確認（A 経由）。次は 048 段階2（決済。3.1.0）の人間の合図待ち。**
- 本体 #357・fix #359。判定は `artifacts/051/review-stage1.md`・`review-fix-poppins.md`。報告は `artifacts/051/stage1.md`（人間の手番の記録を追記）
- 047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**051（Nisoine への改名）: 本体 #357（main 5263af7）+ poppins-300 の fix #359（main fd06660）。人間: Google OAuth の同意画面のアプリ名を Nisoine に変えた（済。A 経由）。残りの人間の手番: デプロイの承認 → iPhone のアイコン入れ直し → X に URL を貼って OGP（未確認）。**
...
**最終更新（旧）**: 2026-09-15 / セッションB。**051（Nisoine への改名）: R 受け入れ → #357 を squash merge（main 5263af7）。次は人間の手番: デプロイの承認 → Google OAuth の同意画面のアプリ名を Nisoine に → iPhone のホーム画面アイコンを入れ直す → X に URL を貼って OGP の絵。**
- 判定は `artifacts/051/review-stage1.md`（必須修正なし。T1・T6 を R が自分で測って一致）。報告は `artifacts/051/stage1.md`
- R の記録（A へ。急がない）: (1) コメント中の `futary`（style.css 1 行目・api の数ファイル）は 2節どおり残っている。(2) `apps/app/public/fonts/poppins-300.woff2`（約 8KB）は参照が無くなった。消すかは A の判断
- 048 段階2（決済。3.1.0）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**051（Nisoine への改名。ドメイン以外）: 実装完了。PR #357（`task/051-rename-nisoine`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間: Google OAuth の同意画面の名前・iPhone でアイコンの入れ直し・X に URL を貼って OGP
- 報告は `artifacts/051/stage1.md`（絵の作り方 `scripts/make-assets.py`・T1〜T6・スクリーンショット・B が決めたこと）。全パッケージのテスト・型・lint 緑。3.0.0
- **A へ**: ワードマークのアルファは 190〜240 で落とし RGB を芯の色に揃えた。ロゴの大きさはホーム 120×32・サインイン 240×64・ランディング 200×53。Poppins 300 の preload/@font-face は外した（ファイルは残す）。定義の `apps/app/assets/apple-touch-icon.png` は実際は `apps/app/public/`（PWA の icon-192/512・manifest の name も変えた）
- 048 段階2（決済。3.1.0）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションA。**051（プロダクト名を Nisoine に。ドメイン以外）を起票した。B の手番。**
- `docs/tasks/051-rename-nisoine.md`。利用者に見える名前と絵だけ。中の名前（`@futary/*`・鍵・バケット・URL）は変えない。キャッチコピー「日々が、ふたりの記録になる。」
- 絵: `docs/sample/nisoine/`（アイコン 1024・ワードマーク。ワードマークは市松が焼き込まれた JPG → B が透過に）。OGP は B が組む。リリース履歴 3.0.0（048 段階2 は 3.1.0）
- 人間の手番: Google OAuth の同意画面のアプリ名・実機でアイコンの入れ直し。ドメインは iOS が動くと分かってから
- 050 完了（人間の実機 OK）。048 段階2（決済）は合図待ち

---

**最終更新（旧）: 2026-09-15 / セッションB。**050（タイムラインの密度）: 完了。人間の実機 OK（2026-09-15。B へ直接「確認OK」）。本体 #349・fix #352。次は 048 段階2（決済）の人間の合図待ち。**
- 判定は `artifacts/050/review-stage1.md`・`review-fix-long-name.md`。報告は `artifacts/050/stage1.md`（人間の実機 OK を追記）
- 4 枚の横一列の初期スクロール（033 から）は個別の報告なし。出たら別の fix
- 048 段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**050（タイムラインの密度）: 本体 #349（main d930591）+ 長い名前の fix #352（main 1c12f4e）。次は人間の手番: デプロイの承認 → iPhone で「X くらい」か・縦長の写真が高さで揃って左に寄っているか・4 枚の横一列が読み込み直後に末尾までスクロールしていないか（033 から。出れば別の fix）。**
- 判定は `artifacts/050/review-stage1.md`・`review-fix-long-name.md`（どちらも必須修正なし）。報告は `artifacts/050/stage1.md`
- A の判断（main 22195e5）: 048 段階2 の版は 2.4.0・ハートは Button の compact（定義を合わせた）・長い名前は fix #352・4 枚の初期スクロールは人間の実機で見る
- 048 段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**050（タイムラインの密度）: R 受け入れ → #349 を squash merge（main d930591）。次は人間の手番: デプロイの承認 → iPhone で「X くらい」か・縦長の写真が高さで揃って左に寄っているか。**
- 判定は `artifacts/050/review-stage1.md`（必須修正なし。壊して確かめた 4 通り）。報告は `artifacts/050/stage1.md`
- **A へ**（R の記録 1・2・3・6）: (1) 048 の 4節の段階2 の版 2.3.0 が 050 とぶつかる → 段階2 は 2.4.0。(2) 050 の 0節 #5「Button をやめて hitSlop」は conventions 4節と衝突。実装は Button の `compact`（上下の余白 12 で 44・`marginVertical: -8` で 28）。定義を合わせる。(3) 名前が全角 13〜15 文字以上だと「名前 · 時刻」の時刻が切れる（計算。直すなら row に分けて名前に flexShrink）。(6) 4 枚の横一列が読み込み直後に末尾までスクロールしていることがある（main でも同じ）
- 048 段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**050（タイムラインの密度）: 実装完了。PR #349（`task/050-timeline-density`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の iPhone で「X くらい」か
- 報告は `artifacts/050/stage1.md`。実測（390pt）: 文字 1 行の投稿はピンク 94 / ホワイト 96（+ 間 8 で 102 / 104 ≤ 104）。縦長 1 枚は 240×360 左寄せ。ハートの当たり判定 44。全パッケージのテスト・型・lint 緑。2.3.0
- **A へ**: `hitSlop` は react-native-web の Pressable が DOM に反映しないので使わず、Button の `compact`（上下の余白 12 + `marginVertical: -8`）で 44/28 を作った。`memory-card` は `PostImages` 共用なので 1 枚の 360 上限が思い出にも掛かる。4 枚の横一列が読み込み直後に末尾までスクロールしていることがある（main でも同じ。050 では触っていない）
- 048 段階1・049 の人間の実機は OK（#348 で記録）。段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションA。**050（タイムラインの 1 投稿を X くらいの高さに）を起票した。B の手番。**
- `docs/tasks/050-timeline-density.md`。1 行の投稿を 390pt 幅で 104pt 以下（B が実測）。名前と時刻を 1 行・ハートを小さく・一覧の gap を 8。画像 1 枚は高さ 360 を上限に比率を保って縮め、左寄せ。2〜4 枚は変えない
- 048 段階1・049: 人間の実機 OK（ZIP と 100 枚。2026-09-15）。段階2（決済）は合図待ち

---

**最終更新（旧）: 2026-09-15 / セッションB。**049（アルバムに一度に 100 枚）: R 受け入れ → #345 を squash merge（main 02980c7）。次は人間の手番: デプロイの承認 → iPhone で 100 枚を 1 回で入れる → 048 の ZIP で 100 枚が落ちるか（iPhone・PC）。**
- 判定は `artifacts/049/review-stage1.md`（必須修正なし。壊して確かめた 6 通り）。報告は `artifacts/049/stage1.md`
- R の記録（A へ。判断は要らないと R は書いている）: (1) 塊の途中で「やめる」と PUT 済みの最大 19 枚が addPhotos 無しの実体として R2 に残る（architecture.md 6節「孤児」の既知の形）。(2) 1 つも入らないうちに閉じると「0 枚まで入りました」（定義どおり。任意）
- 停止条件: iPhone で 100 枚の圧縮が落ちる → `ALBUM_UPLOAD_BATCH_MAX` を 50 に（A の手番は要らない）。048 の ZIP が 100 枚で落ちる → `ZIP_PART_SIZE` を 50 に
- 048 段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-15 / セッションB。**049（アルバムに一度に 100 枚）: 実装完了。PR #345（`task/049-album-upload-batch`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の iPhone で 100 枚を 1 回で入れる → 048 の ZIP で 100 枚が落ちるか
- 報告は `artifacts/049/stage1.md`（T1〜T5・本物のバンドルで確認 → 20 枚ずつ → やめる → 文言の経路・B が決めたこと）。契約 `album.addPhotos`（1〜20）は変えず、画面が 20 枚ずつ順に呼ぶ。app 527・型・lint 緑
- **A へ**: 「途中で閉じられる」は進捗の下の「やめる」（20 枚超のときだけ）。全部の塊が失敗なら 041 の「送れませんでした」のまま。101 枚以上は送らずに止める。塊の PUT が済んでから閉じられてもその塊の addPhotos は呼ぶ
- 048 段階1 は済（#338・#342）。人間の実機（iPhone・PC の ZIP。100 枚）は 049 のデプロイ後にまとめて。段階2（決済）は合図待ち。047（鍵）は段階2の後。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-14 / セッションA。**049（アルバムに一度に入れられる枚数を 20 → 100）を起票した。B の手番。**ZIP の 100 枚を人間が確かめるため。
- `docs/tasks/049-album-upload-batch.md`。契約は変えない（`addPhotos` 1〜20 のまま）。画面が 20 枚ずつ割って順に呼ぶ。塊ごとに入る
- 048 段階1（ZIP）は揃った（main 8e271ac）。人間の実機はデプロイ後。段階2（決済）は合図待ち。047 はその後

---

**最終更新（旧）: 2026-09-14 / セッションB。**048 段階1（ZIP で持ち出し）: 本体 #338（main 6e306b1）+ R の記録 1・2 の fix #342（main fb7f372）。次は人間の手番: デプロイの承認 → iPhone・PC で 1 アルバムと全部が落ちるのを見る（100 枚の ZIP が iPhone で落ちるか。落ちるなら `ZIP_PART_SIZE` を 50 に）。**
- 判定は `artifacts/048/review-stage1.md`・`review-fix-safe-name.md`（どちらも必須修正なし）。報告は `artifacts/048/stage1.md`
- fix #342: `safeZipName` は `.` だけの名前を `album` に倒し制御文字・TAB・改行も `_`。説明文の TAB も空白。Z1 の無圧縮の検査は縮む中身で「ZIP ≥ 中身の合計」。Z1b 3 本。app 512 緑
- 段階2（決済）は人間の合図まで着手しない。次は（合図）→ 048 段階2 → 047（鍵）。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-14 / セッションB。**048 段階1（ZIP で持ち出し）: R 受け入れ → #338 を squash merge（main 6e306b1）。次は人間の手番: デプロイの承認 → iPhone・PC で 1 アルバムと全部が落ちるのを見る（100 枚の ZIP が iPhone で落ちるか。落ちるなら `ZIP_PART_SIZE` を 50 に）。**
- 判定は `artifacts/048/review-stage1.md`（必須修正なし。壊して確かめた 6 通り）。報告は `artifacts/048/stage1.md`
- **A へ**（R の記録 1・2）: (1) Z1 の「無圧縮」のテストは `level: 0` を外しても緑（中身が縮まない。縮む中身で「ZIP ≥ 中身の合計」にすれば赤になる）。(2) `safeZipName` は `..`・`.` を通す（題が `..` だと全部の ZIP で `../` のパス。`/^\.+$/` → `"album"` の 1 行）。題の中の TAB も `captions.txt` の区切りと衝突。A が定義に足すなら B が直す
- A は B の判断 5 点（アルバムごとのフォルダ・captions の形・タイムラインに ⋯ 無し・順に落とす・約 1MB）を受け入れ済み。定義 2節への反映は A
- 段階2（決済）は人間の合図まで着手しない。次は（合図）→ 048 段階2 → 047（鍵）。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-14 / セッションB。**048 段階1（ZIP で持ち出し）: 実装完了。PR #338（`task/048-album-zip-export`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の iPhone・PC で 1 アルバムと全部（100 枚が落ちるかも見る）
- 報告は `artifacts/048/stage1.md`（Z1〜Z6・fflate が expo web のバンドルで動いた記録・落ちた ZIP を開いて突き合わせた記録・スクリーンショット・B が決めたこと）。app 509・型・lint 緑
- **A へ**: 全部のときは ZIP の中をアルバムごとのフォルダに分けた（タスク定義はフォルダの有無を書いていない）。`captions.txt` は「パス TAB 説明文」の 1 行 1 枚。`⋯` はタイムラインの詳細には出さず、ゲストには出す
- 停止条件（fflate がバンドルで動かない）には当たらなかった。「50 枚でも iPhone で落ちる」は人間の iPhone で
- 段階2（決済）は人間の合図まで着手しない。次は（合図）→ 048 段階2 → 047（鍵）。042 の上限 50 は後回し

---

**最終更新（旧）**: 2026-09-14 / セッションA。**046 を 048 に畳んだ。048 = 段階1: ZIP で持ち出し（旧 046）、段階2: Stripe の決済。B は 048 段階1 から。段階2（決済）は人間が「やる」と言うまで着手しない。**
- タスク: `docs/tasks/048-premium-payment.md`。価格 ¥420/月・¥4,200/年・「写真 5 万枚まで」（「無制限」と書かない）。無料トライアル無し
- **段階0（人間の手番。段階2の前）**: Stripe のアカウントとテストモードの価格 2 つ・**特商法の表記／利用規約／プライバシーポリシーの文面**（A が雛形を別 PR で書く）・Webhook の秘密・Billing Portal の有効化
- 順: 048 段階1 → （人間の合図）→ 048 段階2 → 047（鍵）。042 の上限 50 は後回し
- 別件で残っているもの: 名前（`futari` の商標を人間が引く）・独自ドメイン（人間が決める）・ポートフォリオ（別リポジトリ）

---

**最終更新（旧）: 2026-09-14 / セッションB。**045: 完了（本体 #325・記録 1 のテスト #326・使用量のカードを一番下へ #328・警告カードの × #331。main 1a84deb）。次は人間の手番: デプロイの承認 → 実機で free が止まるのを 1 度見る → 人間の許可を取って人間のペアを paid にする。そのあと B は 046（ZIP で持ち出し）へ（`/clear` してから）。**
- 判定は `artifacts/045/review-stage1.md`。報告は `artifacts/045/stage1.md`。切り替えの SQL は `artifacts/045/plan-switch.md`
- 人間の手番は済んだ（実機確認・paid 化）。**人間に頼むこと**: マイページに「プラン: プレミアム」が出て、アルバムに 31 枚目が入ることを 1 度見る（見なくてもよい）
- 人間の指示 2 件（B へ直接）: 使用量のカードは一覧の一番下・警告のカードは × で消せる（この起動の間。残りが変わればまた出す）。A が 3節に足した。**警告の × は人間の実機確認 OK（2026-09-14。A 経由）。人間の指示「paid にして」で B が本番の人間のペアを paid にした（2026-09-14。`couple_plans` に paid・期限なし。読み返して確認）。以後 paid のまま。**R の記録（置く）: 消した状態の復元が useEffect なので枠が読めた直後の 1 フレームだけ警告が見えてから消える。人間が気にしたら useLayoutEffect に
- 次は 046 → 047（鍵）→ 048（決済）。042 の上限 50 は後回し（人間の指示）

---

**最終更新（旧）**: 2026-09-14 / セッションB。**045: 完了（本体 #325・記録 1 のテスト #326・使用量のカードを一覧の一番下へ #328。main 6f8282e）。次は人間の手番: デプロイの承認 → 実機で free が止まるのを 1 度見る → 人間の許可を取って人間のペアを paid にする。そのあと B は 046（ZIP で持ち出し）へ。**
- 判定は `artifacts/045/review-stage1.md`。報告は `artifacts/045/stage1.md`。切り替えの SQL は `artifacts/045/plan-switch.md`
- **人間に頼むこと**: (1) デプロイを承認する（main の CI は緑）。(2) 実機で無料枠（30 枚）で止まる・一覧の一番下の使用量のカード・マイページの「プラン: 無料」を 1 度だけ見る。(3) **人間のペアを paid にする許可**（本番の D1 に書く。B が実行するか人間が実行）。以後 paid のまま
- 人間の指示（B へ直接）で使用量のカードは一覧の一番下（アルバムのカードの下）。A が 3節を直した（2c5db5c）。R の記録: PC 幅の一覧の絵はカードが最初の画面の外に出て写っていない（端末幅の絵で示している）
- 次は 046 → 047（鍵）→ 048（決済）。042 の上限 50 は後回し（人間の指示）

---

**最終更新（旧）**: 2026-09-14 / セッションB。**045（プランの印とアルバムの無料枠）: R 受け入れ → #325 を squash merge（main d75f9ad）。R の記録 1 のテストも #326 でマージ（main 5cfac0c）。次は人間の手番: デプロイの承認 → 実機で free が止まるのを 1 度見る → 人間の許可を取って人間のペアを paid にする。**
- 判定は `artifacts/045/review-stage1.md`（必須修正なし。壊して確かめた 7 通り。#326 で記録 1 を閉じた）。報告は `artifacts/045/stage1.md`
- **人間に頼むこと**: (1) デプロイを承認する。(2) アルバムに写真を足して無料枠（30 枚）で止まる・一覧の使用量のカード・マイページの「プラン: 無料」を 1 度だけ見る。(3) **人間のペアを paid にする許可**（`artifacts/045/plan-switch.md` の SQL。本番の D1 に書く。B が実行するか人間が実行）。以後 paid のまま
- 次は 046（ZIP で持ち出し）→ 047（鍵）→ 048（決済）。042 の上限 50 は後回し（人間の指示）

---

**最終更新（旧）**: 2026-09-14 / セッションB。**045（プランの印とアルバムの無料枠）: 実装完了。PR #325（`task/045-plan-quota-impl`）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の実機で「free で止まる（1 度だけ）」→ **人間の許可を取って人間のペアを paid にする**（`artifacts/045/plan-switch.md` の SQL。本番の D1）
- 報告は `artifacts/045/stage1.md`（T1〜T9・停止条件の確認・切り替え SQL のローカル記録・スクリーンショット 3 状態 × 両モード × 2 幅・B が決めたこと）。api 625・app 473・db 32・型・lint 緑
- A の 3節の書き直し（7acd737。絵 `docs/sample/simpleMode/プレミアム/`）に合わせた。人間が渡したホワイトの見本 5 枚は `docs/sample/simpleMode/プレミアム/white/` に格納
- **A へ**: 警告のカードは FAB の上に固定（閾値は残り 5 枚以下、0 を含む）。`/premium` の絵は既存の線画（ピンク: ハート、ホワイト: ✦）を primary で塗った。シートのピルは部品内の小さな Pill。枠は書けるときだけ couple.get で読む（ゲストは読まない）
- 停止条件はどれにも当たらなかった（既存の画面テストは赤 0。me.delete の FK は当たらず。索引は不要）
- 次は 046（ZIP で持ち出し）→ 047（鍵）。042 の上限 50 は後回し（人間の指示）

---

**最終更新（旧）**: 2026-09-14 / セッションA。**045 を人間の絵（`docs/sample/simpleMode/プレミアム/`）に合わせて書き直し、046（ZIP で持ち出し）・047（やめたあとの鍵）を起票した。B は 045 から。**
- 045: 使用量のカード（一覧）・残りの警告・上限のシート・`/premium`（申し込みボタンは 048 まで置かない）・マイページ。**無料トライアルは作らない**（人間の指示）。「プレミアム」と呼ぶ
- 046: アルバム 1 つ／全部を ZIP で持ち出す（ブラウザで無圧縮。100 枚ずつ。`captions.txt` 同梱）。047 の前に本番に出す
- 047: プレミアムをやめて 30 日の猶予のあと、古い順 30 枚を残して新しい分に鍵（URL を返さない。消さない。Cron 無し）。もう一度プレミアムで戻る
- 048（決済。Stripe。¥480/月・¥4,800/年）は未起票。人間が「入れる」と言ってから
- 順: 045 → 046 → 047 → 048。042 の上限 50 は後回し。044 の定義 1節の直しは #322 の後に

---

**最終更新（旧）: 2026-09-14 / セッションA。**045（プランの印とアルバムの無料枠）を起票した。044 の記録 PR（#322）のあと B が着手。**
- タスク: `docs/tasks/045-plan-and-album-quota.md`。`couple_plans`（行が無ければ free）・無料枠は作ったアルバムの写真の合計 30 枚・超えたら `PLAN_LIMIT`・`couple.get` に `plan`/`albumQuota`・マイページに「プラン: 無料／有料」
- 決済は次のタスク（`couple_plans` に行を書くだけになる形）。**人間のペアはデプロイ後すぐ paid にする**（1節の SQL。人間の許可）
- リリース履歴には入れない。042 の上限 50 は後回し。044 の定義 1節（`substituteNames` は `lib/ai.ts`）の直しは #322 の後に A が 1 本で
- 人間の手番（044）: デプロイ後にまとめを 1 回作り直す

---

**最終更新（旧）: 2026-09-14 / セッションB。**044（AIまとめに名前を出す）: R 受け入れ → #321 を squash merge（main b580774）。次は人間の手番: デプロイの承認 → 実機で「1 回作り直すと名前で呼ばれる」。**
- 判定は `artifacts/044/review-stage1.md`（必須修正なし。壊して確かめた 5 通り。9b1d097 で確定）。報告は `artifacts/044/stage1.md`
- **人間に頼むこと**: (1) デプロイを承認する。(2) AIまとめを 1 回作り直す（回数の歯止め: その期間 3 回・月 10 回）→ まとめの中でふたりが名前で呼ばれる。(3) 古いまとめ（作り直していない期間）は今までどおり A/B のまま
- A へ: 定義 1節の `substituteNames` の置き場（`lib/ai.ts`）は A が直す（A の返事）。`MAX_OUTPUT_TOKENS` のコメントは実測に揃えた（本文 250 トークン前後。luna の reasoning 58〜86）
- 042 の上限 50 は後回し（人間の指示）。段階0（5 枚・20 枚）の結果はまだ聞いていない

---

**最終更新（旧）**: 2026-09-14 / セッションB。**044（AIまとめに名前を出す）: 実装完了。PR #321（`task/044-ai-names-impl`。A の起票ブランチ `task/044-ai-names` が残っていたので別名）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の実機で「1 回作り直すと名前で呼ばれる」
- 報告は `artifacts/044/stage1.md`（T1〜T5・本物の API に 3 回・応答の例・B が決めたこと）。api 604・app 451・型・lint 緑
- 停止条件は 2 つとも越えた: luna は 200（`artifacts/044/trial-luna.log`）。3 回とも `{{A}}` `{{B}}` に従い、素の A/B は 0
- **A へ**: `substituteNames` は `lib/ai.ts` に置いた（印の約束を決める `SYSTEM_PROMPT` と同じファイル。手続きは呼ぶだけ）。`user` の行が無いときの A も「相手」。`get` でもメンバーを 1 回読む（行があるときだけ）
- luna は reasoning tokens が 58〜79 出る（terra は 0 だった）。1024 の上限は変えていない
- 042 の上限 50 は後回し（人間の指示）。段階0（5 枚・20 枚）の結果はまだ聞いていない

---

**最終更新（旧）**: 2026-09-14 / セッションA。**044（AIまとめに名前を出す。LLM には渡さない。モデルを gpt-5.6-luna に）を起票した。B の手番。**
- タスク: `docs/tasks/044-ai-summary-names.md`。出力の印を `{{A}}` `{{B}}` に固定し、`get`/`generate` の応答でサーバが表示名に置き換える。保存は記号のまま。**リリース履歴には載せない**（人間の指示）
- 042 の上限 50 は後回し。042 段階0（5 枚・20 枚）の結果は人間から未聴取

---

**最終更新（旧）: 2026-09-14 / セッションB。**043（リリース履歴）: 完了。人間がデプロイ後の実機で確認し「問題なし」。A に完了を送り、A は定義の文言 2 点を実装に合わせた（70786a8）。B は次の指示まで待機。**
- main 70786a8（#315 実装・#316 記録・A の定義訂正）。判定は `artifacts/043/review-stage1.md`、報告は `artifacts/043/stage1.md`
- 042 の上限 50 は後回し（人間の指示）。段階0（5 枚・20 枚）の結果はまだ聞いていない

---

**最終更新（旧）**: 2026-09-14 / セッションB。**043（リリース履歴）: R 受け入れ → #315 を squash merge（main 749b132）。次は人間の手番: デプロイの承認 → 実機で「シート → 閉じる → 出ない」。**
- 判定は `artifacts/043/review-stage1.md`（必須修正なし。壊して確かめた 6 通り）。報告は `artifacts/043/stage1.md`
- **人間に頼むこと**: (1) デプロイを承認する。(2) ホームを開くと「新機能のお知らせ」が出る → 「閉じる ×」→ 開き直しても出ない。(3) ホームのボタンの NEW が一覧を開くと消える。(4) R の記録 1: 「後で通知する」→ 同じタブの再読み込みでは出ない → タブを閉じて開き直すと出る（`sessionStorage` の寿命。2節どおり。期待と違えば A へ）
- A へ: 1節の「小さな変更は本体の項目に畳む」の一文は A が足す（A の返事。B は触らない）。R の記録 1（「後で」の寿命）も A の判断
- 042 の上限 50 は後回し（人間の指示）。段階0（5 枚・20 枚）の結果はまだ聞いていない

---

**最終更新（旧）**: 2026-09-14 / セッションB。**043（リリース履歴）: 実装完了。PR #315（`task/043-release-notes-impl`。A の起票ブランチ `task/043-release-notes` が残っていたので別名）。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の実機で「シート → 閉じる → 出ない」
- 報告は `artifacts/043/stage1.md`（日付の確認結果・テスト 36 件・証跡・B が決めた数字）。app 451・api 591・型・lint 緑
- 日付は A の表のまま。**A へ**: 1.3.0 の「メモ」（028）は 09-02、1.2.0 の「付き合った日」（023）は 08-31 に別 PR で入っている。項目を分けるかは A の判断（B は日付を変えていない）
- 人間が渡したピンクの見本 4 枚は `docs/sample/simpleMode/リリース履歴/pink/`。ピンクのシートの絵は既存の `sparkle`、ホワイトは見本の贈り物を切り出した（部品の中で分岐）
- 042 の上限 50 は後回し（人間の指示）。段階0（5 枚・20 枚）の結果はまだ聞いていない

---

**最終更新（旧）**: 2026-09-14 / セッションA。**043（リリース履歴）を起票した。B の手番。**042 の上限 50 は後回し（人間の指示）。
- タスク: `docs/tasks/043-release-notes.md`。データはコードの配列（`apps/app/lib/releases.ts`）、「見た」は端末の `localStorage`。表も手続きも無い
- ホームの下に「リリース履歴を見る」（未読なら NEW）・`/releases` の一覧・ホームを開いたときの「新機能のお知らせ」シート（最新 1 項目だけ）
- 文言は A が 1節に書いた（11 項目。git の履歴から利用者に見える変更だけを拾った。リストとほしいものは別項目）。**日付は B が git で確かめる。細かい数字は B が決める**（人間の指示）
- 絵: `docs/sample/simpleMode/リリース履歴/`（4 枚。ホワイト）
- `conventions.md` 8節に「機能を足したら `releases.ts` に 1 項目」を足した

---

**最終更新（旧）: 2026-09-14 / セッションB。**fix #308（選択バーのラベルを「保存」「カバー」「削除」に）を R 受け入れ後に squash merge（main 76a751e）。次は人間の手番: デプロイの承認 → iPhone で折れが直ったことと、段階0（5 枚・20 枚）の結果。**
- R の判定は `artifacts/042/review-fix-bar-wrap.md`。A は 041・042 の 3節の文言を直した（#309）
- #308 は 08:14 に自動で close されていた: A の #309 の本文の「fix #308」を GitHub が閉じるキーワードとして読んだ（A が events で確認）。B が reopen して CI 緑を確かめてからマージした。規約は A が直した（#311。他の PR は `#NNN` とだけ書く）
- **人間に頼むこと**: (1) デプロイを承認する（Deploy は最新の main で 1 つ承認すれば足りる）。(2) iPhone で選択モードのバーが 1 行に収まるか。(3) 5 枚 →「保存」→ 共有シートの「画像を保存」で写真に入るか。(4) 20 枚で同じ（共有シートが出る／出ずに止まるの 2 択）。結果は B が `artifacts/042/spike.md` に書く
- 20 枚が通れば次の PR で上限を 50 に。止まるなら 2 タップの形（042 2節）。落ちるなら 10

---

**最終更新（旧）**: 2026-09-14 / セッションB。**042 はデプロイ済み（人間の iPhone で選択モードに「保存（7 枚）」が出た）。「保存（7 枚）」「カバーにする」の文字が折れたので fix PR #308（ラベルを「保存」「カバー」「削除」に。人間の決定）。段階0 の 5 枚・20 枚の結果はまだ聞いていない。**
- 人間の言（08:00）: 「保存ボタンの文字の改行が微妙なのを直せば、問題ないです」。2 段は「表示領域が減る」ので採らず、1 段でラベルを短くすると決めた。枚数はヘッダーの「N 枚を選択中」
- fix/album-selection-bar-wrap（#308）: 折り返しは jsdom で測れないので回帰テストは無い（PR 本文の 4 点）。R のレビュー → マージ → デプロイ承認（人間）。**A へ**: 042 の 3節「保存（N 枚）」を「保存」に直す（B が送った）
- 5 枚・20 枚で共有シートが出て写真に入ったかを人間に聞いて `artifacts/042/spike.md` に書く。20 枚が通れば次の PR で上限を 50 に。止まるなら 2 タップの形（042 2節）。落ちるなら 10

---

**最終更新（旧）**: 2026-09-14 / セッションB。**042（選んだ写真をまとめて写真ライブラリへ）: R 受け入れ確定 → #305 を squash merge（main cfb4eba）。次は人間の手番: デプロイの承認 → iPhone で段階0（5 枚・20 枚）。**
- 判定は `artifacts/042/review-stage1.md`（29d2e8b を受け入れ、A の決定 #306 を積んだ 3feee96 も確認して確定）。実装の報告は `artifacts/042/stage1.md`
- 上限は「保存」に掛け、選択には掛けない（A の決定 #306）。`sharePhotos` にも 20 枚超なら投げる保険（R の記録 1）。app 415・api 591・型・lint 緑
- **人間に頼むこと**: (1) デプロイを承認する。(2) 本番の iPhone で「選択」→ 5 枚 →「保存（5 枚）」→ 共有シートの「画像を保存」で写真に入るか。(3) 20 枚で同じ。共有シートが出る／出ずに止まるの 2 択。手順と記録欄は `artifacts/042/spike.md`（B が結果を書く）
- 20 枚が通れば次の PR で上限を 50 に（数だけ。A の手番は要らない）。止まるなら 2 タップの形（042 2節）。落ちるなら 10

---

**最終更新（旧）**: 2026-09-14 / セッションB。**042（選んだ写真をまとめて写真ライブラリへ）: 実装完了。PR #305 を出した。R の手番。**受け入れ → squash merge → デプロイ（人間の承認）→ 人間の iPhone で段階0（5 枚・20 枚。`artifacts/042/spike.md`）
- 選択モードに「保存（N 枚）」（`canShare` が真のときだけ。PC には出ない）。タイムライン・ゲストのアルバムにも「選択」を出し、バーは「保存」だけ。21 枚以上選ぶと「保存」が押せず 1 行（A の決定 #306: 上限は「保存」に掛け、選択には掛けない。#305 に積んだ）。取得に失敗した枚は飛ばして残りで共有シート
- 報告は `artifacts/042/stage1.md`。テスト 18 件追加（`share-photos.test.ts`・`album-detail-share.test.tsx`）。app 414・api 591・型・lint 緑。依存は増えていない
- R には SendMessage でレビューを頼み、A の決定を #305 に積んだことも送った。受け入れ → B が squash merge → デプロイ承認は人間
- 人間に頼むこと（デプロイ後）: 5 枚・20 枚を選んで「保存」→ 共有シートの「画像を保存」で写真に入るか。20 枚で止まるなら 2 タップの形に変える（2節）。結果は B が `spike.md` に書く

---

**最終更新（旧）**: 2026-09-14 / セッションB。**041 段階2: #300 をマージ（main 82dd476）→ デプロイ完走 → 人間の iPhone の段階0 が通った（共有シートが出て「画像を保存」で写真ライブラリに入った。`await fetch` のあとの `share` は拒まれない）。段階2 は完了。**段階3（投稿写真をアルバムに複製して入れる）は人間が「要らない」と決めた（2026-09-14）。041 の 9節を「やらない」に。**
**→ A の合図（2026-09-14）: B は 042（`docs/tasks/042-album-share-save.md`）に着手する。**20 枚の上限で実装 → デプロイ → 人間の iPhone で 5 枚・20 枚（042 の 2節）。段階0 の結果は `artifacts/042/spike.md`。50 は次の PR
- 受け入れ後に 1 件積んだ: CI（Node 22 + jsdom）で `File` が別 realm の `Blob` を文字列として包み `size` が 13 になった → `arrayBuffer()` から作る形に。R が 7a1b684 で受け入れ維持（`review-stage2.md`）

---

**最終更新（旧）**: 2026-09-14 / セッションB。**041 段階2: R 受け入れ（head 2eeff40。必須修正なし。判定は `artifacts/041/review-stage2.md`）→ #300 をマージ。次は人間の手番（iPhone の段階0）。**
- デプロイ後、ビューアで「保存」を押す: **共有シートが出て「画像を保存」で写真に入れば通った**。段階1と同じ「ダウンロード」の画面なら `share` が拒まれた（`NotAllowedError`）ので、表示中の 1 枚を先読みして押下で `share` だけ呼ぶ形に変える（8節）
- R の記録（判定に使わない）: `canShareFiles()` は押すたびに判定する（1 回覚えてもよい。実害なし）・失敗して `<a download>` に倒した先は activation を要らないので動くはず（iPhone で観察に含めてよい）
- 042（まとめて写真ライブラリへ）は段階2の後。段階3の要否は A が人間に聞く

---

**最終更新（旧）**: 2026-09-14 / セッションB。**041 段階2（iPhone は共有シートで写真ライブラリへ）: 実装完了。PR を出した。R の手番。**
- `downloadPhoto`: `navigator.canShare({ files })` が真なら `fetch` → `File` → `navigator.share`、偽（PC）は `<a download>` のまま。`AbortError` は何もしない。他の失敗は `<a download>` に倒す。`photo.downloadUrl` は変えていない
- `r2-cors.json` を実体に合わせた（19006 を消し workers.dev を足した。apply 不要）
- 報告は `artifacts/041/stage2.md`。テスト: `photo-download.test.ts`（app 396 緑）・型・lint 緑
- **段階0は人間の iPhone**（デプロイ後）: 保存を押して共有シートが出れば通った。段階1と同じ「ダウンロード」の画面なら `share` が拒まれた（`NotAllowedError`）ので先読みの形に変える
- 042（まとめて写真ライブラリへ）は段階2の後。段階3の要否は A が人間に聞く

---

**最終更新（旧）**: 2026-09-14 / セッションA。**041 段階1 は本番で動いた（人間の iPhone 実機: 概ね問題なし）。人間の要望 2 点を受けて、041 段階2（iPhone は共有シートで写真ライブラリへ）と 042（選んだ写真をまとめて写真ライブラリへ）を定義した。ZIP は落とした。B の手番。**
...
**最終更新（旧）**: 2026-09-14 / セッションB。**041 段階1: R 受け入れ確定（head 96734bc。判定は `artifacts/041/review-stage1.md`）→ #294 をマージ。A・B・R に手番は無い。**
**次は人間の手番**: (1) **リモート D1 に `0022_albums.sql` を当てる許可**（`pnpm --filter @futary/api run db:migrate:remote`。当ててからデプロイ。architecture.md 6節「マージ → リモートへ適用 → 人間の実機確認」）、
(2) 本番で 1 枚保存できる（PC のブラウザ・iPhone の Safari）・5 枚まとめてアップロードできる・画像が見える。段階2の要否は実機のあとに A が人間に聞く。042 は 041 のマージ後（本番 R2 の CORS を人間に確かめてもらってから）。
経緯: R の必須修正 1 件（`removePhotos` の D1 パラメータ上限）と A の決定 #296（`deleted_at` の条件のテスト）を #294 に積んだ。
`pnpm -r test`（api 591 / app 389 / db 31 / ui 16 / date 66）・型チェック・lint 緑。

---

**最終更新（旧）**: 2026-09-14 / セッションB。**041 段階1（PR #294）: R の必須修正 1 件と A の決定 #296 に対応した（head は PR 参照）。main（013da5e）を取り込み済み。R の再判定待ち。**
- 必須修正: `album.removePhotos` の DELETE を 50 個ずつの文に分けて 1 本の `batch()` に（D1 の束縛パラメータ上限 100。`architecture.md` 4節の条文）。画面は 100 枚を超える選択を 100 ずつ送る（`lib/chunk.ts`）。文ごとのパラメータ数を固定するテストを足した
- A の決定: `posts.deleted_at IS NULL` の条件そのものを見るテストを足した（`deleted_at` を直接立てて `post_images` を残す。条件を外すと赤になることを確かめて戻した）
- R の判定文は `artifacts/041/review-stage1.md` に一字一句。対応は `artifacts/041/stage1.md` 末尾「レビュー往復 1 回目」
- `pnpm -r test`（api 591 / app 389 / db 31 / ui 16 / date 66）・型チェック・lint 緑
- R が確定 → B がマージ → **人間の許可を取って**リモート D1 へ `0022_albums.sql` → 人間の実機（1 枚保存・5 枚アップロード・画像が見える）。段階2の要否は実機のあとに A が人間に聞く

---

**最終更新（旧）**: 2026-09-14 / セッションA。**041 段階1（PR #294）: R の判定は必須修正 1 件（`removePhotos` の D1 パラメータ上限）。A は直すと決め、R の記録（`deleted_at` のテスト）も 1 本足すことにした。B の手番。**
- D1 の「1 文 100 パラメータ」は `architecture.md` 4節に条文として残した（ローカル SQLite では見えず本番だけ落ちる種類の穴）
- B が直して R が確定 → B がマージ → リモート D1 へ `0022_albums.sql`（人間の許可）→ 人間の実機
- 詳細は `(tabs)/album-detail.tsx?id=`（静的エクスポートの前提を守る）・「‹ 戻る」は一覧固定・`Sheet` を部品に・思い出カードにも保存ボタン・
  `startDate` を外すと `endDate` も外れる・アップロードは直列・「もっと見る」で次ページ。定義（041 の 2・3節、`architecture.md` 5節）を今の形に合わせた
- R が受け入れたら B がマージ → リモート D1 へ `0022_albums.sql`（人間の許可）→ 人間の実機（1 枚保存・5 枚アップロード・画像が見える）
- 段階2（タイムラインの写真を複製して入れる）の要否は、人間の実機のあとに A が人間に聞く

---

**最終更新（旧）**: 2026-09-14 / セッションB。**041（アルバム）段階1: 実装完了。PR を出した。R の手番。**042 は 041 のマージ後。
- 段階0の結果: **R2 は `response-content-disposition` を返す（(a)）**。署名後にクエリを書き換えると 403。`artifacts/041/download.md`
- 段階1: `albums`・`album_photos`（0022）・契約 `album.*` / `photo.*`・`procedures/album.ts`・`me.delete`・一覧 `(tabs)/album.tsx`・詳細 `(tabs)/album-detail.tsx?id=`・
  作成/編集モーダル（カバー選択）・複数アップロード（進捗）・ビューアの説明文と保存ボタン（投稿カードにも）・ホームの「今日どうだった？」→「アルバム」・デモシード・T1〜T15
- 報告は `artifacts/041/stage1.md`（T1〜T15 の対応・**B が決めたこと 10 点**〈詳細は `?id=`・ヘッダーの戻る・Sheet の共通化・思い出カードにも保存ボタン 等〉）、
  スクリーンショットは `artifacts/041/stage1/`（両モード × ホーム・一覧・作成・詳細・ビューア・選択・進捗・タイムライン・ゲスト・PC 幅）
- `pnpm -r test`（api 589 / app 386 / db 31 / ui 16 / date 66）・型チェック・lint 緑
- **人間の手番（マージ・デプロイ後）**: 本番で 1 枚保存できる（PC のブラウザ・iPhone の Safari）、5 枚まとめてアップロードできる、画像が見える（ローカルは R2 の制約で画像が出ない）
- 040 の残り（Amazon の URL の本番確認）も人間の手番のまま

---

**最終更新（旧）**: 2026-09-13 / セッションA。**041（アルバム）を起票した（#292・#293）。B の手番。**042（まとめて ZIP）は 041 の後。
- アルバムは自分の実体（`albums/` のキー）を持つ写真を**直接アップロードする**（モックどおり）。タイムラインの写真を入れるのは段階2（R2 の複製）
- タスク: `docs/tasks/041-album.md`。**段階0は 30 分の確認（R2 の `response-content-disposition`）で止まらない**（(a)/(b) は先に決めてある）
- **1 枚のダウンロードは 041 に入っている**（ビューアの保存ボタン。タイムラインにも効く）。ZIP だけ 042
- 人間に見てほしいのは 1 つ: **ホームの「今日どうだった？」パネルをアルバムに置き換える（9 枚のまま）**
- 設計ドキュメント: `architecture.md` 3節（ホームの入口）・4節（`albums`・`album_photos`）・5節（`album.*`・`photo.*`）・6節（保存用の署名付き URL）、
  `requirements.md` 5節（アルバムの行を 041 に）、`security-requirements.md` 5節（保存用 URL）
- モック: `docs/sample/simpleMode/アルバム機能/`（人間が置いた 5 枚。ホワイトの絵。ピンクは既存の部品に任せる）
- 040 の残りは人間の手番のまま（下記）

---

**最終更新（旧）**: 2026-09-13 / セッションB。**040（ほしいもの）段階1: R 受け入れ確定（head 0f9d7ab。判定は `artifacts/040/review-stage1.md`）→ #289 をマージ。A・B・R に手番は無い。**
**残りは人間の手番（デプロイ後）**: 本番で Amazon の URL を 1 本、できれば `amzn.asia/d/…` の共有リンクも 1 本貼り、画像が付くことと保存される URL が `/dp/{ASIN}` の正規形になることを確かめる。画像が取れなかった行に写真を付けられること（実機）。
経緯: R の必須修正 2 件（属性値の引用符・Amazon 規則は最終 URL のホストで判定）、A の決定 #291（正規化も最終 URL に・ログの Content-Type 100 文字）、R の追加の必須（画像の fetch の期限共有テスト）をすべて #289 に積んだ。
報告は `artifacts/040/stage1.md`（守りたいことの一文とテストの対応・T1〜T11・B が決めたこと 6 点）、スクリーンショットは `artifacts/040/stage1/`。
`pnpm -r test`（api 549 / app 364 / db 30 / ui 16 / date 61）・型チェック・lint 緑。
**人間の手番（マージ・デプロイ後）**: 本番で Amazon の URL を 1 本貼って画像が付くこと、画像が取れなかった行に写真を付けられること（実機）。

## 041 段階1: 実装した（2026-09-14）

- 段階0: 本物の R2 に aws4fetch で直接署名して実測。`response-content-disposition` 付きで署名 → `content-disposition: attachment; filename="…"` が返る。
  クエリ無しでは返らない。署名後に書き換えると 403。**(a)** で進めた（`createDownloadUrl`。5 分）
- `albums` / `album_photos` 表（0022）・契約 `album.*` 9 本 + `photo.*` 2 本・`procedures/album.ts`（カバーの解決は読む側 1 文で「指定 → 無ければ最新」に倒す）・
  `me.delete` に `album_photos` → `albums` と `albums/` の接頭辞・`packages/date` に表示用の整形 6 関数
- 画面: 一覧（タイムラインのカード + 2 列グリッド。`+` はヘッダー右）・詳細（`?id=`。カバー・見出し 2 行・3 列・FAB・選択モード・編集）・作成/編集モーダル・
  ビューアに説明文/保存/説明の編集・投稿カード（思い出カードも）からのビューアに保存ボタン・ホームの置き換え・デモシードにアルバム 1 件（3 枚）
- **B が決めたこと（A に見てほしい。詳細は stage1.md）**: 詳細は `[id].tsx` ではなく `album-detail.tsx?id=`（静的エクスポートの前提を崩さない）、
  詳細のヘッダー左に「‹ 戻る」（一覧へ固定）、`Sheet` を部品に出して want.tsx も使う、思い出カードにも保存ボタン、`startDate` を外すと `endDate` も外れる、
  アップロードは 1 枚ずつ直列（進捗と ULID の順序のため）
- ローカルの撮影で分かったこと: `<a download>` はクロスオリジンでは遷移になり、本番は attachment で保存になるがローカルは実体が無く 404 に遷移する（`download.md`）
- 撮影スクリプトは `artifacts/041/scripts/`（`make-session.mjs`・`capture.mjs`）

---

## 040 段階1: 実装した（2026-09-13）

- `wants` 表（0021）・契約 `want.*`・`lib/link-preview.ts`（外部 fetch の 1 本。6節の条件を全部この中で）・`lib/want-url.ts`（Amazon の正規化）・
  `procedures/want.ts`・`me.delete` に wants と `wants/` の接頭辞削除・`(tabs)/want.tsx`・ホーム 3 列 × 3 行・アイコンと写真タイル・デモシード各 2 件
- **B が決めたこと（A に見てほしい）**: カードの題名 3 行/メモ 2 行で省略、ゲストは slot 1/2 の 2 人分を見られる、
  IP リテラル等の URL は「保存は通るが取りに行かない」、手で付けた画像があれば画像は取りに行かない、画像無しのしるしは 🔗 のグリフ、
  `ownerName: null` = そちら側の人が居ない（詳細は stage1.md）
- ライブ保存（ローカルの wrangler dev から人間の Amazon URL）: 約 2 秒で画像が付き、URL 正規化・題名の整形（100 文字）・R2 の実体（JPEG 1500×1500・66KB）を D1/R2 で確認。
  **ローカルでは署名付き URL が本物の R2 を指すため画面に画像は出ない**（投稿画像と同じ既存の制約）。本番は人間が確認
- 段階0の測定スクリプトは `artifacts/040/spike/`、撮影は `artifacts/040/scripts/`

---

**最終更新（旧）**: 2026-09-13 / セッションA。**040 段階1（PR #289）: R の判定は必須修正 2 件。A が決めた: (1) `content` 属性の `'` で切れる正規表現を直す (2) Amazon 専用の規則は最終 URL（リダイレクト後）のホストで判定し、正規化も最終 URL に対して行う**（`amzn.asia/d/…` の短縮 URL が Amazon アプリの共有の形）。B が #289 に積み、R が確定 → マージ → デプロイ → 人間が Amazon の URL を 1 本貼る。

---

**最終更新（旧）**: 2026-09-13 / セッションA。**040 段階0の結果を受けて決定した。B は段階1に入る。**
決定: Amazon 専用の抽出（`meta name="title"` + `data-old-hires`。Amazon のホスト限定）を入れる・読み取り 1MB・全体 12 秒・
`charset` で `TextDecoder`・Amazon の題名は前置き/後置きを落として 100 文字・無印は何もしない。
`security-requirements.md` 6節を書き直した（条文は「決まった属性値だけ」+ 店ごとの規則を列挙）。詳細はタスク定義「段階0の決定」。

---

**最終更新（旧）**: 2026-09-13 / セッションB。**040（ほしいもの）段階0を測って止まった。A の決定待ち。**
結果は `artifacts/040/spike.md`。要点: **Amazon は OGP を持たない**（0/30）。Amazon 専用の予備（`meta name="title"` + `data-old-hires`）なら
512KB で 28/30・1MB で 15/15。UA の差は無し。楽天は Cloudflare からだと約 10 秒（5 秒では 0/6）で EUC-JP。ユニクロは OGP で 6/6。無印は 520。
A が決める論点は 6 つ（spike.md 末尾）。段階1はその決定を待って着手する。



## 040 段階0: 測った（2026-09-13）

- 使い捨て Worker（`artifacts/040/spike/`）を `wrangler dev --remote` で Cloudflare 側から実行。URL × UA 2 通り × 3 回
- **Amazon: 200 は 30/30、OGP は 0/30。**題名は `<meta name="title">`（156 文字。上限 100 超）、画像は `#landingImage` の `data-old-hires`。
  画像ブロックは 325〜612KB の位置で応答ごとにずれる。1MB まで読めば 15/15。画像は jpeg 55〜196KB
- **楽天: 5 秒で 0/6。**25 秒待てば 3/3（毎回 10.1 秒）。`charset=EUC-JP`（Workers の `TextDecoder` で復号可）
- **ユニクロ: OGP 6/6**（0.2 秒）。**無印: 520 が 6/6**（Cloudflare 配下で Worker を拒む）
- タスク定義 2節の表に当てると、OGP だけなら「自動を入れない」、Amazon 専用の予備を入れるなら「(1) で行く」。**判断は A**
- 段階1のフィクスチャ（T2）は `spike/html/` に 5 本
- ブランチ `task/040-want-list` はこの証跡と現在地だけ。段階1の実装はまだ無い

---

**最終更新（旧）**: 2026-09-13 / セッションA。**040「ほしいもの」を起票した（#282）。B の手番: 段階0（Amazon が通るかの測定。1〜2 時間）で止まって A に報告。**
AI まとめの不具合は #280 で閉じた。039 は閉じた。残りの人間の手番: 039 のヒーロー用の写真・iPhone 実機。

## 040: ほしいもの（起票。2026-09-13）

人間の指示: 「買ってほしい」ではなく「こういうのが欲しい」を伝える一覧。URL を貼ると小さく画像。
人物タブ（初期は相手。自分を選ぶと追加ボタン）。新しい順だけ。**価格は持たない。**
絵は `docs/sample/simpleMode/新機能/…Wishlist….png`。

**A が決めたこと**: 027「リスト」とは別の表 `wants`・契約 `want.*`。書けるのは本人だけ。
画像は「URL から自動（OGP）」と「手で付ける」の 2 経路で、**自動は主役ではない**（Amazon は無名の fetch を弾くことがある。
他社クローラの名前は名乗らない）。取った画像は R2 に保存し、CSP を開けない。外部 fetch の条件は
`security-requirements.md` 6節「外部 URL の取得」。ホームの機能パネルは 9 枚になるので **3 列 × 3 行**に。

**段階0で止まる**: B が Worker（`--remote` か本番）から Amazon の URL を fetch して `og:image` の取得率を測る。
結果で A が「自動を入れる／入れない」を決める。

---

**最終更新（旧）**: 2026-09-13 / セッションB。本番の AI まとめの不具合は PR #280 で直し、デプロイ後に本番で通ることを確認した
（閉じた）。039 は段階1・2ともマージ済み（#271・#275）。

## fix: AI まとめ — 閉じた（2026-09-13）

- PR #280（R 受け入れ。判定は `artifacts/fix-ai-summary-max-completion-tokens/review.md`）→ main `e48eb97` → Deploy（人間が承認、01:14Z success）
- **本番で確認**: tail で 10:18:27 の `aiSummary/generate - Ok` にエラー行無し（修正前は `openai 400`）。直後の `get` も Ok
- 以後、プロバイダの失敗はサーバログに status + 本文の先頭 200 文字が残る（R の但し書き: キーが混ざらないのは「この PR が足す経路からは入らない」の範囲）
...
**最終更新（旧）**: 2026-09-13 / セッションA。**039（ホワイトモード）は段階1・2ともマージ済み（#271 `d6794d8`・#275 `3856c7f`。R 受け入れ済み）。
A・B・R に手番は無い。残りは人間**: ホームと統計のホワイトを見て一言・**ヒーロー用の本物の写真**（横長 4:3、長辺 1280px を
`docs/sample/simpleMode/` へ。来たら B が `packages/ui/src/assets.ts` の `statsHeroPlaceholder` を差し替える小さな PR）・iPhone 実機（Safari）。
次のタスクの指示を待つ。

## 039: 閉じた（2026-09-13）

- 段階1（基盤 + パレット + 切り替え）: PR #271。R の必須修正1件（T7 のサブパスの穴）を直して確定
- 段階2（モック固有の形。ホワイトのときだけ）: PR #275。R 必須修正無し。A の指示1件（統計ヘッダ）は A が一度誤り、
  `headerShown: false` に訂正して `e73b139` で確定
- 設計文書: `architecture.md` 7節「外観」（仕組み・値・段階2の形・統計ヘッダの訂正）、`security-requirements.md` 7節（inline script 2本）、
  `conventions.md` 6節（留め金はパッケージの境界に打つ）、ADR-014
- **受け入れた差**: ピンクの利用者にも Poppins 300（約 8KB）が preload される（R の記録）。静的 HTML なので外観で分岐するには
  inline script がもう1本要る。8KB のために CSP のハッシュを増やさない
- **残る仮**: 統計のヒーロー写真（`docs/sample/風景/RcmUGlPg.jpg` の切り出し）。本物が来たら差し替え口1箇所

**A の反省（次に効く）**: T7 の指示を「import 文を走査」と手段で書いて穴を作った（→ conventions.md）。
統計ヘッダは「戻るを残す」とスタックのつもりで指示したが Tabs のヘッダで題しか無かった（→ architecture.md）。
**どちらも、対象を見ずに形だけで判断した。**次からは「何を守りたいか」の一文と、対象そのものを見てから書く。


## 039 段階2: 上がった（2026-09-13）

- a〜h のうち、やった: a（ロゴ文字。Poppins 300）・b（記念日カード）・c（写真タイル8枚）・f（統計のヒーロー〈仮〉+ 二段見出し + 区切り線）。
  e は元からモックの形で変更無し。g は足さない（A の判断）。h は「近日公開」を `feature-panel.tsx` の中だけに。d は段階1で済み
- 画面ファイルで `appearance` を読むのは `index.tsx`（a）と `stats.tsx`（f）の2箇所だけ（列挙は報告1節）
- ピンクはホーム・タイムライン・統計で main と画素 0 差（`artifacts/039/stage2/pink-pixel-diff.json`）
- **人間の手番**: 段階2のスクリーンショット確認、ヒーロー用の本物の写真（横長 4:3・長辺 1280px）
- **A の手番**: 報告2・3節の値（ロゴ 40pt / 字間 0.5、タイルの寸法）を `architecture.md` 7節へ

---

**最終更新（旧）**: 2026-09-13 / セッションA。**039 段階1: 人間 OK・R 受け入れ（必須修正1件: T7 のサブパス import の穴）。B が #271 で直してから R が確定。並行して B は段階2へ。**
アセット8枚は到着済み（`docs/sample/simpleMode/`。対応表はタスク定義末尾）。`新機能/` は今回使わない。

## 039 段階1: R の判定（2026-09-13）

受け入れ。必須修正1件: **T7 が `"@futary/ui"` の完全一致で走査しており、`@futary/ui/src/theme` のサブパス import で
`themes` が静的に取れる**（型チェックも通る。R が実測）。反対側も `export *` の先を `tokens.ts` に決め打ちしていて、
`export * from "./theme"` を足しても緑のまま。直し方は R の提案どおり（パッケージ名始まりを全部対象・`export *` の先を全部開く）。
**A の判断: テスト側で塞ぐ。`package.json` の `exports` は Metro での挙動を実測しない限り足さない。**
教訓を `conventions.md` 6節「留め金は『パッケージ名』ではなく『パッケージの境界』に打つ」に書いた。

他5点（T1 の凍結・inline script と CSP・hydrate の検査・分岐の置き場・サーバ側）は問題なし。
記録1件: `appearance.tsx` の theme-color のコメントの理由が古い（B が同じ PR で直す）。
...
A の依頼で 039 より先に片付けた。high 2件（sharp・js-yaml）はどちらも修正版が
あるので更新で対処。`miniflare` が sharp を固定で宣言しており、`pnpm update` が
推移的依存を動かさなかったため、`overrides` を使った。削除条件は
`pnpm-workspace.yaml` のコメントに書いてある（miniflare が sharp>=0.35.4 を
宣言する版へ上がったとき・`@expo/xcpretty` が js-yaml>=4.3.2 を解決するとき）。
詳細は `docs/worklog.md` 2026-09-13 セッションB。

---

**最終更新（旧）**: 2026-09-13 / セッションA。**039（ホワイトモード）を起票した。**
**B の手番。**`docs/tasks/039-white-mode.md` を読んで段階1から着手する。

## 039: ホワイトモード（起票。2026-09-13）

人間の指示: 「whiteMode という UI を B に作らせたい。マイページで切り替えられる。
Apple のようなおしゃれで洗練された画面。B は Fable にした。必要なアセットはあとで上げる。
**詳細な数値の決定は B（Fable）に任せてよい**」。

モック3枚は `docs/sample/simpleMode/`（A がコミット済み）。

**A が決めたこと（仕組みと境界）**:
- `colors` `shadow` `gradients` を静的 export から外し、`useTheme()` から取る。
  **静的 export を消すことが留め金**（型チェックが直し忘れを全部挙げる）
- 保存先は端末の `localStorage`（`futary.appearance`）。サーバに置かない。**ADR-014**
- 既定はピンク。ピンクの見た目は1ピクセルも変えない（T1 で凍結）
- **段階1（基盤 + パレット + 切り替え）が終わったら止まって人間に見せる。**段階2（モック固有の形）は OK 後
- やらないもの: 「・/」の記号、OS ダークモード連動、サーバ同期、3つ目のモード

**B が決めること**: パレット・影・大きさ・字間・ウェイトの数値すべて。決めた値と理由を結果に書く
（A が `architecture.md` 7節へ写す）。

**人間の手番（まだ）**: 段階1のスクリーンショットを見て OK を出す。写真アセット（機能パネル8枚・統計ヒーロー1枚）を
`docs/sample/simpleMode/` に上げる（B が段階1の報告で必要な形・枚数を確定させる）。

`architecture.md` 7節に「外観」の節を足した（仕組みのみ。値は B の報告後）。

---

**最終更新（旧）**: 2026-09-06 / セッションB。AIまとめの既定モデルをOpenAIの
`gpt-5.6-terra`へ変更（人間の直接指示。`fix/ai-model-gpt-5.6-terra`）。
**PR #267 でマージ済み（`8644c4e`）。**

## AIまとめの既定モデル変更（gpt-5.6-terra。2026-09-06）

人間から直接、`apps/api/src/lib/ai.ts`のOpenAI既定モデルを
`gpt-5.6-terra`へ変える指示があった（「1日250万トークンまで無料」との
理由）。

**セッションBが最初に誤った情報で答えた**: web検索の要約だけを見て
「gpt-5.6-terraは無料枠の対象外」と伝えたが、人間が提示したOpenAI公式
ヘルプページ（フィードバック・評価・ファインチューニングデータ・API
入出力の共有について）を実際に開いて確認したところ、**`gpt-5.6-terra`
は無料トークン対象モデルの一覧（10Mトークングループ）に実際に含まれて
いた**（使用量ティア1〜2は1日2.5Mトークン、ティア3〜5は10Mトークン）。
人間の最初の主張が正しく、Bの訂正が誤りだった。要約だけで判断せず、
一次情報を開いて確認すべきだった。

**無料化の条件**: OpenAI組織オーナーが`platform.openai.com`のdata
controls設定で「入力・出力をOpenAIと共有する」をオンにする必要がある
（コード側では制御できないアカウント設定）。この設定をオンにすると、
共有した投稿本文がOpenAI側のモデル改善に使われうる。ADR-013
（`docs/decisions.md`）は「生成のためだけに送る」という前提で同意を
求めているが、futaryは人間と、そのパートナーの2人だけが使う私用アプリ
であり、同意する当人がこのデータ共有の判断者と一致するため、**人間の
判断でADR-013の同意文言は変更しないことにした**。

`apps/api/src/lib/ai.ts`のコメントに、この判断の理由（データ共有は
アカウント設定でありコードでは制御できないこと・ADR-013を変えない
理由）を明記した。`DEFAULT_MODELS.openai`を`"gpt-4o-mini"`から
`"gpt-5.6-terra"`へ変更し、対応するテスト（`ai-summary.test.ts`の
`result.model`アサーション）とデモシード（`packages/db/seed/demo.ts`の
表示用モデル名）も揃えた。

**OpenAI側の「入力・出力を共有する」設定は人間自身が有効にする必要が
ある**（B・A・Rのどのセッションからも操作できない）。

`pnpm -r test`（apps/api 457件・apps/app 293件、全て緑）・型チェック・
lint、全て通過。

以下、2026-09-06（この変更より前）の記録。

**最終更新（旧）**: 2026-09-06 / セッションB。**037・038、両方ともmainへ
マージ完了**（PR #250・PR #259）。**037も038も閉じ、残っているのは
人間の手番だけ**（デプロイの承認・OpenAIのキーの設定・実機確認）。
次のタスクの指示を待つ。

## 038: マージ完了（2026-09-06）

PR #259（見出しの言葉も条件の話に揃えた最終版）をsquash-mergeした
（マージコミット`47be1bc`）。`task/038-viewer-key-scan`ブランチはリモート・
ローカルとも役目を終えた（リモートは削除済み）。

`viewer-key-coverage.test.ts`の作り直しは7回に及んだ（手で並べた一覧→
readProcedure走査→近傍N文字→AST2段→`orpc`識別子起点→import文の形→
`useQuery`側へ→ライブラリから引く→条件を検査する、と数えると更に多い）。
5回目まではAが「`orpc`へどう辿り着いたか」という開いた集合を留め金に
していたのが原因で、Rも同じ枠の中で5回、同じ形の穴を開けていた
（Rが自ら記録: 「もっとうまく`orpc`を追う方法」を探している限り、
追う対象が間違っていることには気づけなかった）。

**一番効いたのはBがライブラリの実装を開いて`query`/`infiniteQuery`を
発見したこと**だとAから評価された。Rはこの2つを`Object.
getOwnPropertyNames`の出力に実際に見ていながら、名前だけで見送って
いた（「出どころから引け」と言った本人が出どころを開いていなかった、
とRが自省）。

R自身も、038の途中で「`getQueriesData`を条件つきにしたら`timeline`が
赤くなる」という帰結を計算せずに指摘していたことが分かった。B（自分）が
実際にテストを走らせて`timeline.tsx`の`getQueriesData`呼び出しが新たに
赤くなることを発見し、`viewer-key-coverage-ignore`コメントを追加して
対応した一連の流れを、Aから「実際に走らせて見つけて報告したのが、今回
いちばん正しい仕事だった」と評価された。
...
## 038: 最後の1件（メッセージの誤誘導）に対応（2026-09-06）

Rが免除・条件の検査の両方を実ファイルで確かめた（`previousQueries`を
全部追い、3箇所だけで他の経路が無いこと・`[queryKey, state.data]`の組で
keyとdataが構造的に対応していることまで確認済み。免除コメントを外すと
落ち、条件を崩すと落ちることも確認済み。「飾りになっていない」）。
**残りは1件、メッセージの誤誘導だった**（PR #262でAが判断確定）。

`conditional`な2つ（`getQueriesData`/`setQueriesData`）が条件を満たさず
赤くなったとき、他の呼び出しと同じ「viewerKeyが確認できません」という
文言を出していた。だがこの2つが壊れているのはviewerKeyの有無ではなく
条件そのもの（戻り値を消費していない／updaterが関数式である）であり、
この文言を読んだ人がviewerKeyを足しても直らない（037の短縮記法のときと
同じ「一番悪い壊れ方」＝メッセージが嘘をつく）。`describeMissingReason`
を新設し、この2つだけ条件の話をする文言を返すようにした。

もう1件（`(old) => 誰かのデータ`のように、関数の形をした注入が条件を
満たしてしまう件）は**Aの判断で直さない**。他人のデータを注入するには
先に他人のデータを手に入れる必要があり、その入口（`getQueriesData`や
単一キーの読み）は別途塞いでいるため、いまは届かない。「厳しすぎるが、
緩いより良い」（Aの言葉）。この限界は`conventions.md`に明記されており、
分類マップの`reason`にも同じ内容を書いた。

**Rの自己言及**: 「`getQueriesData`を条件つきにしたらtimelineが赤くなる、
という当たり前の帰結を計算していなかった」とR自身が書いていた。Aが
「私が『緑のまま』と書いたのと、同じ形」と評した。B（自分）が実際に
テストを走らせて`timeline.tsx`の`getQueriesData`呼び出しが新たに赤く
なることを発見し、ignoreコメントを追加して対応した一連の流れが、Aから
「実際に走らせて見つけて報告したのが、今回いちばん正しい仕事だった」と
評価された。
...
## 038: 残り2件（getQueriesData/setQueriesDataの理由の誤り。2026-09-06）

Rが「留め金は閉じた」と判定した（列挙はライブラリから引かれ、別名import・
`useSuspenseQuery`が実ファイルで赤くなることを確認済み。`query`/
`infiniteQuery`を実装から見つけたのが効いた）。**残り2件は穴ではなく、
分類理由の誤りだった**（PR #261でAが判断確定）。

Rが実装（`node_modules`内のqueryClient.js）を読んだところ:
- `getQueriesData`の理由「他人の枠を覗くことにはならない」は誤りで、
  実際には**他人の`state.data`が配列に入って返る**
- `setQueriesData`の理由「updater関数が各自の既存データを変換するだけ」も
  誤りで、**updaterが関数のときだけ正しい。値を渡すと全員の枠へ注入される**

「いまのコードは安全（`timeline.tsx`は返り値を消費せず、updaterに関数を
渡している）。ただし、安全な理由が書いてある理由と違う」。「理由が違うと、
次に`getQueriesData(...)[0][1]`を読んで画面に出す人が止まらない」（Rの
指摘）。**条件を検査する。書くだけにしない**、との判断で、2つを`prefix`
から新設の`conditional`バケットへ移し、`getQueriesData`は戻り値が消費
されていないこと、`setQueriesData`はupdaterが関数式であることを機械的に
検査する形にした。

`timeline.tsx`の実際の`getQueriesData`呼び出し（戻り値を変数へ代入して
いる）は、この検査により新たに赤くなったため、既存の`setQueryData`
（`onError`側）と同じ形で`viewer-key-coverage-ignore`コメントを追加した
（戻り値はcontext経由でonErrorのsetQueryDataへ、同じキーへ書き戻すため
だけに使う。画面には表示しない）。`setQueriesData`の呼び出しはupdaterが
関数式のためコメント無しで緑のまま。

Rの「小さいもの」の注文（「今日のappで実際に使われている9種」の手書き
一覧が、`cancelQueries`を1件消しただけでセキュリティと無関係に赤くなる
件）にも対応し、理由をコメントに明記した。

`pnpm -r test`（apps/api 457件・apps/app 291件、全て緑）・型チェック・
lint、全て通過。

以下、2026-09-06（この対応より前）の記録。

## 038: T9の検査をライブラリから引く形にする（2026-09-06）

037で「TanStack Queryのキャッシュのキーを取るAPI」という閉じた集合へ
留め金を移したこと自体は正しかった（Rの判定）が、実際には手で9件を
...
`fetchQuery`・`getQueryState`等）。「閉じた集合であることと、その集合を
正しく持っていることは別だった」（Aの言葉）。

**1. 列挙をライブラリから引く**: `import * as ReactQueryModule from
"@tanstack/react-query"`をテストに追加し、`Object.keys(...)`・
`Object.getOwnPropertyNames(QueryClient.prototype)`で実際の公開面を
テスト実行時に読む形にした（59件のexport・34件のメソッドを実測）。
分類マップとの`toEqual`で固定し、ライブラリを上げて公開面が変わると
このテストが赤くなるようにした。

**2.「キーを取るもの／取らないもの」の仕分け**: `node_modules`内の実際の
実装（`@tanstack/query-core`のqueryClient.js）を読んで、戻り値が
キャッシュ済みデータそのものを返すかで判断した。exact（厳密に
viewerKey必須）19件・prefix（前方一致で不要）7件・excluded（データを
読み書きしない）67件に分類。**実装を読んだことで、タスク定義にもRの
指摘にも無かった`query`/`infiniteQuery`（`fetchQuery`/`fetchInfiniteQuery`
の後継の新API）が実はデータを返す危険な項目だと分かった**（手で写す
限り絶対に気づけなかった項目）。

**3. 別名importとブラケット記法を塞ぐ**: 037で`lib/orpc`にやったのと
同じ形を`@tanstack/react-query`にも適用した。`@tanstack/react-query`
からのimportは別名なしの名前付きimportに限り、`queryClient`のメソッド
呼び出しはドット記法に限る（決めた内容は`artifacts/038/summary.md`に
明記。**conventions.mdへの反映はAの担当**）。

**4. 両側から当てる**: Rの12通りを合成コードで実際に検証し、想定どおり
（データを返す10通りは赤/緑が分かれ、`setQueryDefaults`系は理由つきで
対象外、ブラケット記法・別名importは別検査で赤）になることを確認した。
037までに閉じたもの（使用箇所6通り・短縮記法4通り・`lib/`・
`apps/app/app/test/`・免除の名指し・再エクスポート3通り）が引き続き
効くことも再確認した。

`pnpm -r test`（apps/api 457件・apps/app 283件、全て緑）・型チェック・
lint、全て通過。

以下、2026-09-06（038着手より前）の記録。

## 037: マージ完了（2026-09-06）

PR #250をsquash-mergeした。マージ直前、CIの`gitleaks`が赤くなっている
ことに気づいた（`security-requirements.md`「検出1件で赤。例外なし」）。
検出されたのは`apps/api/test/ai.test.ts`のテスト用偽キー（実際の秘密情報
ではない）で、末尾が「ハイフン+数字3桁」という形だとgitleaksの
`generic-api-key`ルールに引っかかっていた（ローカルで`gitleaks
detect --no-git`を使い、候補の置き換え文字列を試して確認した。この
worklog自体にも元の文字列をそのまま書くとgitleaksが再度検出するため、
ここでは形だけを記す）。

**gitleaksはコミット履歴の差分全体を見るため、新しいコミットで文字列を
直すだけでは直らない**（古いコミットの差分に残ったままだと拾われ続ける）。
このタスクブランチは自分だけの未マージのものだったため、`git reset --soft`
でコミットを1つに整理し直してから修正を含めてコミットし、
`git push --force-with-lease`で上書きした（人間に確認のうえforce-push。
mainや他セッションのブランチには触れていない）。CIが緑になったことを
確認してからマージした。

`gh pr merge --delete-branch`はworktree構成（`main`が`futary-A`で
チェックアウト済み）のため一部失敗したが、GitHub上のマージ自体は成功
しており、リモートブランチは`gh api`で個別に削除した。
...
**残っているのは人間の実機確認だけ**（完了条件「人間が実機で見て、境目が
見えないと言った」。`artifacts/036/manual-check.md`参照）。次は人間の
デプロイ承認と実機確認を待つ。

`conventions.md`6節に「測定を足したら、両側から当てる」が追加された
（036で同じ形の間違いを3回踏んだことを受けた規約化。3回のうち2回はA、
1回はR。B実装の問題ではないが、次に測定を追加するときは自分でも
壊れた状態と対象の無い場所の両方に当てて確認すること）。

## 036: 縦の継ぎ目（Rレビュー指摘・R-1/R-2対応後の3巡目）

R-1（正規表現バグ）は解消済みとRが確認。R-2（`cover`→`stretch`）は
**横の継ぎ目が消えた代わりに縦の継ぎ目が出た**とRが指摘した。
`stretch`にしても画像は横に引き伸ばされておらず、`bokeh.png`の元の幅
（853px）の位置（x=852→853）に、1280px幅で差5・1920px幅で差4の段差が
残っていた（人間のiPhone測定+4.4と同水準）。

原因をDOMで実測したところ（`artifacts/036/bokeh-dom-measurement.json`）、
`resizeMode="stretch"`は`background-size: 100% 100%`に正しく翻訳されて
いたが、**背景を持つ要素自体の幅が画面幅ではなく画像の自然幅（853px）の
ままだった**（`position:absolute`に`left:0`/`right:0`だけでは、この要素は
画面幅まで広がらない）。`resizeMode`は箱の中身の収め方を決めるだけで、
箱自体の大きさは決めないというRの指摘どおりだった。

`Image`のstyleに`width: "100%"`を明示的に追加し、DOMで390/768/1280/1920
の全幅で背景要素の幅が画面幅と一致することを確認した。あわせて、行方向
だけでは縦の段差を検出できない（Rの指摘。#242と同じ「基準が対象を
測っていない」形、向きは逆）ため、列ごとに隣接列の差を出す測定を追加し、
4幅ともx=853付近の段差が消えたことを確認した（`artifacts/036/seam.md`
「Rレビュー指摘: 横の継ぎ目が消えた代わりに縦の継ぎ目が出た」節）。

`pnpm --filter @futary/app test`（224件緑）・型チェック・lint、全て通過。

## 036: Rレビュー対応（R-1・R-2、2巡目）

**R-1**: `danger`スコープテストの正規表現が`g`フラグ付きで`.test()`を
使い回しており、`lastIndex`が進むため2件目以降のファイルを取りこぼす
バグがあった（`viewer-key-coverage.test.ts`と同じ形）。`g`を外して修正し、
実際に別ファイル（`list.tsx`）へ一時的に`variant="danger"`を足してテストが
赤くなることを確認してから元に戻した。

**R-2**: `resizeMode="cover"`と`BOKEH_HEIGHT`(420)の組み合わせが、窓幅が
1195pxを超えるデスクトップ（1280・1920等）で崩れることが判明した。
`cover`の拡大率は横方向の必要倍率で決まるため、広い窓では縦方向がその
倍率で拡大された上でクロップされ、焼き込んだアルファのランプの下端が
切り落とされて継ぎ目が復活する。確認が390×844でしか行われていなかった
ことが原因。`resizeMode="stretch"`に変更し、縦横比への依存自体を無くした。
390/768/1280/1920の4幅で再実測した。フェード帯の最大差（1.5〜2.0）は
対照区間の基底ノイズ（0.75〜1.0）の**約2倍**で、「同水準」ではない
（Aの指摘で訂正）。ただし桁は同じで、「何百行も完全に同じ値」の区間は
出ておらず、フェード帯だけグラデーションが2重に重なる分の量子化増加として
妥当と判断した。詳細は`artifacts/036/seam.md`・`seam-measurement-widths.json`。

## 036: 継ぎ目

`packages/ui/assets/bokeh.png`にアルファのランプ（下35%を255→0）を
焼き込み、`Screen`から固定色へのフェードレイヤーを削除した。Aに実測結果を
上げたところ、**「差が1.0以上の行が無いこと」という基準そのものが誤りだった**
との訂正を受けた（対照区間＝ボケの乗らない領域でも同水準のノイズが出た
ため。PR #242 / `e6a22fd`でタスク定義を訂正済み）。訂正後の基準（対照区間
との比較）では、修正前のy=420の段差（最大4.0、対照区間の基底ノイズ1.0を
明確に超える）が、修正後は対照区間と同水準（1.0）まで下がっており、
問題になっていた「何百行も完全に平坦」のパターンも修正前後とも検出
されないことを確認した。詳細は`artifacts/036/seam.md`参照。

`BOKEH_HEIGHT`(420)が画像の縦横比（853×300）に依存する値であることを
コードコメントに追記した（Aの指摘）。

**完了条件に「人間が実機で見て、境目が見えないと言った」が追加された。**
数値だけでは完了と言えないため、`artifacts/036/manual-check.md`に
実機確認項目として明記した。

## 036: danger

`danger`トークン・バリアント（`delete-account.tsx`の「ふたりのデータを
削除する」）は実装・テストとも完了。既存テスト224件（新規2件含む）・
型チェック・lint、全て通過。

タブバーの不具合: `(tabs)/index.tsx`（ホーム）にしか`TAB_BAR_CLEARANCE`
（浮かせたタブバーぶんの下パディング）が入っておらず、`calendar`・
`timeline`・`memory`・`stats`・`list`・`mood`・`profile`・
`delete-account`の8画面は`padding: space.lg`のみだった。`href: null`の
画面（memory等）もタブバー自体は重なって表示されるため対象になる。
8画面すべてに`paddingBottom: TAB_BAR_CLEARANCE`を追加し、タイムライン
（無限スクロール）・リスト・気分の記録は実際に最下部までスクロールして
最後の要素がタブバーに隠れていないことを実測で確認した
（`artifacts/035/screens/*-bottom.png`）。

サインインの「中央に大きなロゴ」は、ホーム上部で使っている既存の
`logoMark`画像をそのまま大きく（224×79）出す形にした。新しいフォント・
新しい画像は増やしていない。

ゲストモードでは認証が要る3画面（マイページ編集フォーム・アカウント
削除・投稿作成）はスクリーンショットで確認できていない（範囲外として
`artifacts/035/screens.md`に明記）。詳細は同ファイル参照。

**【追加の自分の誤り】** 撮り直した`font-number-700.png`/`font-number-800.png`
から比較画像`font-700-vs-800.png`を作り直したはずが、最初に再生成した
ものは前回コミット済みの（無効な）画像とSHA256が完全一致していた。調べた
ところ、比較画像を作るスクリプト自体に問題があり（中間クロップ結果を
使い回していたか、実行時に静かに失敗していたと推測。スクリプト自体は
一時ファイルで残っておらず特定しきれていない）、新しいソース画像の内容が
反映されていなかった。作り直したスクリプトでクロップ領域のピクセル差分を
実測（101250バイト中4522バイトが異なる、最大差198）してから合成し直し、
HEADと異なるハッシュになったことを確認してからコミットした。

**【自分の誤り】最初に出した700/800比較画像は、実は比較になっていなかった。**
原因は2つ: (1) `CARD_HEIGHT`修正（108→113）を挟んで2枚を別条件で撮って
いた (2) より重大なことに、`+html.tsx`のフォントパスを検証用に一時的に
書き換えたあと本番用`${baseUrl}`へ戻すタイミングが2枚目の撮影より前で、
戻した時点で開発サーバーでは404になる（既知の詰まり）ため、**2枚目は
実際にはPoppinsが読み込めずシステムフォントのフォールバックで描画されて
いた。**`document.fonts`の`status:"loaded"`だけを見て「読めている」と
判断し、`getComputedStyle`や実際の描画幅までは確認していなかったのが
根本原因。

Aの指摘（「同じ文字の描画幅を700と800で測り、違う値になっていることを
確かめよ」）どおりやり直し、独立したテストページ（`file://`はCORSで
不可と判明。ローカルHTTPサーバー経由）で500/700/800それぞれ単体を検証
（幅111.44/113.75/114.75px、段階的に増加を確認）、同条件でアプリ本体も
撮り直した。**正しく比較すると700と800の差は非常にわずかで、最初に
報告した「800の方が明らかに太い」は誤りだった。**訂正した。

詳細は`artifacts/035/font.md`参照。iPhoneでの`apple-touch-icon`実機確認
項目をAの指示で`artifacts/035/manual-check.md`に追加した。

`fontFamily.ja`（Windowsは`BIZ UDPGothic`を`Yu Gothic UI`より先に）を
`packages/ui`の共有`Text`・生`RNText`の両方に適用し、撮影で効果を確認した
（`artifacts/035/font-step1-*.png`）。Poppins（500・700。latin、各約7.6KB）
を`apps/app/public/fonts/`にself-hostし、`+html.tsx`にインライン
`@font-face`（Google FontsのCDNは参照しない）。`fontFamily.numeric`を
「数字が主役の箱」（記念日カードの72pt・会った日数の94・COMING SOON）
だけに当て、日本語には混植していない。`Text`の`weight`に`medium`(600)を
追加。ボタン・タグライン・COMING SOONの字間はAの指定値（0.04em/0.15em/
0.08em）を反映した。

**72pt数字の700 vs 800は比較用の`@font-face`を一時追加して両方撮影し**
（`artifacts/035/font-number-700.png`・`font-number-800.png`・並べた
`font-700-vs-800.png`）、目視では800の方が明らかに太いことを確認した
うえで、判断はAに委ねた（現在のコードは800のまま）。

COMING SOONの字間を仕様の0.08em（0.64）に変えたところ、Poppinsでの
1行の高さが11→16pxに伸び、`CARD_HEIGHT`(108)で再び4pxはみ出す不具合を
実測で発見・113に再調整して解消した。

開発サーバー固有の詰まり: `+html.tsx`の`${baseUrl}`（"/app"）付きURLは
ローカルの`expo start --web`ではpublicファイルの実際の配信パスと合わず
404になる（`apple-touch-icon.png`も同様で035より前からの既知の状態と
判明）。コードは`${baseUrl}`のまま維持し、確認時だけ一時的にプレフィックス
無しへ書き換えて撮影し、確認後に戻した。詳細は`artifacts/035/font.md`参照。

1〜3節を実装・報告後、Aから「COMING SOONがカードからはみ出している」
（`CELL_HEIGHT`がバッジ1行分しか見込んでいない件）と指摘があり、
`letterSpacing`を1.2→0.8にしつつ`CARD_HEIGHT`も96→108へ実測に基づいて
引き上げ解消した（`letterSpacing`だけでは「今日どうだった？」がまだ
11pxはみ出していた）。

Aから4節（ボケ画像・カード半透明・影）へ進む許可が出たため実装した。
`docs/sample/mockup/signin.jpgのy260〜560px`を切り出し`docs/sample/
bokeh.png`・`packages/ui/assets/bokeh.png`として保存し、`Screen`の
最背面（グラデーションの上）に`opacity0.5`・画面上部0〜420ptで敷いた。
`Screen`1箇所の変更のため全画面に効く（サインイン画面でも視認できることを
確認済み。ホーム画面では記念日カードがこの範囲の大半を覆うため効果が
見えにくい）。カードの半透明・`shadow.card`の濃度は1節時点で実装済みの
ため追加変更なし。

人間の最初の評価は「リッチ感がなく、前よりひどい」だった。Aによれば
これは「部品だけの中間状態を見せた段取りの問題」であり実装の質の問題では
ないとのことで、Aがモックと現状を同じ物差しで実測した視覚仕様（数値）を
`docs/tasks/035-rich-ui.md`に追記し（PR #235）、効く順1〜3だけ進めるよう
指示した。

**1. 記念日カード**: 数字72pt/lineHeight76・アバター86pt（80+光るリング）・
中心間128・「94」だけprimaryのBadgeピル、を仕様の数値どおり実装し実測で
確認した。カード高は実測297pxで仕様の目安「≈260」より大きいが、個々の
数値は全て仕様どおり入れた結果であり、独断で追加削減はしていない。
装飾のスパークルは`docs/sample/透過素材/6sj6V6ve.png`から新規に切り出し
`packages/ui/assets/sparkle.png`として保存した。

**2. デモバナー**: ピル型・`gradients.card`地に変更、文言を19文字に短縮
（Aの判断）、デスクトップでは`layout.maxWidth`列内に収まるよう修正。

**3. 機能パネル**: 列間10・行間12・カード幅76・角丸`radius.input`・
アイコン28pt・ラベル11pt/weight600の数値をそのまま実装。**react-native-web
はcolumnGapとパーセント幅(`width:"25%"`)を併用しても幅を自動で詰め直さず、
4列グリッドが3列に折り返す不具合を実測で発見した**（033で使ったのと同じ
`onLayout`実測px方式で解決）。

その他: `shadow.card`濃度・`secondary`ボタンの枠色（`border`→`primary`）は
Aが`architecture.md`7節に書いた値をそのまま反映。アイコンは線画にしていない
（PNGで線幅を変えられないため。Aの判断どおり）。

`daysTogetherParts`（`apps/app/lib/stats.ts`）を新設し、記念日の表示を
「付き合って」「365」「日目」の3つのTextに分割。関連テストは`testID`で
確認する形に書き換えた。`pnpm run test`（apps/app 222件・apps/api 410件、
全緑）・`pnpm -r type-check`・`pnpm -w eslint .`、全て通過。

Playwright（`playwright-core`。システムのChrome）でモバイル幅（375×812）・
デスクトップ幅（1280×900）のホーム画面を撮影し
`artifacts/035/step1-mobile.png`・`step1-desktop.png`として保存した。
詳細は`artifacts/035/step1-3.md`参照。

`pnpm --filter @futary/app test`（222件緑）・型チェック・lintは4節実装後も
再確認済み。Playwrightで撮影した4節の証跡は`artifacts/035/step4-mobile.png`
（ホーム）・`step4-signin-mobile.png`（サインイン）。詳細は
`artifacts/035/step1-3.md`・`step4.md`参照。

`pnpm --filter @futary/app test`（222件緑）・型チェック・lintは書体仕様
実装後も再確認済み。

**次にやること**: 700 vs 800の判断、および残り12画面・サインイン個別
作り込みへの許可を待つ。

以下、2026-09-05（035より前）の記録。

`me.delete`の網から`post_images`・`reactions`（どちらも`couple_id`ではなく
`post_id`で参照する側）が漏れていた（031のレビューでRが報告）。
`apps/api/test/me.test.ts`に新しいテストを追加し、`sqlite_master`から
アプリの全表を機械的に検出、免除は「表」ではなく「残ってよい行の条件」で
`ALLOWED_TO_REMAIN`に登録した（表ごと免除にすると、自分の`session`が
残っていても鳴らない）。既存の`couple_id`走査テストはそのまま残した
（2つの網が違う主張をしているため。タスク定義5節）。

**タスク定義3節の前提（「テストのDBにはそのペアのデータしか無い」）が
実際には成立しないことを実測で発見した。** このテストファイル内は
D1の状態が`it()`をまたいで共有されており（`@cloudflare/vitest-plugin`は
「isolated per-test storage」を謳うが、実測ではフルスイート実行時に
他のテストが残した`couples`行が新しいテストの開始時点で既に見えた）、
「削除後は0件」ではなく「このテストが増やした分が削除後にちょうど元へ
戻ったか」（`COUNT(*)`の差分）で見る形に設計を直した。詳細は下の論点L91、
`artifacts/032/test-results.md`参照。

**フォールトインジェクションを3件実施し、全て元に戻した**（`git diff`で
`me.ts`の無変更を確認済み）: (1) テストデータのもれ（`moods`のテスト
データ作成を外す）→ 消す前チェックで赤 (2) 削除文のもれ（`user`の
DELETEを外す）→ CASCADEで連動する`session`/`account`が残り赤
(3) 表ごと免除だと検知漏れになる（`user`/`session`/`account`をnull免除に
変更）→ フォールト2の状態のまま**誤って緑になる**ことを確認し、条件免除の
必要性を実証した。
...
**最終更新**: 2026-09-04 / セッションB。**034（pnpm auditの通信リトライ）
完了。PR #228、CIグリーンでmainへsquash merge済み。デプロイ（実行ID
33851415289）も成功し、`pnpm audit`3ステップ（出力専用・ゲート・陳腐化検出）
すべてを含む全ステップが本番デプロイ経路で成功したことを確認した。**

npmのセキュリティ勧告APIへの通信が間欠的にタイムアウトし、CI・デプロイの
両方が2026-09-04当日に5回止まった。Aが起票時「外側に1秒→2秒→4秒のリトライを
足す」と書いたが、Bが出したジョブログから**pnpmは既に内側で2回・計3分
リトライしていた**ことが分かり、外側に足す短い待ちは何も足さないと判明。
AがPR #227（未マージだが訂正内容はそのまま採用）で「pnpm自身の待ちを
延ばす」方針に訂正した。

**3節（先に実測すること）を先に済ませてから実装した。**

1. **`.npmrc`の`fetch-retries`設定は効かないと実測で判明した。**
   到達不能なレジストリ（`http://127.0.0.1:9/`）に対し繰り返し試験したところ、
   `.npmrc`に`fetch-retries=5`を書いても既定の2回のままだった
   （`pnpm audit`だけでなく`pnpm add`でも同様。audit固有ではない）。
   `npm_config_fetch_retries`環境変数も同様に無効だった。**CLIフラグ
   `--fetch-retries=5`のみが実際に反映される**ことを確認し、そちらを採用した
   （ソースコード上は`.npmrc`からも読めるはずに見えたが、実測を優先した。
   `artifacts/034/test-results.md`）。
2. **脆弱性が見つかったときはリトライされないことを、npmの勧告APIと同じ
   形式で固定の脆弱性を返すモックHTTPサーバーを立てて確認した。**
   `--fetch-retries=5`を渡していても、正常な200応答（脆弱性あり）に対する
   リクエストは1回だけ・0.5秒で終了コード1。`Will retry`は一度も出ない。
3. 到達不能なレジストリに対し実際に`--fetch-retries=5`で実行し、
   待ちが既定の約3分10秒から約4分10秒に伸びる（`N retries left`の初期値が
   2から5に変わる）ことをタイムスタンプ付きログで確認した。
...
リトライ設定値はこの1箇所にのみ存在する。ラッパー自身は「リトライすべきか」を
判断していない（`spawnSync`で`pnpm audit`をそのまま呼び、終了コードと出力を
そのまま返すだけ）。区別ロジックを自作していないため、区別を誤って脆弱性を
通す経路も無い。効果が無いと判明した`.npmrc`は削除した。

`pnpm -r test`（apps/app 222件・apps/api 409件、全て緑）・`pnpm -r
type-check`・`pnpm -w eslint .`、全て通過。実レジストリに対しても
`check-audit-ignore-staleness.mjs`・ゲートの両方が正常終了することを
確認済み（詳細は`artifacts/034/test-results.md`）。

**次にやること**: `task/034-audit-retry`をpushしてPRを作成し、Rのレビューを
依頼する。

以下、2026-09-04（034より前）の記録。

**最終更新**: 2026-09-04 / セッションB。**033（複数画像を横にスライド）
完了。PR #224、Rの受け入れを得てmainへsquash merge済み**
（`033: 複数画像を横にスライド (#224)`）。

人間から「Xのように複数画像を右にスライドできるようにしたい」という
要望を受け、Aが031の2つの判断（正方形グリッド・ライトボックスは
ボタンのみ）を覆して起票した。

**0節（先に確かめること）を実測してから進めた。**`ScrollView horizontal
pagingEnabled`がreact-native-web 0.21.2で効くかを、使い捨ての検証画面を
実際にBrowser paneで操作して確認（`artifacts/033/0-node-verification.md`）。
効いたため、ライブラリを入れる判断（`react-native-gesture-handler`等）は
発生しなかった。

0節の直後、Aから「ページ幅＝コンテナ幅のままでは次の画像の端が見えない」
指摘を受け、子要素をコンテナ幅の88%（`ROW_ITEM_WIDTH_RATIO`。仮置きの数値）
に狭める設計へ直した。

**実装中に2つの不具合を発見・修正した**（どちらも実測して気づいた。
「たぶん動く」で進めなかった）:
1. react-native-webの横スクロールの中身は幅が定まらないコンテナになる
   ため、子要素への`width:"88%"`のような相対指定が正しく解決されない
   （561px幅のコンテナで77pxにしかならない不具合を実測で発見）。
   `onLayout`で実測したpx値へ直した。一覧行（`post-images.tsx`）・
   ライトボックス（`image-viewer.tsx`）の両方に同じ形の不具合があった
2. react-native-webの`ScrollView`は内部で`onScroll`しか呼ばず、
   `onMomentumScrollEnd`はWeb版からは一度も呼ばれない（ソースを読んで
   確認）。当初`onMomentumScrollEnd`で設計していたが`onScroll`（都度
   計算し直す。デバウンス不要）へ直した

**security-auditorの監査でHigh・Medium以上はゼロ。**Low5件のうち4件を
その場で修正した（詳細は`artifacts/033/security-audit-raw.md`）。
特に「デスクトップのマウスドラッグがライトボックスの『閉じる』として
誤発火しうる」不具合は、Browser paneでの実機確認でも実際に再現
（テキスト上のドラッグが閉じてしまっていた）・修正後に解消することを
確認した。

`pnpm -r test`（apps/app 222件、全て緑）・`pnpm -r type-check`・
`pnpm -w eslint .`、全て通過。デスクトップ幅・モバイル幅375×812の両方で
一覧行の横送り・ライトボックスのスワイプ/ボタン併用・1枚のときの見え方
維持をBrowser paneで確認済み。

**分からなかったこと**: ライトボックス内でのマウスドラッグ操作が、
Browser paneの自動操作では安定して再現できなかった（テキスト選択に
化けることがある）。マウスホイールでの横送りでは実機確認できたため
`onScroll`ハンドラ自体は機能している。**実機（トラックパッド・実タッチ）
でのスワイプ操作そのものと、「端がどれくらい見えれば続きに気づくか」は
人間の確認が必要**（`artifacts/033/manual-check.md`）。

**Rレビュー（2往復）**: R-1: `onScroll`のコメントが「正しさ」の説明のみで
「毎回計算が入る」ことへの答えになっていなかった → 費用が問題にならない
理由（`Math.round`の結果がドラッグ中の大半で同じ値になり、Reactが
再描画しないこと）に差し替えた。R-1あわせて「幅0の初回フレームが実際に
見えるか」の確認を求められ、`requestAnimationFrame`でタイムスタンプ付きに
実測（幅0は`ms=0`〜`ms=5`のみ、`ms=15`には494pxへ切り替わっており
1フレーム未満）。**Aレビューで「見えない」という判断と、その根拠にした
知覚閾値の一般論（出典なし）を外すよう指摘され、測定事実だけを記録する
形に直した**（「見えない」は「無い」の主張であり、測ったものではない
一般論を根拠にしないという指摘）。R-2: 031で`post-card.tsx`から
`post-images.tsx`へ表示コードを移した際の単一画像の見え方が、031・033の
どちらのレビューでも実機確認されていない点を指摘され、
`manual-check.md`に明示の確認項目として追加した。

**CIが`pnpm audit`のnpmレジストリタイムアウトで、031・033を通じて3回目の
同じ落ち方をした**（`error (23)`のタイムアウト。詳細は`docs/worklog.md`）。
4回目の実行で通過しマージした。Aから「3回目なので、次に同じ落ち方をしたら
再実行ではなく通信リトライを組み込む対応を別タスクとして起票する」との
方針を受けている（`continue-on-error`や無視リストでゲートを緩める対応は
採らない）。

**次にやること**: 次のタスクの指示を待つ。

以下、2026-09-04（033より前）の記録。
...
Rから追加の提案（必須ではない）を受け対応済み: 認可テストの番人
（`authorization.test.ts`「許可リストに無い手続きは3基底のいずれかを
経由している」）が手続き数の下限（`toBeGreaterThanOrEqual`）だけを
見ており、security-auditorが指摘した「26のまま更新し忘れても`>=`で
緑のまま通る」という穴自体は塞がっていなかった。#180で走査対象一覧に
施したのと同じ形（数ではなく名前を完全一致で固定する）に直し、
実際に1つ名前を変えて赤くなることを確認（fault injection）してから
戻した。Rが確認した「`me.test.ts`のmoods機械的検出が両方向で正しく
閉じている」点は変更不要。
...
`created_by`は引き続きIDを返さないが`createdByName`（表示名）を出すように
なり、`note`（自由記述・0〜200文字）を追加した。権限（ペアで共有）は
変えず、「相手が入れたwishをもう1人が編集・削除できる」ことをテストで
固定した（確かめずに現状維持と書かない、というAの指摘）。`wish.update`
（`{id, title?, note?}`。渡されなかった項目は変えない。`created_by`は
更新しない）を新設した。

**重要な発見**: oRPCの`.input()`（Zod）スキーマのバリデーション失敗は、
契約で`INVALID_INPUT`を宣言していても`@orpc/server`の実装上**常に
`BAD_REQUEST`になる**（ソースで確認済み）。027の`titleSchema`もこの問題を
抱えていたが、テストがエラーコード不問だったため気づかれなかった。028の
タスク定義がエラーコードを明示していたため、契約のZodスキーマから長さ
チェックを外し、procedures側で明示的に検証してINVALID_INPUTを返す形に
統一した。

**security-auditorの監査でHigh以上はゼロ。**Low 4件、全て対応済み
（title境界値テストの欠落・定数名の衝突懸念・検証順序・
security-requirements.md 6節の記述漏れ）。詳細は`docs/security-report.md`
「028」・`artifacts/028/security-audit-raw.md`参照。

`pnpm -w test`（apps/app 203件・apps/api 377件・packages/db 21件）・
`pnpm -r type-check`・`eslint .`全て通過。Bがブラウザで未認証（デモ）
...
**Rレビュー（R-1、1往復）**: 新設した「`couple_id`列を持つ全表がme.delete後に
0件になる」テストが、**将来「次の表」が増えたときに空振りする**穴を指摘された
（検出はしても、その表へ行を作らないため削除後0件チェックがもともと0件で
常に通ってしまい、削除文の足し忘れを検知できない）。削除前に各表へ
テストデータが実在することを要求するチェックを追加して解消し、
「受け入れます」を得てマージした（検出条件もバッククォート必須から
`/couple_id/`へ緩め、緩めても誤検知が増えないことをRが実測で確認済み）。
...
**続けて人間から新しい機能要望2件（アカウント削除・招待コード再発行）が
出て、Aがタスク024・025を起票した。**

**025（招待コード再発行）**: PR #184としてRレビュー1往復（R-1: 満員時の
FORBIDDENが「もう一度お試しください」に丸められる問題）を経てマージ完了
（2026-09-01）。

**024（アカウント削除）**: 着手前にRの先読みを受け、削除順序に冪等性の
矛盾が見つかりAが訂正（`couple_members`を後ろへ回す。
`resolveCoupleContext`には触らない）。R2は行から鍵を集めず接頭辞で消す形に
変更（プロフィール画像2人分を含む）。実装完了後にsecurity-auditorを実行し
（**High以上ゼロ**。タスク定義の完了条件を満たす）、Medium 2件・Low 6件の
うち実装レベルで直せるもの全て対応済み（詳細は`docs/security-report.md`
2026-09-01「024 アカウント削除」節）。**特に大きかった指摘**: 削除の
手順1〜6を個別の`run()`ではなく`db.batch()`1本にまとめる変更1つで、
「並行書き込みで本文・写真が回収不能な孤児になる」Mediumと、当初
「残る」と受け入れていた孤児`couples`行の両方が同時に解消された。

**A判断待ちだったMedium 2件、Aが判断（PR #186・#187）を出し、Bが実装完了
（2026-09-01）**:
1. **招待コードのレート制限キーをuser_idからGoogleアカウント
   （`account.account_id`）の塩付きハッシュへ変更。**`user`へのFKを外した
   （`apps/api/src/lib/account-hash.ts`）。副作用として`invite_failures`は
   `me.delete`から削除する手順自体が不要になった（時間窓1時間で自然に
   切れる）。マイグレーションは0014（`user_id`列を落とす）・0015
   （`account_hash`列をNOT NULLで足す。既存行はNOT NULL制約を満たせない
   ため一度空にする。実害はデプロイ直後にレート制限カウントが一度だけ
   0に戻ることのみ）の2本に分けた（drizzle-kitのリネーム検出プロンプトが
   非対話環境で使えないため。1本にまとめようとすると対話プロンプトが
   必須になり生成できなかった）
2. **`me.delete`にサーバ側の再認証を追加。**直近5分以内のサインイン
   （`session.createdAt`。Better Auth本体のソースで固定値であることを
   確認済み）が無ければ`REAUTH_REQUIRED`。画面（`delete-account.tsx`）は
   `me.get().sessionIsFresh`を見て、確認フロー（2段階の確認）に入る前に
   「もう一度ログインしてください」に落とす（Aの追加決定・PR #187:
   確認をやり切った後に拒むと「押したのに何も起きなかった」になるため）。
   サーバ側の`REAUTH_REQUIRED`は、確認中に5分を跨いだ場合の最終防御として
   別途残す（T5と同じ理由）

**マージの経緯**: 上記2件をBが実装している間に、Aが同じ決定を
`docs/security-requirements.md`・`docs/tasks/024-account-deletion.md`へ
先に書いてmainへマージ済みだった（PR #186・#187）。`feature/024-account-
deletion`ブランチをmainへマージした際、`security-requirements.md`の
レート制限キー節でコンフリクトが発生（同じ決定をAとBが別々の文面で
書いていた）。**Aの文面を残し、実装ファイルへのポインタだけ追記**した
（`conventions.md`「判断は、それが上書きする記述と同じ場所に書く」。
2箇所に同じ決定を書いていた重複も解消。詳細はマージコミット参照）。

**Bが実装中に見つけた、24タスクファイルの訂正待ち1件をL90として起票**
（`docs/tasks/024-account-deletion.md`「2. まっさらな状態から登録し直せる」
節の「`invite_failures`は`user`より先に消す」が、PR #186より前の記述の
まま取り残されている。実装は新しい決定〈消さない・FK無し〉で進めた）。

`pnpm -w test`・型チェックすべて通過。ファウルトインジェクション
（一時的に無効化して落ちることを確認）で新規テストの有効性を確認済み。
`feature/024-account-deletion`ブランチとしてRへレビュー依頼予定。
`artifacts/024/manual-check.md`に人間の実機確認項目（**本番で試す前に
ローカルで途中で止めて再実行を確認すること**。再認証まわりの実機確認は
追記が必要）を列挙済み。

## ゲストではじめる→/composeで読み込み中のまま止まる不具合（発見・修正）

**症状**: サインイン画面で「ゲストではじめる」を押すと、URLが`/compose`に
飛び、「読み込み中…」の表示のまま永久に戻ってこない。

**調査の過程**: 最初は(1) `(auth)`・`(onboarding)`グループに`_layout.tsx`が
無くルーティングの警告が出ている点、(2) 識別変化時に`<Stack>`自体を
早期returnで消していた構造、(3) `enterGuestMode`内の`router.replace()`
呼び出しのタイミング、の3つを疑い、それぞれ実測で確認・修正を試みたが、
いずれも単独では不具合を再現しなくすることができなかった（Metro開発
サーバーのHMR用WebSocketが不安定でノイズになったため、`wrangler dev`が
配信する本番相当の静的ビルドで検証をやり直した）。

**真因**: PR #174で入れた識別変化エフェクトの`queryClient.clear()`
（「正しさのためではなく容量のため」と位置づけていたもの）が、
`couple.get`が新しいviewerKeyで発火した直後というピンポイントの
タイミングで走ると、発火したばかりの問い合わせをキャッシュごと消してしまい、
`retry:false`のため二度と再試行されず`fetchStatus:"fetching"`のまま
永久に止まっていた。`queryClient.getQueryCache().getAll()`が空になる
ことまで実測して確認した。

**対処**: `queryClient.clear()`の呼び出しを削除した（正しさは
`viewerKey`をqueryKeyに含めることで既に担保されており、この呼び出しは
無くても正しさは壊れない）。あわせて、識別変化のたびに`<Stack>`を
早期returnで消していた構造もやめ、常にマウントしたままローディング・
未解決状態をオーバーレイで重ねる形にした（実測でStack.Protectedの
guardが自然に既定画面へ導くことを確認）。`(auth)`・`(onboarding)`への
...
本番デプロイ後、人間が「Googleログイン→ログアウト→ゲストモード→
デモを見る、としたら自分の実アカウントに入ってしまった」と報告。
調査の結果、**2つの独立した問題**があった。

1. **`resolveCallbackURL()`のリダイレクト先バグ**（軽微・実害なし）:
   015でランディングページを`/`に分離した際の直し忘れで、ログイン後に
   `/app/`ではなく`/`（ランディングページ）へ戻っていた。「デモを見る」を
   押すと、実はGoogleログインが（既存の同意済みアカウントで一瞬で）
   完了済みだったため、自分のホーム画面が出て混乱した。**セキュリティ上の
   実害は無い**（動画のフレーム解析で`accounts.google.com`への正規の
   遷移を確認済み）
2. **T9（新設）: クライアント側キャッシュ経由の他人データ開示**（重大・
   共有端末では実質的な情報漏洩）: `couple.get`等5つの問い合わせが
   `coupleId`を引数に取らない設計（正しい設計）の副作用として、
   TanStack Queryのキャッシュキーが「誰が呼んだか」を区別しない。
   リロード無しで本物のログイン⇄ゲスト⇄未認証を切り替えると、直前の
   別人のキャッシュが新しい識別の画面に一瞬そのまま出る。**人間が
   報告した症状は、実際にはこちらで起きていた可能性が高い**（Rレビュー
   指摘）。B最初の対策（`useEffect`での`queryClient.clear()`）は
...
Privateリポジトリでは使えないため）・Cloudflare APIトークン・Worker用
シークレット（`BETTER_AUTH_SECRET`等）・Google OAuth本番リダイレクトURI・
R2用APIトークン・R2 CORS設定を順に完了させ、PR #172をマージ・
Required reviewers承認・デプロイ実行まで進めた。デプロイ直後に2件の設定ミスが
見つかったがその場で修正した（詳細は`artifacts/016/test-results.md`
「本番デプロイ後の実機確認」）: (1) Better Auth初期化のfail-closed assertionで
500（`BETTER_AUTH_SECRET`/`BETTER_AUTH_URL`の設定不備）→修正して403
（デモ未整備による正しい拒否）に (2) Google側`redirect_uri_mismatch`→
承認済みリダイレクトURI未登録が原因、登録して解消。

続けてBが`pnpm --filter @futary/db run seed:remote`を実行（本人の明示的な
確認を得てから。193クエリ・662行書き込み、R2へ画像6件アップロード、全て成功）。
本番URL（`https://futary-api.sarada7739.workers.dev`）でホーム・タイムライン・
カレンダー・思い出・統計の5画面全てが正しく動作すること、R2署名付きURL経由の
画像表示が機能していることをBがブラウザで直接確認済み。

**未確認で残っているもの**（`docs/tasks/016-release.md`完了条件参照）:
- ログインの最後まで（Google認証情報の入力から完了まで）は人間が未実施
  （Googleの正式なログイン画面までは遷移確認済み）
- 画像アップロードを伴う投稿作成（PUT経由。GET表示は確認済みだがPUTは未確認）
- R2バケットが非公開であることの確認（ダッシュボードでの目視。まだ未報告）
- `main`ブランチ保護の設定
- iPhone Safariでの実機確認（022の残り）
- 本番のスクリーンショットを画像ファイルとして保存（モバイルビューポート
  エミュレーション下でBのクリック操作がタイムアウトする現象が発生し未達成。
  デスクトップ幅では目視確認済み）

015はPR #170、Rの受け入れを得てmainへsquash merge済み（`76ef4dd`）。
014は完全に閉じている（PR #165。`7aef87d`）。**公開ドメイン（論点L1）は
...
| L90 | `docs/tasks/024-account-deletion.md`「2. まっさらな状態から登録し直せる」節の「`invite_failures`は`user`を参照している。`user`より先に消す」が古いまま残っている。同じファイル内の「1. 削除の順序」節（`invite_failures`は消さない）・完了条件（`invite_failures`のキーがGoogleアカウントのハッシュ）・テスト項目（`invite_failures`が`user`を参照していない）はすべて新しい決定（PR #186）に揃っており、この1行だけがPR #186より前の記述のまま取り残されている（Bが#186・#187とのマージ中に発見） | 読み手が新旧どちらが正か迷う。実装（`apps/api/src/procedures/me.ts`）は新しい決定どおり（`invite_failures`を消さない・FK無し）で実装済み | Aが当該行を削除・訂正。実装は正しい方で進めた（conventions.md「Bが設計ドキュメントの誤りを見つけたとき」） |
| L91 | **`apps/api/test/me.test.ts`は、D1の状態が同じファイル内の`it()`をまたいで共有されている。**`@cloudflare/vitest-plugin`は「isolated per-test storage」を謳うが、032の新テスト実装中に実測したところ、フルスイート実行時はテストの先頭（自分のデータを作る前）の時点で他のテストが残した`couples`行が既に1件見えた（単体実行では見えない。`SELECT COUNT(*) FROM couples`で確認） | 「このテストのDBにはこのテストのデータしか無い」という前提で書かれたテスト（032が最初に書いた素朴な実装がまさにこれで、フルスイートでのみ失敗した）は、実行順序次第で誤って赤くなりうる。`me.test.ts`以外の大きなテストファイルにも同じ前提が潜んでいる可能性がある | 032では「削除後0件」ではなく「このテストが増やした分の差分」で見る形に直して回避した（`artifacts/032/test-results.md`）。原因（プラグインの既知の制約か設定か）は未調査。他のテストファイルで同種の前提が疑わしくなったら、その都度実測して確かめる |
