...
送った直後に出た3件の論点（L81・L82・L83）、すべて解決・反映済み。**
L83（FABの影が正方形に見える不具合）は`fix/fab-shadow-square`で修正し
mainへマージ済み（PR #130）。L81（パネルの見た目がモックアップと違う形）・
L82（人間が置いた新規素材`UlQMVnAB.png`の扱い）はAが判断し
（PR #131）、パネルをアイコングリッドに寄せる・新規素材は見本として扱い
6個のアイコンを新しく描き起こす、と決まった（人間が「説明文がある方が
分かりやすい」と答えればカードのままでもよい設計）。**次はパネルアイコン
6個の描き起こしに着手する（回答を待たずに着手してよいとAの指示）**）。
...
| ~~L81~~ | ~~020の機能パネルの見た目が、人間が出したモックアップ（円いアイコン＋短いラベルの4+3グリッド）と違う形（タイトル＋説明文を書いたカードを縦に並べる形）になっている~~ → **解決・設計反映済み（PR #131）。Aが「グリッドに寄せる」と判断した。**理由: (1) ホームの役割は「入口を選ぶ」ことで、説明文つきカードを8枚縦に積むとスクロールが要り一目で見渡せない (2) 4列なら8枚が2行に収まる (3) 020の受け入れは「人間が自分の絵と見比べる」形にすると決めてあり、形が違うと見比べにならない。**ただし人間が「説明文がある方が分かりやすい」と答えたらカードのままでよいとも書いた**（Bが既に問いを出していたため）。アイコンはどちらの形でも要るため、回答を待たずに着手してよい | | 解決済み・設計反映済み（PR #131） |
