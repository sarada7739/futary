...
のコード側実装完了。Rレビュー依頼前。**スキーマ（`is_shared`列・CHECK）・
契約（`isShared`/`canEdit`）・`event.update`/`event.delete`の認可WHERE句・
`event.list`の`canEdit`計算・UI（「ふたりの予定にする」・編集不可の表示・
自分を締め出す更新を画面で先に止める）まで実装した。**security-auditorで
High 1件（マイグレーションのSELECT不具合。修正・実機確認済み）・Medium 3件・
Low 3件を検出し、すべて対応済み**（`docs/security-report.md`参照）。
実装中にAへ2回設計判断をエスカレーションした: (1) 021の範囲を1・2だけに
絞り3（翌日「会った日」に変わる）は公開後へ回す人間の判断 (2) `kind`の
変更で共有していたイベントから相手（または自分）を締め出せる経路
（security-auditor発見。「自分を締め出す更新を拒む」形で解決）。
apps/api 272件・apps/app 105件すべて緑。**次はRレビューを依頼する**）。
