...
**最終更新**: 2026-08-29 / セッションB（PR #21・#27マージ完了。L26・L27解決済み）
...
ブランチ命名規約の整理（`fix/`新設・二重発火防止の規約化。PR #24）、
引用先/引用元の食い違いの判定基準（PR #21）、M2着手前の2論点（旧L26・旧L27。
PR #27）もすべて反映済み。
...
3. M1受け入れ後、M2（006〜009: 投稿・画像・タイムライン・リアクション）に着手する。
   007（画像圧縮）でapps/appにVitest + React Native Testing Libraryを導入し、
   `Button`の二重発火防止も同タスクで実装する方針が決定済み
   （`docs/tasks/007-image-upload.md`参照）
