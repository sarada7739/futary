...
- **人間に頼むこと**: (1) デプロイを承認する（main の CI は緑）。(2) 実機で無料枠（30 枚）で止まる・一覧の一番下の使用量のカード・警告の ×・マイページの「プラン: 無料」を 1 度だけ見る。(3) **人間のペアを paid にする許可**（本番の D1 に書く。B が実行するか人間が実行）。以後 paid のまま
- 人間の指示 2 件（B へ直接）: 使用量のカードは一覧の一番下・警告のカードは × で消せる（この起動の間。残りが変わればまた出す）。A が 3節に足した。**警告の × は人間の実機確認 OK（2026-09-14。A 経由）。**R の記録（置く）: 消した状態の復元が useEffect なので枠が読めた直後の 1 フレームだけ警告が見えてから消える。人間が気にしたら useLayoutEffect に
