...
**最終更新**: 2026-08-29 / セッションB（M1実機確認・バグ2件発見修正。Cookie属性等3項目は未確認）
...
1. `fix/oauth-callback-and-double-submit`（実機確認で発見したバグ2件の修正）を
   レビュー・マージする
2. 残り3項目の実機確認を行う: Cookie属性実地確認（`HttpOnly`/`Secure`/`SameSite=Lax`）・
   リロード後のログイン状態維持・ログアウト→サインイン画面へ戻るUI導線
   （`docs/tasks/003-auth-google.md` の「保留」節参照。ログイン成功・D1レコード
   作成・004のオンボーディング導線は確認済み）
3. 上記が済んだら 003・004 を「完了タスク」に移動する。005は認可の実装・テスト・
...
| L26 | `packages/ui` の `Button`（`react-native-web` の `Pressable`）が、環境によっては1クリックで `onPress` を2回発火させる。003の実機確認で発見。認証フロー（`sign-in.tsx`）にはピンポイントでガードを追加して対応したが、`Button`/`Pressable` 自体の一般修正はしていない | 同じ`Button`は004のオンボーディング（`invite.issue`等）でも使われている。今はDB制約（UNIQUE、used_at）が二重送信の実害を受け止めているだけで、「画面ごとにガードを書く」のは005で潰した「手続きごとに認可を書く」と同じ構造（Rレビュー指摘）。M2で投稿等の書き込みUIが増える前に`Button`側で共通対応する方が筋が通る | **M2着手前が妥当**（Rの見立て。最終判断はA） |
