...
**最終更新**: 2026-08-31 / セッションB（futary-f2）。**014（ゲスト・デモ体験）
完了。PR #165をRの受け入れを得てmainへマージ済み（`7aef87d`）。** Rの
1回目レビューで差し戻し2件（R-1・R-2）・軽微1件（R-3）を受け、すべて
対応して2回目のレビューで受け入れられた。最も重かったR-1（デモペアを
解決できないゲストがどのStack.Protectedのguardにも入れず空白画面から
戻れなくなる）は、Aが`architecture.md` 3節「ルーティングと画面遷移」
（当時7節。のちにナビゲーション関連の規則がまとめて移動）に「ルーティングは、
必ずどれか1つが真になる」を新設する判断を出し、それに従って
`resolveRootRoute`（純関数）へ切り出し・状態組み合わせの総当たりテスト
（`apps/app/test/root-route.test.ts`）まで実施した（詳細は
`artifacts/014/test-results.md`「Rレビュー1回目の差し戻し」節）。
022・023もマージ・リモートD1適用済みで完全に閉じている
（`65b1f11`・`ceb14ae`。詳細は下の記録を参照）。人間は就寝中のまま。
A経由で「014→015→016のデプロイ前まで、人間の確認を挟まず止まらずに
進めてほしい（実機確認はまとめて後で）」との指示を受けたため、Rの受け入れは
これまでどおり通しつつ、人間への確認依頼は出さずに進めている。
**次は015（ランディングページ）へ着手する。**
