...
**最終更新**: 2026-09-16 / セッションB。**056（PR #398）: R 差し戻し。必須修正 1 は 0節 #3 の形に由来するので A の判断待ち: 框の中でデモの読み込みが失敗（`demoFailed`）すると、利用者が何も押していないのに親ページ（LP）が `/app/` へ飛ぶ。R の案: 親を飛ばすのは `exitGuestMode`（利用者の操作）だけにし、`demoFailed` は框の中に「デモを読み込めませんでした」を出して止める。**
