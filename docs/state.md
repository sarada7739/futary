...
**最終更新**: 2026-08-30 / セッションB（**018・fix/meetup-daysとも
...
| L73 | 018（カレンダーの改善。設定者・時間・会った日の一意化。付随のfix/meetup-days含む）は**コード側完了・レビュー完了・リモートD1へのマイグレーション適用も完了**したが、画面が認証必須のためB（自動化）は実機確認ができない | 設定者名・時間入力・会った日の上書きが実際にどう見えるか、統計カードの「会った日数：N日」表示が違和感を生まないかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/018/manual-check.md`参照（項目7はRレビューで修正済み。画面表示に合わせてある） |
