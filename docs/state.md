...
**最終更新**: 2026-09-16 / セッションA。**056: R が #398 を差し戻し（必須修正 1）。A の判断: 親を飛ばすのは `exitGuestMode` の 1 箇所だけ（`showAuth` で飛ばさない）・框の中ではサインイン画面を出さず失敗は「デモを読み込めませんでした」+「アプリを開く」の 1 行・043 のシートは框の中で出さず `releaseSeen` も書かない。0節 #3・#3b・#3c・T4b・T7。B の手番。**
