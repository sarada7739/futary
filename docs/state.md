...
**最終更新**: 2026-08-30 / セッションB（**018マージ済み。付随のfix/meetup-days
...
| L73 | 018（カレンダーの改善。設定者・時間・会った日の一意化）はコード側完了だが、画面が認証必須のためB（自動化）は実機確認ができない。リモートD1への`0008_event_time_and_meetup_unique.sql`適用もまだ（0007の教訓でレビュー・マージ後に行う方針） | 設定者名・時間入力・会った日の上書きが実際にどう見えるか、`meetupCount`の意味変化がホームの統計カードで違和感を生まないかが未確認 | Rレビュー→マージ後、人間が実機で確認する。確認項目は`artifacts/018/manual-check.md`参照。リモートD1マイグレーション適用の手順も同ファイルに記載 |
