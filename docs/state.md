...
- 選択モードに「保存（N 枚）」（`canShare` が真のときだけ。PC には出ない）。タイムライン・ゲストのアルバムにも「選択」を出し、バーは「保存」だけ。21 枚目は選ばせず 1 行。取得に失敗した枚は飛ばして残りで共有シート
...
- **A の判断が要るかもしれない 1 点**: 共有シートのある環境では削除・カバーの選択にも 20 枚の上限が掛かる（選択モードが 1 つのため。1節のとおり）。iPhone で 21 枚以上をまとめて消す場面があるなら A が決める
