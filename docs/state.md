...
**最終更新**: 2026-08-30 / セッションB（**010（カレンダーAPI）、PR #86
（`task/010-calendar-api`）でRレビュー往復1回目対応済み。** `events`テーブル・
`lib/date.ts`（JST集約）・`event.list/create/update/delete`を実装し、
繰り返し記念日の射影（年またぎ・400日上限・3暦年に触れる窓・うるう年02-29の
両方向）をテストで証明した。**Rが`lib/date.ts`内の矛盾を発見**（`yearsBefore`が
`projectMonthDay`の02-29規則と食い違い`2023-03-01`を返していた）。
`projectMonthDay`を直接呼ぶ形に修正して解消（必須修正、対応済み）。
`monthsBefore`の月末繰り上がりの扱いは仕様未決定のためL61として起票し、
Aへ判断を依頼した（013着手前に必要。現状の挙動のままテストで固定済み）。
テスト175→178件。詳細は`artifacts/010/review.md`参照。
005の認可テストも`event`系4手続きを追加した上で緑。**010単体は
security-requirements.md 10節1の必須監査対象に該当しない**ため、単体監査は
せずM3の他タスクとまとめる方針（ユーザー確認済み）。
...
| M3 | **017** → 010〜013 | **画像の全画面表示** → カレンダー・統計・思い出し | 着手中（017完了。010実装済み・Rレビュー待ち） |
...
- 010-calendar-api（PR #86〈ブランチ `task/010-calendar-api`〉。Rレビュー往復1回目の
  必須修正（`yearsBefore`の矛盾）に対応済み、再レビュー待ち。`monthsBefore`の月末繰り上がりは
  L61としてA判断待ち。`artifacts/010/test-results.md`・`artifacts/010/review.md`参照。
...
7. ~~次はM3本体、010（カレンダーAPI）に着手する~~ → **実装完了。Rレビュー待ち。**
   `events`テーブル・`lib/date.ts`・`event.list/create/update/delete`を実装し、
   繰り返し記念日の射影（年またぎ・400日上限・3暦年に触れる窓・うるう年）を
   テストで証明した
8. ~~`docs/sample/風景/`の用途~~ → **解決**（L55・L56）。014 のデモ投稿の写真として
   5枚を使う（`eHaCqEMx.jpg` は架空のブランド看板が写るため除外）。
   **014 の着手前に L55〈デモペアの構成〉の人間の判断が要る**
9. **010（PR `task/010-calendar-api`）のRレビューを待つ。** 受け入れられたら
   `main`へマージし、011（カレンダー画面）に進む
...
| L61 | `apps/api/src/lib/date.ts` の `monthsBefore` は、日が月末を超える場合（例: `2026-03-31` の1ヶ月前）に JS の `Date` の自動繰り上げに任せており、`2026-03-03` を返す（月末〈2/28〉に寄せない）。013（`memory.get` の「ちょうど1ヶ月前の投稿」）で実際に使う関数で、**利用者からは誤りに見えうる**（010 Rレビュー指摘） | 月末に寄せる（3/31→2/28）方が利用者の直感に近いはずだが、`architecture.md` にはこの規則が無く、010の02-29の規則（射影専用。`projectMonthDay`）とは別の問題。**Bの裁量では決められない仕様判断** | **013 着手前。** 決まるまでは現状の挙動のまま固定し、`date.test.ts` にテストで固定済み（`monthsBefore("2026-03-31", 1) === "2026-03-03"`）。**Aの判断待ち** |
