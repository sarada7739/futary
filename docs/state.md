...
**最終更新**: 2026-08-30 / セッションA（**M2 を人間が受け入れた。** L4 もハート1種で決定。
次は M3: L59 の `fix/` → 017 → 010〜013）
...
1. ~~M2 の受け入れ判定を人間に依頼する~~ → **完了（2026-08-30）。受け入れられた。**
   L4 もハート1種のままで決定
2. **`fix/` で L59（画面の最大幅）を入れる。** `Screen` が既定で `layout.maxWidth = 640`
   を適用し、外す画面だけが明示的に外す（`architecture.md` 7節「レイアウト」）。
   **呼び出し側に書かせない**
3. **017（画像の全画面表示）に着手する。** `docs/tasks/017-image-lightbox.md`。
   `Modal` を使い expo-router のモーダルルートにしない、`post.list` の署名付きURLを
   そのまま使う、新しい API 手続きを作らない、閉じる導線3つ、縦長写真の確認
...
7. 017 のあと: 010〜013（カレンダー・統計・思い出し）
