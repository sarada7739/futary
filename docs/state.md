...
**最終更新**: 2026-09-06 / セッションB。**038（T9の検査をライブラリから
引く形にする）実装完了。**`@tanstack/react-query@5.102.3`の公開面を
テスト実行時に読む形へ作り直し、Rが実測した12通りの逃げ道に対応した。
テスト（apps/api 457件・apps/app 283件）・型チェック・lint全て通過。
PR作成・Aへ報告予定。まだmainへはマージされていない。
