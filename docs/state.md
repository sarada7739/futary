...
| L59 | **画面の最大幅に制約が無い。** PC 幅（1280px）で投稿カードが **1248px（97.5%）**まで伸びる。R が `artifacts/002/desktop-home.png` を実測して確認。A も `packages/ui` / `apps/app` に `maxWidth` が**1箇所も無い**ことを確認済みで、**008 のタイムラインにもそのまま効いている**（過去の記録ではなく現行の不具合） | 写真と文章を読む画面で 1248px 幅は読みにくい。転職で読まれる想定のプロダクトとして、PC で開いた第一印象に直接効く | **`fix/` で扱える小ささ**（コンテナに `maxWidth` を1つ）。017 の前後どちらでもよい。**R が 002 のレビューで指摘していたが記録されず30タスク消えていた**（L60 の実例） |
