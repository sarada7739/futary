...
| L26 | `packages/ui` の `Button`（`react-native-web` の `Pressable`）が、環境によっては1クリックで `onPress` を2回発火させる。003の実機確認で発見。認証フロー（`sign-in.tsx`）にはピンポイントでガードを追加して対応したが、`Button`/`Pressable` 自体の一般修正はしていない | `invite.issue` 等、他の書き込み系ボタンでも同様の二重送信が起きうる。招待コード発行の二重実行自体は実害が小さい（再発行で前のコードを無効化するだけ）が、006以降で「一度きりであるべき」操作（投稿の二重送信等）が増えると影響が大きくなる | 006以降、書き込み系ボタンを追加する際に都度検討。頻発するなら`Button`コンポーネント自体に再入防止を組み込むことを検討する |
