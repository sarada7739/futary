...
PR #165、Rの1回目レビューで差し戻し2件（R-1・R-2）・軽微1件（R-3）を受け、
すべて対応して再レビュー依頼済み。** 最も重かったR-1（デモペアを解決できない
ゲストがどのStack.Protectedのguardにも入れず空白画面から戻れなくなる）は
Aが`architecture.md`（当時7節。のちに3節「ルーティングと画面遷移」へ
まとめて移動）に「ルーティングは、必ずどれか1つが真になる」を
新設する判断を出し、それに従って`resolveRootRoute`（純関数）へ切り出し・
状態組み合わせの網羅テストまで実施した（詳細は`artifacts/014/test-results.md`
「Rレビュー1回目の差し戻し」節）。022・023はマージ・リモートD1適用済みで
完全に閉じている（`65b1f11`・`ceb14ae`。詳細は下の記録を参照）。人間は
就寝中のまま。A経由で「014→015→016のデプロイ前まで、人間の確認を挟まず
止まらずに進めてほしい（実機確認はまとめて後で）」との指示を受けたため、
Rの受け入れは
これまでどおり通しつつ、人間への確認依頼は出さずに進めている。

## 014の実装内容（詳細。Rレビュー依頼前）
