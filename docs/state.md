...
**最終更新**: 2026-08-30 / セッションB（**018を人間が実機確認し「基本的に
確認OK」。続けて新規要望が出た**）。人間の要望: (1) 「予定」(kind='plan')
は個人の予定なので設定者(created_by)以外は削除・編集できないようにする
(2) ただし予定に「２人の予定」チェックボックスを付け、チェックされていれば
設定者以外も編集可能にする (3) チェックされた予定は翌日に自動で
「会った日」(kind='meetup')に変換される。**認可（誰が書き込めるか）に
関わる仕様変更のため、Bの独断で実装せずAへ設計判断とタスク定義を依頼中**
（返答待ち。自動変換の実装方式〈Cron Triggers等の新規インフラか、読み取り時
変換か〉・018の会った日1日1件制約との衝突・シード〈014〉への影響など、
B目線の論点も合わせて伝達済み）。
...
| M4 | **018** → 014〜016 | **カレンダーの改善** → ゲストデモ・LP・仕上げと公開 | 未着手 |
...
| ~~L73~~ | ~~018（カレンダーの改善。設定者・時間・会った日の一意化。付随のfix/meetup-days含む）の人間の実機確認~~ → **解決（2026-08-30）。** 人間が実機で確認し「基本的に確認OK」との回答。`artifacts/018/manual-check.md`への結果追記は未実施（口頭の確認のみ）。**確認と同時に新規要望が出た**（予定の編集権限・「２人の予定」自動変換。L74参照） | | 解決済み（2026-08-30） |
| L74 | 人間から新規要望3点: (1) 「予定」(kind='plan')は個人の予定なので設定者(created_by)以外は削除・編集できないようにする (2) 予定に「２人の予定」チェックボックスを付け、チェックされていれば設定者以外も編集可能にする (3) チェックされた予定は翌日に自動で「会った日」(kind='meetup')に変換される | 認可（誰が書き込めるか）に関わる仕様変更。自動変換の実装方式（Cron Triggers等の新規インフラか、読み取り時変換か）・018の会った日1日1件制約との衝突（変換先の日付に既にmeetupがある場合の扱い）・シード（014）への影響など、設計判断が複数必要 | **Aへ設計判断とタスク定義を依頼中**（返答待ち） |
