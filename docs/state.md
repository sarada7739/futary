...
| L63 | **日付計算が`apps/app/lib/calendar.ts`（`todayJst`/`daysInMonth`/`normalizeYearMonth`/`formatDate`）と`apps/api/src/lib/date.ts`の2箇所に分かれている**（011。`apps/app`が`apps/api`に依存しない構成のため生じた重複。011 Rレビュー R-36） | 010のタスク定義は「日付計算はここに集約する」と明記していたが、実際には2箇所になった。現時点で挙動の食い違いは無いが、**012（`daysTogether`）・013（1ヶ月前・1年前）は両側とも日付計算を使う。**増えてから寄せる方が高くつく。`packages/contract`は既に両方から参照されており置き場所の候補になる | **012着手前にAの判断を仰ぐ**（共有パッケージへ寄せるか、重複を許容して同期の規則を決めるか。Rの指名） |
