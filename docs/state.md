...
**最終更新**: 2026-08-30 / セッションB（**013（思い出し）、Rの受け入れを
得てmainへマージ済み（PR #100）。ブランチも削除済み。この時点でM3
（017→010〜013）が実装として完了した。** `memory.get`（探索順4段:
...
（現在、実装が進行中のタスクは無い。次はM3の人間による受け入れ判定を待つ）
