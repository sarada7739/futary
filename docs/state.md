...
**最終更新**: 2026-08-30 / セッションB（**020（ホームの再構成）実装完了。
Rレビュー依頼前**）。ホームを投稿一覧から「状態を見て入口を選ぶ画面」に
...
（`artifacts/020/test-results.md`）。**次はRへレビュー依頼。**
B目線で気づいた点3件をAへ報告予定: `requirements.md`5節の「検索」行が
古くなった・タスク定義の「モックアップの7枚」とパネル表（8行）が
食い違う・`architecture.md`7節のナビゲーション節が本タスクを反映して
いない。ホーム・タイムライン等の各画面は認証必須のためB（自動化）は
実機確認ができず、`artifacts/020/manual-check.md`に確認項目を列挙した。
...
| L79 | 019（記念日とプロフィールの設定）はコード側完了・レビュー完了・リモートD1へのマイグレーション適用も完了したが、マイページ・ホーム画面が認証必須のためB（自動化）は実機確認ができない | 名前・アイコン変更が再ログインしても消えないか、記念日設定がふたりに共有されるか、`married_upcoming`の表示が実際にどう見えるかが未確認 | **020とまとめて人間へ依頼する**（L80参照。020が同じホーム画面を作り変えたため、019単体の依頼を送った直後に020が完了し、まとめる方が手間が少ないと判断した） |
| L80 | 020（ホームの再構成）はコード側完了・Rレビュー依頼前だが、ホーム・タイムライン・思い出・統計の各画面が認証必須のためB（自動化）は実機確認ができない | パネルの見た目・遷移・記念日カードの非表示時の挙動・統計ページの3/4件表示が実際にどう見えるかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/020/manual-check.md`参照（019分も含む）。**014のデモはこの画面を最初に見せるため、この受け入れが本ブロッカーになる** |
