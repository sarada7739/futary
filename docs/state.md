...
**最終更新**: 2026-08-30 / セッションB（**019・020ともにRの受け入れを得て
mainへマージ済み（PR #127）。PR #107はAが救出したPR #129にマージ済みの
ため、B判断でクローズした。次は019+020まとめての人間の実機確認を依頼する
（L79・L80）**）。ホームを投稿一覧から「状態を見て入口を選ぶ画面」に
...
| L80 | 020（ホームの再構成）はRの受け入れを得てmainへマージ済み（PR #127）だが、ホーム・タイムライン・思い出・統計の各画面が認証必須のためB（自動化）は実機確認ができない | パネルの見た目・遷移・記念日カードの非表示時の挙動・統計ページの3/4件表示・思い出/統計からホームへ戻る導線（Rレビューで追加）が実際にどう見えるかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/020/manual-check.md`参照（019分も含む）。**014のデモはこの画面を最初に見せるため、この受け入れが本ブロッカーになる。次はこの受け入れ依頼を人間へ行う** |
