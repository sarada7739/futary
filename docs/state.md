...
のコード側実装完了。PR #140としてRレビュー依頼中。**スキーマ
（`is_shared`列・CHECK）・契約（`isShared`/`canEdit`）・
`event.update`/`event.delete`の認可WHERE句・`event.list`の`canEdit`計算・
UI（「ふたりの予定にする」・編集不可の表示）まで実装した。security-auditorで
High 1件（マイグレーションのSELECT不具合。修正・実機確認済み）・Medium 3件・
Low 3件を検出、すべて対応済み。**さらにRレビューで「kindの変更が権限を奪う」
問題への対応（PR #142）が指摘の半分（自分が締め出される経路）しか塞げて
いないと発覚（相手を締め出す経路が残っていた）。Aが「更新後も実行者が
編集できること」の条件を追加する判断を出したところ、Rがさらに「共有plan
にしてから持ち主が非共有にする」2段階での迂回を発見（PR #143）。最終的に
Aが権限の条件を「操作が安全か」ではなく「状態遷移が許されるか」で書き直す
判断をし、`kind<>'plan'`から`kind='plan'`への変換自体を拒む形（区分を
またぐ変換だけを見る）で決着した。画面側も「ふたりの予定を条件付きで
固定する」形をやめ、元がplan以外のときは種別の選択肢からplanそのものを
外す形に変えた。**Aが自己評価: 「021だけで自分の設計が3回続けて足りて
いなかった（更新前しか見ていない→実行者しか見ていない→1回の操作しか
見ていない）」。**security-report.mdの記録も、監査原文の範囲（「片方の
判断だけで」）に書き戻した。apps/api 277件・apps/app 107件すべて緑。
**次はRレビュー結果を待つ**）。
