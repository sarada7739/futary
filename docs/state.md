...
**最終更新**: 2026-09-13 / セッションA。**040 段階1（PR #289）: R の判定は必須修正 2 件。A が決めた: (1) `content` 属性の `'` で切れる正規表現を直す (2) Amazon 専用の規則は最終 URL（リダイレクト後）のホストで判定し、正規化も最終 URL に対して行う**（`amzn.asia/d/…` の短縮 URL が Amazon アプリの共有の形）。B が #289 に積み、R が確定 → マージ → デプロイ → 人間が Amazon の URL を 1 本貼る。
