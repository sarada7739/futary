...
送った直後に出た論点（L81〜L84）、L84までmainへマージ済み。**L83（FABの
影が正方形に見える不具合。PR #130）とL84（マイページ・予定登録のボタンが
単語の途中で折り返る不具合。PR #135）はRの受け入れを得てmainへマージ済み。
L81・L82（パネルの見た目・新規素材の扱い）はAが判断し（PR #131・#132）、
パネルアイコン6個をSVGで描き起こし`FeaturePanel`を4列×2行のグリッドに
全面書き換えた（`fix/panel-icon-grid`）。**Rレビューで「ラベルが2行＋
次フェーズ行のときセル高さ92pxでは足りない可能性がある（根拠のない値）」と
指摘され、`packages/ui`の`Text`に`lineHeights`（sizeごとの固定行送り）を
追加し、`CELL_HEIGHT`をそこから積み上げて導出する形（96px）に直した。**
テスト（apps/app 96件・packages/ui 7件）すべて緑、type-check・lint通過。
**次はRの最終確認を得てから、019+020+PR #130・#133・#135の受け入れを
人間へまとめて依頼する（「前回報告の2件は直った」を先に伝える。Rの助言）**）。
