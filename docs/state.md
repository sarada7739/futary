...
| L77 | **D1特有の制約を019実装中に発見。**FOREIGN KEYで参照される親テーブル（`couples`）にCHECK制約を追加しようとすると、drizzle-kitが生成する「新テーブルへ差し替える」形のマイグレーション（`PRAGMA foreign_keys=OFF; ...; DROP TABLE`）が`FOREIGN KEY constraint failed`で失敗する（D1がこのPRAGMAを無視して常にFKを強制するため。実測確認済み） | 今後、他の親テーブル（`couples`以外にも`posts`等）にCHECK制約を追加するタスクが出るたびに同じ失敗を踏みうる。019では`ALTER TABLE ADD COLUMN` + `TRIGGER`に手で書き換えて回避した（`packages/db/migrations/0009_couple_dates.sql`・`packages/db/src/schema/couple.ts`のコメント参照） | **Aへ報告済み（回答待ち）。**architecture.md 4節への反映（「PRAGMA foreign_keys=OFFはD1で無視される」と同じ節に、親テーブルへのCHECK制約追加はTRIGGERで代替する旨を追記）が適切と判断するかはA次第 |
| L78 | 019のタスク定義に「結婚した日（`married_date`）が未来の日付（結婚予定日をまだ迎えていない）のときの`stats.get`の`daysTogether`の仕様」が無い。`marriedDateSchema`は`anniversaryDateSchema`と同じ検証（1年後まで登録可）を流用しているため、未来の`married_date`を入力すること自体は現状ブロックしていない | 暫定実装は「負の値を出さない」制約だけ守り、`{status:"married", days:1}`を返す（`computeDaysTogether`のコメント参照）。`upcoming`のような別ステータスを設けるかはAの判断が要る | **Aへ確認依頼中（回答待ち）** |
