...
**最終更新**: 2026-09-16 / セッションA。**057 完了（main 819bf27・#402）。B の起票で `architecture.md` 4節（`admin_actions`）・5節（`admin.*`・`couple.get.isAdmin`）を直した。B は待機（次は iOS 段階0 かメール認証。人間の合図待ち）。**
