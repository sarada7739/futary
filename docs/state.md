...
- 人間の指示 2 件（B へ直接）: 使用量のカードは一覧の一番下・警告のカードは × で消せる（この起動の間。残りが変わればまた出す）。A が 3節に足した。R の記録（置く）: 消した状態の復元が useEffect なので枠が読めた直後の 1 フレームだけ警告が見えてから消える。人間が気にしたら useLayoutEffect に
