...
**最終更新**: 2026-08-29 / セッションB（PR #24マージ完了。ブランチ命名3種類・二重発火防止を規約化）
...
**この時点でM1完了。人間による受け入れ判定を依頼する**（下記「次の一手」参照）。
...
| M | タスク | 内容 | 状態 |
|---|---|---|---|
| M1 | 001〜005 | 足回り・デザイン基盤・認証・ペア成立・認可 | 実装完了（001〜005すべてmainへマージ済み）。**人間の受け入れ判定待ち** |
...
- 005-authorization-middleware（PR #19、`main` へ squash merge 済み）: 実装・
  テスト（64件緑）・security-auditor（2回）・Rレビュー（往復2回）まで完了。
  人間の指示により003・004の実機確認より先に着手した。M1の最終タスクのため、
  003・004と合わせてM1の人間受け入れ判定を待つ
...
**005のマージでM1（001〜005）の実装は完了。実機確認も一部進行した（2026-08-29）。**
**実機確認で見つかったバグ2件の修正（PR #22）もmainへマージ済み。**

1. 残り3項目の実機確認を行う: Cookie属性実地確認（`HttpOnly`/`Secure`/`SameSite=Lax`）・
   リロード後のログイン状態維持・ログアウト→サインイン画面へ戻るUI導線
   （`docs/tasks/003-auth-google.md` の「保留」節参照。ログイン成功・D1レコード
   作成・004のオンボーディング導線は確認済み）
2. 上記が済んだら 003・004 を「完了タスク」に移動する。005は認可の実装・テスト・
   監査・レビューが完了しているため、実機確認自体は不要（couple/inviteの認可は
   既にauthorization.test.tsで機械的に検証済み）。M1全体の受け入れ判定として
   005も合わせて「完了タスク」に移動する
4. M1受け入れ後、M2（006〜009: 投稿・画像・タイムライン・リアクション）に着手する
...
| L14 | 003で実際のGoogleログインが未検証（クライアント未入手のため人間判断で保留） | → **一部解決（2026-08-29）**。人間がクライアントを作成し、実機で確認した。実際のログイン成功（2アカウント）・D1への`user`/`account`レコード作成・004のオンボーディング導線（ペア作成→招待コード発行→別アカウントで参加）は確認済み。**Cookie属性実地確認・リロード後のログイン状態維持・ログアウトUI導線の3項目は未確認のまま残る**。詳細は`artifacts/003/manual-check.md`の追記部分参照 | 残り3項目を人間に確認してもらい次第、003を「完了タスク」に移動する |
...
| L26 | `packages/ui` の `Button`（`react-native-web` の `Pressable`）が、環境によっては1クリックで `onPress` を2回発火させる。003の実機確認で発見。認証フロー（`sign-in.tsx`）にはピンポイントでガードを追加して対応したが、`Button`/`Pressable` 自体の一般修正はしていない | → **一部解決**（PR #24）。`conventions.md` 4節に「副作用を伴うボタンは二重発火を防ぐ」を追加し、対象（ログイン・投稿・リアクション・招待コードの発行と使用・イベントの登録と削除）を明記した。**004の既存ボタン（couple.create/invite.issue/invite.accept）はまだガード未実装のまま残っている**（規約は決まったが遡及適用はしていない） | 004の既存ボタンへの実装、または`Button`コンポーネント自体への共通化は、M2着手前に検討するのが妥当（Rの見立て。最終判断はA） |
| L27 | `apps/app` にテスト基盤が一切無い（Vitest等が未導入）。`fix/oauth-callback-and-double-submit`で修正した2件（callbackURLの相対パス問題、ボタン二重発火）は実機確認でのみ検証でき、自動テストが無いため退行しても誰も気づけない（Rレビュー指摘） | M2（006〜009）は投稿・画像・タイムライン・リアクションとUI中心のタスクが続く。UIバグの回帰検知ができない状態が続く | M2着手前に検討するのが妥当（Rの見立て）。`docs/conventions.md` 6節はE2E（Playwright）を「主要導線」向けに規定しているが未導入。最終判断はA |
