...
- fix/reject-get-writes（Rの受け入れ済み。**対応対象としていたHigh指摘
  〈GET経由での書き込み手続き実行=CSRF〉は誤指摘だったと判明**。コード・回帰テストは
  残し記述を訂正済み。下記L52参照。security-report.mdの重複記録
  〈#59・#60が同じ誤指摘の訂正を別々の形で書いていた〉をRの指摘で解消するため、
  `fix/reject-get-writes-v2`ブランチとして組み直し中。PR #60はこの内容で更新する）
