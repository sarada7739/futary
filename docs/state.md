...
出て、Aがタスク024・025を起票した。025（招待コード再発行）を実装完了し
`feature/025-invite-reissue`ブランチとしてPR作成中。024（アカウント削除）は
着手前にRの先読みを受け、削除順序に冪等性の矛盾が見つかりAが訂正
（`couple_members`を後ろへ回す。`resolveCoupleContext`には触らない）。
訂正後の順序をRが再度実測し、`invite_failures`が言い換えの過程で
チャット上の要約から抜けていただけでタスク定義自体には正しく含まれている
ことを確認済み。024は次に着手する。**
