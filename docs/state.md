...
**最終更新**: 2026-09-13 / セッションA。**041（アルバム）を起票し、人間の指摘で訂正した（#292 → 訂正 PR）。B の手番。**042（まとめて ZIP）は 041 の後。
- **訂正**: 最初の版は「写真は投稿から選ぶ。アップロード無し」だったが、**モックはアップロード**（詳細の + と作成の「カバー写真を選択」）。
  **アルバムは自分の実体（`albums/` のキー）を持つ写真を直接アップロードする**形に書き直した。タイムラインの写真を入れるのは段階2（R2 の複製）
