...
**最終更新**: 2026-09-05 / セッションB。**036着手・実装済み。`task/036-seam-and-danger`
ブランチで作業。継ぎ目修正・dangerバリアントとも実装したが、継ぎ目の実測が
タスク定義の基準を文字どおり満たせていない点をAに判断を仰いでいる
（下記）。PRはまだ出していない。**

## 036: 継ぎ目の実測結果（Aへの確認事項）

`packages/ui/assets/bokeh.png`にアルファのランプ（下35%を255→0）を
焼き込み、`Screen`から固定色へのフェードレイヤーを削除した。
修正前（`f48ae5a`時点）はy=420に最大差4.0の明確な段差があったが、
修正後は同じ位置の最大差が1.0まで下がった。**ただし、画面全体では
差が1.0以上の行がなお172行残っている**（最大2.0）。

切り分けのため、ボケ画像が一切乗らない純粋なグラデーション領域
（y=500〜800）でも同じ測定をしたところ、そこでも39行が差1.0以上
（最大1.0）出た。スクリーンショットのPNG量子化・ブラウザのグラデーション
描画自体に行単位で1.0前後の基底ノイズが乗ることが分かった。ボケの
フェード帯（y=270〜420）はこれよりやや高い86行・最大2.0。

**「200行以上完全に平坦」という、今回問題になった特徴的なパターンは
消えている**（20行以上の完全な平坦区間: 修正前後とも無し。ただし修正前は
y=420に明確な段差があったのに対し、修正後はその段差だけが消えている）。
一方で「差が1.0以上の行が無いこと」という基準は、このレンダリング経路
（スクリーンショット→PNG→行ごとの平均色）では、ボケの有無に関わらず
文字どおりには成立しない。

詳細・数値は`artifacts/036/seam.md`参照。停止条件「アルファのランプでも
継ぎ目が消えなかったら、測定結果を持ってAに上げる」に該当するかも含め、
Aの判断を待つ。
