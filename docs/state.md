...
**最終更新**: 2026-08-28 / セッションB（005 マージ完了。M1完了、人間の受け入れ判定待ち）
...
**005のマージでM1（001〜005）の実装がすべて完了した。次は人間によるM1受け入れ判定。**

1. 人間が Google Cloud Console で OAuth クライアントを作成し、`.dev.vars` の
   `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` を実際の値に差し替える
2. 実機確認（実際のGoogleアカウントでのログイン）を行う:
