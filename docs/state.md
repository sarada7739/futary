...
**最終更新**: 2026-09-16 / セッションB。**056（PR #398）: A の決定（#400。0節 #3・#3b・#3c）で直した。親を飛ばすのは `exitGuestMode` だけ・框の中はサインイン画面を出さず「デモを読み込めませんでした」+「アプリを開く」・お知らせのシートは框の中で出さず `releaseSeen` も書かない。T4b・T7 緑。R の手番（2 回目）。**
