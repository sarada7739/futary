...
**最終更新**: 2026-08-30 / セッションB（**013（思い出し）を実装し、
Rへレビュー依頼済み**（ブランチ`task/013-memory`）。`memory.get`（新規。
探索順4段: 1ヶ月前→半年前→1年前→7日以上前からランダム1件→該当なしnull）・
`apps/app/components/memory-card.tsx`を実装し、ホームに組み込んだ。
**この時点でM3（017→010〜013）が実装としては完了。**着手前にRが2件を
先読み指摘: **L69**（`memory.get`に`deleted_at IS NULL`が無い。012のL65と
同型が2回続いたため、Aが個別修正ではなく`architecture.md`4節に「`posts`を
読むクエリには必ず`deleted_at IS NULL`を含める。例外なし」を規則化。読む場所の
一覧表付き。4段すべてに実装済み）・実装助言2件（ランダム選択を`(coupleId,
JST日付)`を種にした決定的なハッシュ+`ORDER BY created_at, id`で完全に決定的に
する。画像は署名付きGET URLを発行する）。7日境界は「ちょうど7日前を含む」で
Rと合意し、両側をテストで固定。テストはapps/api 193件（+17）・
apps/app 61件（+5）すべて緑、型チェック・lint通過。「タップで元の投稿へ
遷移する」はホーム画面に個別ルートが無いため、既存のImageViewer（017）で
画像を全画面表示する形に読み替えた（設計判断。実装メモに明記）。
**ホーム画面は認証必須のためB（自動化）は実機確認ができない**
（`artifacts/013/manual-check.md`。L70として起票。**M3全体
〈L59・017・L62・R-37・L68・本タスク分〉をまとめて人間の受け入れ判定を
依頼する**。013の停止条件どおり）。
...
| M3 | **017** → 010〜013 | **画像の全画面表示** → カレンダー・統計・思い出し | 実装完了（013実装完了・R待ち）。**人間の受け入れ判定待ち** |
...
- 013-memory（ブランチ`task/013-memory`）— 実装完了。Rへレビュー依頼中。
  **完了すればM3全体が実装完了となる。** 詳細は`artifacts/013/test-results.md`・
  `docs/tasks/013-memory.md`実装メモ
...
16. ~~次はM3の013（思い出し）に着手する~~ → **実装完了。Rへレビュー依頼中**
    （ブランチ`task/013-memory`）
17. **013のRレビュー結果を待つ。** 対応後は`artifacts/013/review.md`に保存する
    （conventions.md 8節）
18. **013マージ後、M3全体（017・010・011・012・013）の人間の受け入れ判定を
    依頼する。** 実機確認項目は`artifacts/013/manual-check.md`にL59・017・
    L62・R-37・L68分もまとめて列挙済み
...
| L70 | 013（思い出し）はコード側完了だが、画面が認証必須のためB（自動化）は実機確認ができない。**013の完了はM3全体の完了を意味する**（タスク定義の停止条件） | M3全体（017・010・011・012・013）の実機確認がまとめて未実施のまま。特に月末クランプ（L61）が013で初めて利用者から見える形になるが、それも含めて未確認 | **M3の受け入れ判定として、人間が実機で一度に確認する。**確認項目は`artifacts/013/manual-check.md`にL59・017・L62・R-37・L68分もまとめて列挙した |
| L70 | **`検索` タブが「準備中です」のまま残る。** `requirements.md` 5節で検索はスコープ外（「ボトムタブに枠はあるが中身は次フェーズ」）。`アルバム` はカレンダーに置き換えるため解消するが、検索は残る | **デモは公開前提**で、面接官が最初に触る画面である。5つのうち1つが「準備中です」なのは弱い。ただし「作りかけを隠す」ために枠だけ消すと、**次フェーズの構想が見えなくなる**という逆の効果もある | **016 の仕上げで決める。** 選択肢は3つ。(1) タブから外して4つにする (2) 「準備中です」ではなく**次フェーズの予告**として意図的に見せる (3) 検索を実装する（スコープ外の判断を覆すことになる）。A の推奨は (2)。**作っていないことを隠すより、何を作る予定かを見せる方が読み手に効く** |
