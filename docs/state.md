...
Rの受け入れを得てmainへマージ済み（PR #149）。Rレビューで軽微な指摘2件
...
mainへマージ済み（PR #149）。人間の実機確認（特にiPhoneのSafariでホイールの
値が確定するか）はまだ。**リモートD1への0011適用はマイグレーション自体の
リスク（既存の時刻データ）を考えると人間の実機確認後に行う方が安全と判断し、
まだ実施していない。**次は023（付き合った日を登録時に聞かない）に着手する
（B担当。Aは設計のみで実装は書かない）。**着手前にRへ先読みを依頼する
（019〜022と同じ進め方）。
...
シード仕様はPR #119で固め済みとRから聞いている）。**022はRの受け入れを得て
mainへマージ済み（PR #149。人間の実機確認はまだ）。次は023に着手する**
（`artifacts/022/`参照）。
