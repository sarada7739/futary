...
**最終更新**: 2026-08-30 / セッションB（**019+020の受け入れ依頼を人間へ
送った直後、人間から2件の反応があった: (1) ホームパネルの見た目が
サンプル画像（アイコングリッド）と違う形（カードリスト）である点への質問
（B側の実装判断だったが伝え損ねていたと回答済み） (2) FABボタンの周囲に
薄い四角い影が見える不具合の報告、および新規デザイン素材
（`docs/sample/透過素材/UlQMVnAB.png`）の配置。FABの不具合は原因を特定し
`fix/fab-shadow-square`で修正、Rレビュー依頼中。新規素材はJPEGを.pngに
誤って拡張子を付けたもの（透過なし）と判明し、Aへ報告予定。次はRレビュー
結果を待つ**）。ホームを投稿一覧から「状態を見て入口を選ぶ画面」に
...
20. **019・020まとめての人間の実機確認を依頼する。**確認項目は
    `artifacts/020/manual-check.md`参照（019分も含む。L79・L80）
21. 021（予定の持ち主。個人の予定/２人の予定チェックボックス/翌日自動変換）に
    着手する。Aの設計は`docs/tasks/021-plan-ownership.md`に完成済み
    （Cron Triggers実現性確認・`is_shared`列追加・単一WHERE句での認可・
    冪等な変換ジョブ・実装完了後の必須security-auditor実行を含む）
...
| L79 | 019（記念日とプロフィールの設定）はコード側完了・レビュー完了・リモートD1へのマイグレーション適用も完了したが、マイページ・ホーム画面が認証必須のためB（自動化）は実機確認ができない | 名前・アイコン変更が再ログインしても消えないか、記念日設定がふたりに共有されるか、`married_upcoming`の表示が実際にどう見えるかが未確認 | **020とまとめて人間へ依頼する**（L80参照。020が同じホーム画面を作り変えたため、019単体の依頼を送った直後に020が完了し、まとめる方が手間が少ないと判断した）。**020もRの受け入れを得てmainへマージ済み（PR #127）となったため、まとめての依頼が可能になった** |
| L80 | 020（ホームの再構成）はRの受け入れを得てmainへマージ済み（PR #127）だが、ホーム・タイムライン・思い出・統計の各画面が認証必須のためB（自動化）は実機確認ができない | パネルの見た目・遷移・記念日カードの非表示時の挙動・統計ページの3/4件表示・思い出/統計からホームへ戻る導線（Rレビューで追加）が実際にどう見えるかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/020/manual-check.md`参照（019分も含む）。**014のデモはこの画面を最初に見せるため、この受け入れが本ブロッカーになる**。019+020の受け入れ依頼は人間へ送付済み。回答待ちの間に人間からL81・L82の2件が新たに出た |
| L81 | 020の機能パネルの見た目が、人間が出したモックアップ（円いアイコン＋短いラベルの4+3グリッド）と違う形（タイトル＋説明文を書いたカードを縦に並べる形）になっている。020のタスク定義はパネルの**枚数**の差（7→8。上記L80解消時に先に伝達済み）は明記していたが、**見た目の形**の差は明記も伝達もしていなかった | パネル用のアイコン素材が無かったため、B（このセッション）が`packages/ui`の既存`Card`コンポーネントで代用した実装判断だった。人間に問われて初めて、形の違いを伝え損ねていたことに気づいた（枚数は伝えたが形は伝えていなかった、という部分的な見落とし） | 人間へ経緯を説明済み。**人間の判断待ち**。近づけるならアイコングリッド化のタスクを別途起票する。L82の新規素材（8枚分のアイコンを含む）が使えるかどうかとあわせて検討する |
| L82 | 人間が`docs/sample/透過素材/UlQMVnAB.png`にホーム機能パネル8枚分のアイコン・カード見本・FAB/ハート/ベル/アルバム/検索/人物アイコンのスプライトシートを配置した（L81のアイコングリッド化に使える可能性がある）。**ファイルの中身を確認したところ、拡張子は`.png`だが実体はJPEG（`ffd8ffe0`のマジックバイト）で、アルファチャンネルを持たない＝透過していない** | JPEGには透過を表現する仕組みが無いため、このファイルのままではアイコンとして白背景ごと切り出すことになり、円形ボタン等の上に重ねると背景の四角が見えてしまう（L83のFAB影の不具合とは別の理由で、同じ「意図しない四角」が起きうる） | **Aへ報告し出自・用途の記録（PR #29と同じ形）を依頼する予定。**再エクスポート（本当に透明なPNG/WebP）が必要なことも人間に伝える |
| L83 | ホーム下部の「＋投稿」FABの周囲に薄い四角い影が見えると人間が画像付きで報告した | `apps/app/app/(tabs)/_layout.tsx`の`FabTabButton`が`shadow.fab`を`Pressable`に適用していたが`borderRadius`が無く、react-native-webのbox-shadowが正方形のまま落ちていた（`Card`は`shadow.card`と`borderRadius`を必ず対で持つのに対し、ここだけ抜けていた）。`borderRadius: 28`を追加して修正し`fix/fab-shadow-square`としてRレビュー依頼中 | **Rレビュー待ち。**修正はコード側完了・type-check/lint/テスト（apps/app 96件）緑。画面が認証必須のためB（自動化）は実機確認ができず、`artifacts/fix-fab-shadow-square/manual-check.md`に記録済み。人間の実機確認はL80とまとめて回収する |
