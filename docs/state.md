...
**最終更新**: 2026-08-30 / セッションB（**010（カレンダーAPI）、Rの受け入れを得て
mainへマージ済み（PR #86）。ブランチも削除済み。** `events`テーブル・
`lib/date.ts`（JST集約）・`event.list/create/update/delete`を実装し、
繰り返し記念日の射影（年またぎ・400日上限・3暦年に触れる窓・うるう年02-29の
両方向）をテストで証明した。Rレビューで`lib/date.ts`内の矛盾
（`yearsBefore`が`projectMonthDay`の02-29規則と食い違う）が見つかり修正、
`monthsBefore`の月末繰り上がりの扱いはL61としてAへ判断を依頼し
「**存在しない日付は、その月の末日に寄せる**」という一般則に決定
（`architecture.md` 5節、PR #87、mainへマージ済み）。`projectMonthDay`を
一般化し`monthsBefore`/`yearsBefore`ともこれに統一。**Rの最終確認では
私が指示していない範囲（負のnでの5/31→6/30、年跨ぎの月計算等）も含めて
7件を実行して確認し、マージ前修正なし・再レビュー不要で受け入れ**。
テスト175→178→181件。詳細は`artifacts/010/review.md`参照。005の認可テストも
`event`系4手続きを追加した上で緑。010単体はsecurity-requirements.md 10節1の
必須監査対象に該当しないため単体監査はせず、M3の他タスクとまとめる方針
（ユーザー確認済み）。**次はM3の011（カレンダーUI）に着手する**（着手前に
`docs/tasks/011-calendar-ui.md`とAのPR #84〈月グリッドの取得範囲〉を読む）。
...
| M3 | **017** → 010〜013 | **画像の全画面表示** → カレンダー・統計・思い出し | 着手中（017・010完了。次は011） |
...
10. **次は011（カレンダー画面）に着手する。** 着手前にAのPR #84
    （月グリッドの取得範囲。28〜42日で一定しない、12月・1月は年をまたぐ）を
    読む。010で作った`event.list`の400日制限と噛み合う設計になっているはず
    （Rの指摘）
