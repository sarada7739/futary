...
**最終更新**: 2026-08-30 / セッションB（**L59（画面最大幅）の実装完了。**
`packages/ui`の`Screen`が既定で`layout.maxWidth`（640px）を適用する形にした。
opt-out（`unconstrained`プロパティ）を新設。型チェック・lint・テストすべて緑、
サインイン画面での実測（`maxWidth: 640px`が実際に効いていること）も確認済み。
認証必須画面のPC幅実機確認は017とまとめて後で回収する（`artifacts/
fix-layout-maxwidth/manual-check.md`）。次は017（画像の全画面表示）着手）
