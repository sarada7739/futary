...
実装完了。Rレビュー依頼前。** 022・023はマージ・リモートD1適用済みで完全に
閉じている（`65b1f11`・`ceb14ae`。詳細は下の記録を参照）。人間は就寝中のまま。
A経由で「014→015→016のデプロイ前まで、人間の確認を挟まず止まらずに進めて
ほしい（実機確認はまとめて後で）」との指示を受けたため、Rの受け入れは
