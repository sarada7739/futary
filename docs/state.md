...
（`task/010-calendar-api`）。Rレビュー往復1回目の必須修正＋L61（Aの判断）を
反映済み。再レビュー待ち。** `events`テーブル・`lib/date.ts`（JST集約）・
`event.list/create/update/delete`を実装し、繰り返し記念日の射影
（年またぎ・400日上限・3暦年に触れる窓・うるう年02-29の両方向）をテストで
証明した。**Rが`lib/date.ts`内の矛盾を発見**（`yearsBefore`が`projectMonthDay`の
02-29規則と食い違い`2023-03-01`を返していた）→修正。`monthsBefore`の月末
繰り上がりの扱いはL61としてAへ判断を依頼し、**「存在しない日付は、その月の
末日に寄せる」という一般則に決定**（PR #87。`architecture.md` 5節に新設。
射影の02-29規則はその一例と位置づけ直された）。`projectMonthDay`を一般化し、
`monthsBefore`/`yearsBefore`ともこれに統一。テスト175→178→181件と推移。
詳細は`artifacts/010/review.md`参照。005の認可テストも`event`系4手続きを
追加した上で緑。**010単体はsecurity-requirements.md 10節1の必須監査対象に
該当しない**ため、単体監査はせずM3の他タスクとまとめる方針（ユーザー確認済み）。
...
- 010-calendar-api（PR #86〈ブランチ `task/010-calendar-api`〉。Rレビュー往復1回目の
  必須修正（`yearsBefore`の矛盾）とL61（Aの判断: 存在しない日付は月末に寄せる。PR #87）を
  反映済み、再レビュー待ち。`artifacts/010/test-results.md`・`artifacts/010/review.md`参照。
