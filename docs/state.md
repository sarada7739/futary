...
**最終更新**: 2026-08-29 / セッションB（M1実機確認 全項目完了。人間の最終受け入れ確認待ち）
...
| M | タスク | 内容 | 状態 |
|---|---|---|---|
| M1 | 001〜005 | 足回り・デザイン基盤・認証・ペア成立・認可 | 実装・実機確認完了（2026-08-29）。**人間の最終受け入れ確認待ち** |
...
## 進行中タスク（実機確認は完了。人間の最終確認待ち）

- 003-auth-google（PR #5、`main` へ squash merge 済み）: 実装・テスト・監査・
  レビュー（往復2回）まで完了。**実機確認も2026-08-29に全項目完了**
  （ログイン成功・D1レコード作成・リロード後のセッション維持・Cookie属性・
  ログアウト導線）。人間から「これで003を完了タスクにしてよい」旨の
  明示的な確認を得たら「完了タスク」へ移動する
- 004-couple-and-invite（PR #9、`main` へ squash merge 済み）: 実装・テスト
  （52件緑）・security-auditor（2回）・Rレビューまで完了。**オンボーディング
  導線の実機確認も2026-08-29に完了**（ペア作成→招待コード発行→別アカウントで
  参加）。003と同様、人間の明示的な確認を得たら「完了タスク」へ移動する
- 005-authorization-middleware（PR #19、`main` へ squash merge 済み）: 実装・
  テスト（64件緑）・security-auditor（2回）・Rレビュー（往復2回）まで完了。
  認可はauthorization.test.tsで機械的に検証済みのため実機確認は不要。
  003・004と合わせて人間の確認を得たら「完了タスク」へ移動する
...
**M1（001〜005）の実装・実機確認は2026-08-29に全て完了した。**
実機確認で見つかったバグ2件の修正（PR #22）もmainへマージ済み。
ブランチ命名規約の整理（`fix/`新設・二重発火防止の規約化。PR #24）も反映済み。

1. 人間からM1の最終受け入れ確認（003・004・005を「完了タスク」に移動して
   よいという明示的な合図）を得る
2. 確認が取れたら 003・004・005 を「完了タスク」に移動する
3. M1受け入れ後、M2（006〜009: 投稿・画像・タイムライン・リアクション）に着手する。
   着手前に L26（`Button`の二重発火防止をコンポーネント側に組み込むか）・
   L27（`apps/app`にテスト基盤が無い）の2論点をAが検討する予定
   （Rの見立てで「M2着手前が妥当」とされている）
