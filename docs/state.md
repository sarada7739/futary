...
| ~~L2~~ | ~~ロゴのスクリプト体をどう用意するか~~ → **解決**: `docs/sample/sample.png` からロゴ部分を矩形のまま切り出し `apps/app/assets/logo.png` として採用（002で対応）。背景色がトークンの `bg` と同一のため透過処理はしていない | 015 のLPアセットでも同じ画像を流用予定 | 解決済み（002） |
