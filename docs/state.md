...
**最終更新**: 2026-09-05 / セッションB。**035（見た目を作り込む）着手。
部品ができた時点で停止し、Aへレビューを依頼中。`task/035-rich-ui`
ブランチで作業。14画面へはまだ進んでいない。**

モックアップ（`docs/sample/mockup/`）をコミットし、`expo-linear-gradient`を
導入・Web動作確認済み。新規トークン2件（`gradients`・`shadow.glow`。
後者は当初`avatarGlow`として提案したがAが「アバターの光るリングとFABの
光彩は同じ見た目」という理由で改名）をAに提案・承認され、
`docs/architecture.md`7節に反映済み（PR #234）。

**部品**: `Screen`（グラデーション地）・`Badge`（新規。ピル型バッジ。まだ
どこからも呼んでいない）・`Avatar`（`glow`オプション追加）・`FeaturePanel`
（白いカード化。020の「枠線も背景も持たない」判断を覆した。「次フェーズ」→
「COMING SOON」）・ボトムタブ（浮かせてピル型に。中央＋ボタンに光彩）を作った。

ローカルにデモペアをシードし、Browser paneでゲスト閲覧のホーム画面を
モバイル幅・デスクトップ幅の両方で確認した。グラデーション地・白い
機能パネル・浮いたタブバー・光るFABが実際に描画されることを確認済み。
詳細・確認結果は`artifacts/035/checkpoint.md`参照。

`pnpm --filter @futary/app test`（222件緑。「COMING SOON」文言変更に
合わせテスト1件更新）・`pnpm -r type-check`・`pnpm -w eslint .`、全て通過。

**次にやること**: **ここでいったん停止**（タスク定義0-2節の必須の停止点）。
人間・Aの確認を待つ。承認され次第、ホーム・サインインの個別の作り込み→
残り12画面の通し確認へ進む。
