...
送った直後に出た論点（L81〜L84）、すべてRの受け入れを得てmainへマージ済み。**
L83（FABの影。PR #130）・L81/L82（パネルのアイコングリッド化。
PR #131・#132・#133）・L84（マイページ・予定登録のボタン折り返し。
PR #135）。`Text`に`lineHeights`を追加した副作用で、他の画面（タイムライン
本文・カレンダーの予定行・ボタンの高さ等）も行間がわずかに変わる
（意図した変更。固定高さの箇所は`Text`を内包しないため無関係とRが確認済み。
`artifacts/020/manual-check.md`項目15）。**次は019+020の受け入れを人間へ
依頼する（前回報告の2件〈FABの影・ボタン折り返し〉が直ったことを先に
伝える。Rの助言）**）。
...
20. ~~019・020まとめての人間の実機確認を依頼する~~ → 依頼を送った直後に
    ...
    mainへマージ済み（PR #130・#131・#132・#133・#135）。改めて依頼する**
...
| L80 | 020（ホームの再構成。パネルのアイコングリッド化・FABの影修正・ボタン折り返し修正まで含め全てmainへマージ済み）だが、ホーム・タイムライン・思い出・統計・マイページ・カレンダーの各画面が認証必須のためB（自動化）は実機確認ができない | パネルの見た目・遷移・記念日カードの非表示時の挙動・統計ページの3/4件表示・思い出/統計からホームへ戻る導線・FABの影・ボタン折り返し・パネルグリッドの高さが実際にどう見えるかが未確認 | **人間が実機で確認する。**確認項目は`artifacts/020/manual-check.md`（019分・L81〜L84分も含め全15項目）と`artifacts/fix-profile-primary-date-wrap/manual-check.md`参照。**014のデモはこの画面を最初に見せるため、この受け入れが本ブロッカーになる。019+020の受け入れ依頼を人間へ送付する（前回報告の2件が直ったことを先に伝える）** |
