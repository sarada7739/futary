...
| Medium | `event.ts`の`eventUpdate` | UPDATEのWHEREは更新前の行で評価されるため、同じUPDATE文で`kind`自体も変えられることを利用し、「記念日・会った日（どちらでも編集できる）を設定者でない側が編集し、その場でkindをplan・is_sharedを0にする」で相手を締め出せた（自分が締め出されることもある）。021以前は無害だったが、持ち主の概念を入れたことで権限を奪う手段になっていた | AがWHERE句に「更新後の状態でも実行者が編集できること」を追加する設計を決定（security-requirements.md 3節項目8） | **対応済み**。`(?newKind <> 'plan' OR ?newIsShared = 1 OR created_by = ?)`をWHEREに追加し、「自分を締め出す更新を拒む」形にした。画面側でも、非設定者が記念日・会った日や共有中のplanをplanに変えるとき「ふたりの予定」を立てたまま固定し外せないようにした。テスト4件（拒む2つ・通る2つ）を追加 |
