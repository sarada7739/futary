...
- 絵は `docs/sample/nisoine/`: `icon-1024.jpg`（N とハート。1024×1024）・`wordmark-transparent.png`（`Nisoine` の文字。透過 PNG。ぼかしの縁あり）・`wordmark-on-checkerboard.jpg`（同じ絵の JPG。予備）
...
| 3 | ワードマークの透過 | **人間の透過 PNG（`wordmark-transparent.png`。RGBA 1536×1024）を使う。**文字の周りにぼかしの縁（アルファ 1〜127 の画素が約 2.4 万）があるので、**B がアルファの薄い画素を透明に落とす**（閾値は B。文字の縁 1〜2px だけ残す）。落とし切れなければ JPG から濃い茶を抜く | ぼかしの縁は濃い背景で滲む |
...
- ワードマークの縁が落とし切れない（滲む・文字が欠ける）→ 人間に縁の無い透過 PNG を頼む。他は先に進める
