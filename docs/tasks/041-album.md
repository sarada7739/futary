...
| # | 論点 | 決定 | 理由 |
|---|---|---|---|
| 1 | アルバムの写真はどこから来るか | **アルバムに直接アップロードする**（`post.uploadUrl` と同じ署名付き PUT。R2 のキーは `couples/{coupleId}/albums/{imageId}.jpg`）。**タイムラインの写真を入れるのは段階3**（サーバが R2 の実体を複製する） | モックがそう描いている。アルバムの写真が投稿の行に依存すると、投稿を消したときアルバムからも消える。**アルバムは残しておくもの**で、投稿とは寿命が違う |
...
CREATE TABLE album_photos (
  ...
  taken_at    INTEGER NOT NULL,                    -- 並び順に使う。アップロードなら追加した時刻。段階3の複製なら元の投稿の created_at
  created_at  INTEGER NOT NULL
);
...
| タイムラインの写真をアルバムに入れる | **段階3**（9節）。段階1では作らない |
| アルバムのカラー | **持たない**（人間の指示） |
| 検索窓 | **置かない**（0節 #5） |
| ビューアのハート | **置かない**（0節 #6） |
...
## 9. 段階3: タイムラインの写真をアルバムに入れる（要否は人間に聞く）
...
**要否は A が人間に聞いてから。**要らなければ 9節を「やらない」に移す。
...
040 のマージ後（済）。段階2（共有シート）は段階1のデプロイ後（済）に人間の要望で決めた。042 は段階2の後。段階3は要否を人間に聞いてから。
