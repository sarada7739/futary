...
> **起票の訂正（2026-09-13）**: 最初の版で A は「アルバムの写真は投稿の写真から選ぶ。専用のアップロードは作らない」と決めていた。
> **モックの詳細画面の「+」と作成画面の「カバー写真を選択」はどちらもアップロードである。**人間の指摘で直した。
> 以下は直したあとの定義。**投稿の写真をアルバムに入れる経路は段階2**（8節）。

## 0. 先に決めたこと（A。覆すなら人間が言う）

| # | 論点 | 決定 | 理由 |
|---|---|---|---|
| 1 | アルバムの写真はどこから来るか | **アルバムに直接アップロードする**（`post.uploadUrl` と同じ署名付き PUT。R2 のキーは `couples/{coupleId}/albums/{imageId}.jpg`）。**タイムラインの写真を入れるのは段階2**（サーバが R2 の実体を複製する） | モックがそう描いている。アルバムの写真が投稿の行に依存すると、投稿を消したときアルバムからも消える。**アルバムは残しておくもの**で、投稿とは寿命が違う |
...
CREATE TABLE album_photos (
  ...
  taken_at    INTEGER NOT NULL,                    -- 並び順に使う。アップロードなら追加した時刻。段階2の複製なら元の投稿の created_at
  created_at  INTEGER NOT NULL
);
...
- **選択**: 押すと選択モードに入る（ヘッダーが「N 枚を選択中」「やめる」になり、写真にチェックが出る）。
  下に「カバーにする」（**1 枚のときだけ**押せる）「削除」（確認: 「N 枚を削除しますか？」）。**タイムラインには無い**。
...
| タイムラインの写真をアルバムに入れる | **段階2**（8節）。段階1では作らない |
| アルバムのカラー | **持たない**（人間の指示） |
| 検索窓 | **置かない**（0節 #5） |
| ビューアのハート | **置かない**（0節 #6） |
| まとめてダウンロード（ZIP） | **042** |
...
## 8. 段階2: タイムラインの写真をアルバムに入れる（段階1の受け入れ後）

**サーバが R2 の実体を複製する**（`env.BUCKET.get` → `put`。Worker を通るが署名も CPU もほぼ使わない）。
アルバムの写真はどれも自分の実体を持つので、**投稿を消してもアルバムは壊れない**。容量は 2 倍になる（R2 の無料枠 10GB。1600px の JPEG 1 枚 300〜600KB）。

| 手続き | 入力 | 備考 |
|---|---|---|
| `album.copyFromPosts` | `{ id, photos: [{ postId, position }] (1〜20) }` | 自ペアの `post_images` に無い ref は `INVALID_INPUT`。`taken_at` = 元の投稿の `created_at`、`caption` = 投稿本文。同じ元から 2 回入れられる（止めない。写真は写真） |

画面: 詳細の FAB を押したとき「写真を選ぶ」「タイムラインから選ぶ」の 2 択。後者はタイムラインの写真の 3 列グリッド（`photo.list`）で複数選択 → 「N 枚を追加」。

**段階1のマージ後に着手。**段階1の受け入れで A が段階2の要否を人間に確かめる（要らなければ 8節を「やらない」に移す）。
