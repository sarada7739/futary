...
- `invite.accept`: コードを受け取ってペアに参加する
  ...
  - 招待の消費と参加の INSERT は `batch()` にまとめる。
    片方だけ成立する状態を作らない
  - 失敗回数を **`user_id` 10回/時間 と IP 50回/時間の二本立て**で制限する。
    超過で `RATE_LIMITED`（`docs/security-requirements.md` 4節「レート制限のキー」）
- オンボーディング画面
...
| 有効期限 | 24時間 |
| 同時発行数 | 1ペアにつき1件 |
| レート制限 | `invite.accept` の失敗を `user_id` 10回/時間 + IP 50回/時間 |
