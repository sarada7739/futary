...
| 4 | 置き場所 | **ヒーローの直後に節「さわってみる」**（`id="demo"`）。**2 列: 左にスマホ・右に見出し + 触り方の案内 + ボタン**（059）。「基本機能は無料」の統計カード（054）は残す | 上で見た例（ゆい & れん・576 日目）が下で動いている形 |
...
| 8 | `sandbox` | **付けない** | 同じオリジンで、付けると `window.top` が壊れる（#3 の親への遷移に要る）。Cookie は #12 で送らないので `sandbox` で切る必要も無い |
| 9 | 見出し・文言 | `さわってみる` / `デモ画面です。投稿は見るだけで、書き込みはできません。` / 触り方の案内（059 の 0節 #5）/ ボタン `自分たちで始める`（→ `/app/`） | 054 の 3 節と同じく、ここに無い文言は足さない |
| 10 | デモペアの負荷 | 何もしない。LP の訪問者がスクロールした分だけ `couple.get` 等の読み取りが増える。既存のレート制限のまま | 読み取り専用。問題が出たら測ってから |
| 11 | OGP・sitemap | 触らない | 節が増えるだけ |
| 12 | 框の中に本人のデータを映さない | **框の中（`window.top !== window.self`）では、API と認証の fetch に Cookie を送らない**（`credentials: "omit"`。同じオリジンでも付かない）。サーバから見て未認証 = 常にデモペア。二重に `_layout` の `isAuthenticated` も框の中では false（人間の指示。2026-09-16） | ログイン中のブラウザで別タブの LP を開いたとき、枠の中に自分たちの投稿が映る経路を塞ぐ。LP は誰かに画面を見せながら開くページ |
...
- `apps/landing/index.html`・`style.css`: 節「さわってみる」（0節 #4〜#7・#9）。`assets/phone-frame.png`（画面部分が透明な PNG。**150KB 以下**を目安。超えるなら幅 1024 → 800 に）
- 框の中では fetch に Cookie を送らない（0節 #12。API クライアントと Better Auth のクライアントの両方）
...
- `wrangler dev` で `/` を 1280px 幅で開き、枠の中でデモがスクロール・タブ切り替えできる。デモの帯の「ログイン」を押すと**親ページ**が `/app/` に移る（框の中でサインイン画面が出ない）。**ログイン中のブラウザで `/` を開いても枠の中はデモ**（request に Cookie が無い）。375px 幅では節が出ない。画面キャプチャを `artifacts/056/`
