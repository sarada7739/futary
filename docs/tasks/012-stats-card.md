...
## 目的
ホーム画面の最上部に「付き合って○日目」「~~会った回数~~ → **会った日数**」を表示する（018 で改名）。
...
| ~~`meetupCount`~~ → **`meetupDays`** | `events` の `kind = 'meetup'` の件数。**018 の部分 UNIQUE により1日1件なので日数と一致する。**現行の定義は `architecture.md` 4節が持つ |
...
- カードのデザイン
  - 2人のアバターを並べ、間にハートを置く
  - 「付き合って **365** 日目」を大きく、「**会った日数: 48日**」を小さく（018 で改名）
