...
| 5 | 既に枠を超えている分 | **このタスクでは足せないだけ。**有料をやめたあとの扱い（猶予 → 鍵）は **047** | 047 は持ち出し（048 段階1）が本番に出てから |
...
- **一覧の一番下（アルバムのカードの下）**に **「写真の使用量」のカード**（人間の指示。2026-09-14。絵 `05` はタイムラインの直下だが、人間が一番下と決めた）: バー（`used / limit`）・「27 / 30 枚」・「あと 3 枚」・右に**「プレミアムで 50 万枚まで ›」**（→ `/premium`）
...
- **残りが 5 枚以下**のとき、FAB の上に警告のカード（絵 `01`）: 「残り N 枚です」「あと N 枚で上限（無料プラン 30 枚）に達します」「プレミアムで 50 万枚まで」（→ `/premium`）。
...
- **残り 0 で FAB を押す**と、選ぶ前にシート（絵 `04`）: 鍵の絵・「写真の上限に達しました」「大切な思い出をもっと残すために、プレミアムプランへ。」・
  現在のプラン（無料: 「30 枚まで保存可能」の 1 行だけ。「アルバムグループ 3 つまで」は書かない）・矢印・プレミアム（「写真 50 万枚まで」の 1 行だけ。価格は 048 まで出さない）・
...
- **月額／年額の切り替えとカードは 048 で**（価格を出すのは買えるようになってから）。このタスクでは**「できること」のカード 1 枚**: 「写真 50 万枚まで」の 1 行だけ（他の行は存在しない機能なので書かない）
...
| | 扱い |
|---|---|
| 決済（Stripe Checkout・Webhook）・価格の表示・申し込みボタン | **048。**`couple_plans` に `source='stripe'`・`expires_at` を書く形になる。価格は ¥420/月・¥4,200/年（人間の決定。048） |
| 無料トライアル | **作らない**（人間の指示） |
| 有料をやめたあとの鍵 | **047** |
...
044 の後。042 の上限 50 は後回しのまま。**048（ZIP で持ち出し + 決済）→ 047（鍵）の順。**

## 進捗

- 2026-09-14 実装 #325・#326（テスト）・#328（使用量カードは一番下）・#331（× で消せる）。R 受け入れ。デプロイ後、人間の実機 OK。人間の指示で本番のペアを paid にした（#335）。**完了。**次は 048 段階1（ZIP）
