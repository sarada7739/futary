...
## 完了条件
- [x] **ローカルで**未ログインのままデモペアのタイムライン・カレンダー・統計・思い出しが見える
      （**本番への投入は 016。**デプロイが 016 なので、014 の完了を本番に依存させない）
- [x] 書き込み系がサーバ側ですべて拒否される（テストで証明。既存の`authorization.test.ts`が担保）
- [x] デモペア以外のデータが取得できないことがテストで証明されている
- [x] **`DEMO_COUPLE_ID` が実在する非デモペアを指すとき拒否されることがテストで証明されている**
      （`security-requirements.md` 3節の項目6。**この値に初めて実在の ID が入るのがこのタスク**）
- [x] シードデータの出典が記録されている（`packages/db/seed/README.md`）
- [x] **シードを2回連続で実行して、2回目も成功し結果が同じである**
- [x] **生成されたマイグレーションに `CREATE INDEX` が2本入っている**（目視）
- [x] **適用前に CHECK 違反行を数え、結果を記録した**（ローカルD1は空のため0件。本番投入前に016でも実測すること）
- [x] `meetupDays` が実際に触って自然に見える数字になっている（94）
- [x] カレンダーに**2人の名前**と、**時間ありと時間なしの予定**が混ざって出る
- [x] **security-auditor の指摘で High 以上がゼロ**（デモ経路は本番データ漏洩の主要脅威 T4/T5。Medium 3件・Low 4件はすべて対応済み。`artifacts/014/test-results.md`参照）
- [x] `artifacts/014/` に証跡（**人間の実機確認の記録**、拒否のテスト結果）を保存
