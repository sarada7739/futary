...
- `apps/app/app/(tabs)/album-detail.tsx`
  - 起動時に `canShareFiles()` を 1 回評価する（`useMemo`）。**真のときだけ**選択モードの下のバーに「保存」（0 枚では押せない）。PC（偽）には出ない。ラベルは「保存」「カバー」「削除」（3節の「保存（N 枚）」は iPhone の幅で「保存（7 / 枚）」「カバーに / する」と折れたので、人間の決定で枚数を外した。枚数はヘッダーの「N 枚を選択中」）
  - **選択モードに入れる条件**を `canWrite`（メンバーのアルバム）**または** `canShare` に広げた。**タイムラインとゲストのアルバムは `canShare` が真なら「選択」が出て、下のバーは「保存」だけ**（「編集」「カバー」「削除」「+」は出ない）。偽なら今までどおり出ない
...
| # | 何を | どこで（テスト名） |
|---|---|---|
| T1 | `canShare` が偽なら選択モードに「保存」が出ない（カバー・削除はある）。真なら出て、0 枚では押せず、選ぶと押せる（枚数はヘッダー） | `test/album-detail-share.test.tsx`「選択モードの「保存」（042 T1）」 |
| T2 | 25 枚のアルバムで 21 枚選ぶと「保存」が押せず 1 行が出る。押しても `share`・`photo.downloadUrl` は呼ばれない。「削除」は押せる。1 枚外して 20 枚に戻すと「保存」が押せて 1 行が消える。PC は「保存」自体が無く、選択に上限が無いことは既存の「101 枚選んで削除」がそのまま緑 | 同「20 枚の上限（042 T2）」 |
...
1. **5 枚**: 「選択」→ 5 枚を選ぶ →「保存」→ 共有シートの「画像を保存」。写真アプリに 5 枚入ったか
