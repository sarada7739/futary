...
## 追補: 絵を人間の原本に差し替え（2026-09-17）

- `docs/sample/weather/icons-source.png`（1168×880・RGBA。#407）から `artifacts/058/scripts/make-icons.py` で 5 つを切り出した（上段真ん中は使わない）。マスの角丸の地は、縁からの塗りつぶし（白〜薄いピンクで R ≥ B の画素だけ）で透明に。**「隣との差」で広げると雲の淡い勾配を伝って雲まで消えた**（実測。雲が 67 画素になった）ので、絶対の色で判定する形にした。雲は青みがかった白（B > R）なので止まる
- 外接矩形を正方形 + 8% の余白 → 112×112。5〜10KB
- 画面は `artifacts/058/stage2/`（同じ流れを本物の絵で撮り直し）。仮の絵は消えた
- 気づき: 雲の絵は白に近く、ホワイトモードの白い地では薄い（人間の絵のまま。判断は人間）
