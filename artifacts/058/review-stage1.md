...
## 追補 PR #408（a281d16。人間の絵から切り出し）— R の判定

futary-R で a281d16 を checkout。ui 20 緑。コードの差分は `assets.ts` の注釈だけ。`worklog.md` は追記のみ。

**受け入れ。必須修正なし。**

- 5 枚とも 112×112 RGBA、四隅のアルファ 0（PIL で測った）。不透明の割合は 9〜25%（雲が一番多い）
