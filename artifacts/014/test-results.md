...
`__new_<表名>`が残る」を新設。**この節番号の指摘は、置き場所そのものの誤りを
見つける入口になった**: 014当時、この記述を含むスキーマ・CHECK・TRIGGER・
マイグレーション関連の記述240行が5節（API）へ丸ごと間借りしており、
Aが4節（データモデル）へ移した（`76dc05d`。「投稿のレスポンスに投稿者情報を
含める」というAPIの説明の途中に挟まっていたため段落も分断されていた）。
以降、この種の引用は「architecture.md 4節」でよい。
