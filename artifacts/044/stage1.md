...
## 追記（A の依頼。MAX_OUTPUT_TOKENS のコメントに数字の根拠）

1 回だけ追加で投げて completion_tokens の内訳を取った（`artifacts/044/trial-luna-usage.log`）: 204 文字の出力で completion_tokens 255、うち reasoning 86。本文は約 170 トークン（0.83 トークン/文字）。300 字なら本文 250 前後 + reasoning 100 弱で 1024 に収まる。A の文案の「約 450 トークン」と既存コメントの「約 125 トークン」はどちらも実測と合わないので、実測値でコメントを書いた。
