...
apps/app test:       Tests  80 passed (80)
apps/api test:  Test Files  14 passed (14)
apps/api test:       Tests  234 passed (234)
```

apps/apiは205件（018完了時点）→234件（+29。couple.test.ts +9・stats.test.ts +5・
authorization.test.ts +2・r2-signed-url.test.ts 新設5件・me.test.ts +8）。
apps/appは69件→80件（+11。profile-screen.test.tsx 新設9件・stats-card.test.tsx +2）。
...
2列にまたがる制約（`primary_date='married'ならmarried_dateが必須`）は
TRIGGER（`BEFORE INSERT`/`BEFORE UPDATE`）で表した。エラーメッセージに
...
`packages/db/migrations/0009_couple_dates.sql`参照。**この制約はarchitecture.md
に恒久化する価値があると判断し、Aへ報告した**（docs/worklog.md参照）。
...
- `apps/api/src/procedures/stats.ts`（変更） — `computeDaysTogether`が
  `primary_date`に従って`together`/`upcoming`/`married`/`hidden`を出し分ける。
...
- `apps/app/components/stats-card.tsx`（変更） — `daysTogetherLabel`が
  `married`/`hidden`に対応（hiddenは表示自体を出さない）

## 未確認・要確認事項

- **結婚した日が未来（結婚予定日をまだ迎えていない）のケースの仕様が
  タスク定義に明記されていない。** 暫定的に「負の値を出さない」制約だけ守り
  1日目として返す実装にしたが、Aへ確認を依頼した（docs/worklog.md参照）
