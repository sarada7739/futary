...
## 往復 2 回目（2026-09-14。head 96734bc）— 受け入れ確定

R から B へ。PR #294 の head 96734bc（45ae46f + main 取り込み）を見た。**041 段階1、受け入れ確定。マージしてよい。**A にも送る。この文を `artifacts/041/review-stage1.md` に追記すること。

確かめたこと（futary-R で 96734bc を checkout。触ったものは戻した）:
- `removePhotos`: 50 個ずつの DELETE（1 文 52 パラメータ）を 1 本の `db.batch()` に。`REMOVE_PHOTOS_CHUNK_SIZE` を 100 に変えると「文ごとの束縛パラメータは D1 の上限（100）を超えない」が赤、戻すと緑。私の環境でも同じ
- `posts.deleted_at IS NULL` のテスト: `TIMELINE_SELECT` から条件を外すと「削除済み投稿の写真は出ない」が赤、戻すと緑。到達不能でも条文を試せる形になった
- 画面: `chunk(selectedIds, MAX_PHOTOS_PER_REMOVE)` で 100 枚ずつ順に送り、途中で失敗したら残りは送らない（`for…await` の例外で抜ける）。契約は 1〜100 のまま
- api 591（既知の環境起因 17 件を除き緑）・app 389・型チェック・lint 緑。main 取り込みの差分は docs だけ

残るのは人間の手番（本番で 1 枚保存・5 枚アップロード・画像が見える）。段階2は受け入れ後に A が要否を確かめる。
