...
2026-09-16 / セッションB。タスク定義 `docs/tasks/056-demo-in-phone-frame.md`（main cddf7cb。#399 で 0節 #12・T3b、#400 で 0節 #3・#3b・#3c・T4b・T7）。
...
| `apps/app/app/_layout.tsx` | `isGuestMode` の初期値を `isDemoEntry()`（0節 #2。Web だけ）。`isAuthenticated = !!session && !inFrame`（二重の守り: 万一セッションが見えても框の中を実ユーザーの画面にしない）。**親を飛ばすのは `exitGuestMode` の 1 箇所だけ**（0節 #3。R の差し戻しを受けて `useEffect(showAuth && inFrame)` はやめた。`demoFailed` では飛ばない）。**框の中では `(auth)` を出さず**（`guard={showAuth && !inFrame}`）、未認証でゲストでもない状態は「デモを読み込めませんでした」+「アプリを開く」（`<a href="/app/" target="_top">`）の 1 行だけ（0節 #3b） |
| `apps/app/app/(tabs)/index.tsx` `lib/release-seen.ts` | 0節 #3c: 框の中では 043 の「新機能のお知らせ」を出さず、`markReleaseSeen`・`deferRelease` は框の中では書かない（同じオリジンの localStorage/sessionStorage を汚さない） |
...
| T3b | `frameCredentials()`: 外 include・中 omit。**本物の `lib/orpc.ts` の fetch が框の中で `credentials: "omit"`**（`orpc-frame-credentials.test.ts`）。框の中でセッションが見えても認証済みとして扱わずデモのまま | `demo-frame.test.tsx` `orpc-frame-credentials.test.ts` | 緑 |
| T4b | 框の中で `?demo=1` → `couple.get` が reject → **親は飛ばない**・サインイン画面は出ない・「デモを読み込めませんでした」+「アプリを開く」（`target="_top"`）。框の外の失敗はサインイン画面。框の中で `?demo=1` 無しも同じ 1 行 | `demo-frame.test.tsx` | 緑 |
| T7 | 框の中では未読でもお知らせのシートが出ず、`releaseSeen`（localStorage）も「後で」（sessionStorage）も書かれない。NEW のバッジ（読むだけ）は出る。框の外は今まで通り | `home-releases.test.tsx` | 緑 |
...
`pnpm lint`・`pnpm type-check`・`pnpm test`（api 736・app 581・db 32・date 67・ui 16）すべて緑。

## 画面と計測（`artifacts/056/`。`scripts/capture.mjs`。`build:public` の出力を `wrangler dev` で）

| ファイル | 何 |
|---|---|
| `pc-demo-home.png` `pc-demo-home-closed.png` | 1280 幅。框の中にデモのホーム（563 日目）。**お知らせのシートは出ない**（`releaseSheetInFrame: false`・`releaseSeenInStorage: null`。0節 #3c） |
| `pc-demo-calendar.png` | タブ「カレンダー」→ 框の URL が `/app/calendar` |
